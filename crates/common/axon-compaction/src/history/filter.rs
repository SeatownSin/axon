...
//!
//! Behavior is byte-for-byte identical for Axon chat (`T = Arc<AxonTurn>`).
...
///   summaries so their `<axon_user_queries>` sections can be split out.
...
/// Extracts **all** `<axon_user_queries>...</axon_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<axon_user_queries>";
    let end_tag = "</axon_user_queries>";
...
/// Extract a `<axon_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<axon_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<axon_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<axon_user_queries>\n");
    ...
    result.push_str("</axon_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<axon_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<axon_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<axon_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<axon_user_queries>` block.
...
/// `<axon_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<axon_user_queries>` from earlier rounds.
...
///   would see the prior `<axon_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<axon_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<axon_user_queries>"));
        assert!(out.ends_with("</axon_user_queries>"));
        assert!(out.contains("<axon_query>hello world</axon_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<axon_user_queries>\n<axon_query>first</axon_query>\n</axon_user_queries>";
        let inner2 = "<axon_user_queries>\n<axon_query>second</axon_query>\n</axon_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<axon_user_queries>\n<axon_query>Q1</axon_query>\n</axon_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<axon_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<axon_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<axon_user_queries>\n<axon_query>Q</axon_query>\n</axon_user_queries>";
