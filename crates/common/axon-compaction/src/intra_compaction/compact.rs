...
// The `Shared` summarizer reuses axon-build's full-replace summarization core
...
/// `FullReplace` strategy (default): axon-build's full-replace — summarize the
...
/// `<axon_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching axon-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through axon-build's shared core.
    ...
    //     Shared with Axon chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<axon_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<axon_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<axon_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): axon-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<axon_user_queries>` preamble so
...
/// — axon-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and axon-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // axon-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // axon-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Axon chat's
...
/// the literal error messages produced by the Axon chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Axon chat sampler when the
    ...
    /// — guards the forwarding that Axon chat (`Arc<AxonTurn>`) relies on.
    #[test]
