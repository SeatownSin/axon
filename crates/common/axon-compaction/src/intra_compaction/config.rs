...
///
/// - `FullReplace` (default): axon-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from axon-shell's
...
/// time (token threshold only, matching axon-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as axon-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<axon_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "axon-4.20";
