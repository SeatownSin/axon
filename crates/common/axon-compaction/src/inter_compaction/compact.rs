...
//! extraction, chunk summarisation, and the final `<axon_user_queries>`
//! + `<chunk_summary>` assembly — is shared. The harness supplies the
//! candidate items, the *current* user-queries preamble (Axon chat
...
pub struct ChunkedCompactionOutput {
    /// `<axon_user_queries>` preamble + `<chunk_summary index="i">` blocks.
...
/// 2. [`separate_prior_user_queries`] — split prior `<axon_user_queries>`
...
/// round's user messages (Axon chat: verbatim from the raw request, with
/// attachment refs). `conversation_id` / `response_id` are threaded
/// through for log correlation only.
///
/// Observer events (the Axon chat observer maps them to the
...
pub async fn sample_compaction_chunked<T: CompactionItemBuilder + Send + Sync>(
    ...
    // Step 2 — split prior `<axon_user_queries>` out of every prior
