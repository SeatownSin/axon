...
//!
//! The canonical `sample → classify → retry` loop, used by **both** axon-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Axon chat's intra `Shared` summarizer
...
//! the winning summary (axon-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the axon-build host uses to step down its input size).
