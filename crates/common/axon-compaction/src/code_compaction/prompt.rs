//! axon-build's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so axon-build's full-replace
//! prompt lives alongside the rest of its [`code_compaction`](crate::code_compaction)
//! subsystem. The Axon chat's step-level intra prompt
//! ([`format_compaction_prompt`](crate::prompt::format_compaction_prompt))
//! stays at the crate root.

/// Build axon-build's session-level summarization prompt (no chat history).
///
...
/// `axon-shell::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `axon-shell`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (axon-build) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// axon-build's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes axon-build's prompt.
        assert_eq!(
