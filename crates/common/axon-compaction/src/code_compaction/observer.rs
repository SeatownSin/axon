//! Observability seam for the full-replace (axon-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (axon-build:
...
//!
//! Emission points are part of the behavior contract: the axon-build observer
