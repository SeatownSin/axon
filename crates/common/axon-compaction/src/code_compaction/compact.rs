//! axon-build's full-replace compaction pass.
//!
//! axon-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `axon-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). axon-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (axon-build's
...
/// Run axon-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam axon-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // axon-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce axon-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_axon_build_history_shape() {
        let llm_turns = vec![
