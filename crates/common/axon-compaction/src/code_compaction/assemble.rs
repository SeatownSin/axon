//! Compacted-history assembly (axon-build's rebuild structure, generic).
//!
//! Moved from `axon-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! axon-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // axon-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn axon_build_order_recent_before_summary() {
