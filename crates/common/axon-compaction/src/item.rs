...
//! harness implements [`CompactionItem`] for its `AxonTurn`;
//! axon-build implements it for `axon_sampling_types::ConversationItem`.
//!
...
//! `<axon_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `AxonRole` (Axon chat) and the
/// `ConversationItem` variants (axon-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Axon chat) — maps to System on harnesses without a
...
/// extraction (`<axon_file id=".." name=".." />` lines in the
/// `<axon_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Axon chat's
    /// `AxonTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Axon chat: a
    ...
    /// strips their `<axon_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<axon_file>` lines in the
    /// `<axon_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Axon chat: a `Developer` turn with category
    ...
    /// requests/results (Axon chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up axon-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Axon chat's
/// `AxonTurn`; adding these constructors to it as required methods would break
/// that impl. They are also axon-build-specific (Axon chat's tail-keep path
...
///
/// The axon-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Axon chat stores turns as `Arc<AxonTurn>`).
