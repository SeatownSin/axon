...
//! stay in each product host (for example `axon-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `axon-sampling-types`. It is decoupled from both Axon chat and
//! axon-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — axon-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Axon chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Axon chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Axon chat and
//! axon-build; hosts still own snapshotting and host-only sections).
...
// axon-build's full-replace subsystem now lives under `code_compaction`;
