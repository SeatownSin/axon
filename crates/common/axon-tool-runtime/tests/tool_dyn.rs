...
//! `AxonToolDyn` / `AxonToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use axon_tool_protocol::{ToolCapabilities, ToolId};
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&axon_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
