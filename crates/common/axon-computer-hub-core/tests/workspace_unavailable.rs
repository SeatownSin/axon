...
use axon_computer_hub_core::{error_from_envelope, is_workspace_unavailable, tool_error_from_wire};
...
use axon_tool_runtime::{ToolError, ToolErrorKind};
use serde_json::json;
...
fn non_custom_decoded_error_is_not_recognized() {
    let wire = ToolErrorWire::ToolNotFound {
        tool_id: axon_tool_protocol::ToolId::new("ns:tool").unwrap(),
    };
    let err = error_from_envelope(envelope_for(&wire));
    assert_ne!(err.kind, ToolErrorKind::Custom);
    assert!(!is_workspace_unavailable(&err));
