...
impl InMemRegistry {
    fn install(&self, session: SessionId, tool_id: ToolId, handle: Arc<dyn ToolHandle>) {
        let registration = ToolRegistration {
            ...
            server_id: None,
            description: handle.description(&axon_tool_runtime::ListToolsContext::default()),
            input_schema: None,
...
impl ToolRegistry for InMemRegistry {
    ...
    fn list_servers_for_user(
        &self,
        _user_id: &axon_tool_protocol::UserId,
    ) -> Vec<axon_computer_hub_core::registry::ServerRecord> {
        Vec::new()
    }
    ...
    fn get_server_record(
        &self,
        _connection_id: &ConnectionId,
    ) -> Option<axon_computer_hub_core::registry::ServerRecord> {
        None
    }
}
...
async fn collect(
    stream: &mut ToolStream<axon_tool_runtime::TypedToolOutput>,
) -> Vec<ToolStreamItem<axon_tool_runtime::TypedToolOutput>> {
...
async fn missing_tool_resolves_as_terminal_not_found() {
    ...
    match &items[0] {
        ToolStreamItem::Terminal(Err(e))
            if e.kind == axon_tool_runtime::ToolErrorKind::NotFound =>
        {
            assert!(
...
async fn invalid_arguments_surface_as_terminal_error() {
    ...
    match &items[0] {
        ToolStreamItem::Terminal(Err(e))
            if e.kind == axon_tool_runtime::ToolErrorKind::InvalidArguments => {}
...
async fn authorize_returns_bound_principal_with_invoke_scope() {
    ...
    assert!(principal.has_scope(axon_computer_hub_core::LOCAL_INVOKE_SCOPE));
