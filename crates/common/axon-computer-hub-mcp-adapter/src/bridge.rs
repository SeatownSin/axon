...
/// [`axon_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`axon_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl axon_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            axon_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == axon_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
