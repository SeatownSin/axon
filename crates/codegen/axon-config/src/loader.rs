...
/// `.axon/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.axon` to the user tier).
...
impl ConfigLayers {
    ...
    /// wraps this with the `AXON_CAMPAIGNS_OVERRIDE` env layer.
...
/// `AXON_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("AXON_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`axon_shell::util::config::load_effective_config`) when remote campaigns
/// or `AXON_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// Path to `$AXON_HOME/campaigns_state.json` under `home`.
...
/// can't be parsed (broken `AXON_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match axon_version::installed_semver() {
...
mod tests {
    ...
    /// `AXON_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `AXON_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("AXON_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads AXON_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("AXON_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("AXON_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("AXON_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("AXON_CAMPAIGNS") },
        }
    }
}
