//! Memory-system configuration value types, extracted from axon-shell
...
//! `axon-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.axon/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.axon/memory/` are scanned:
