//! MCP server configuration value types, extracted from axon-shell
//! (config dependency inversion).

use agent_client_protocol as acp;
use axon_mcp::oauth_config::McpOAuthConfig;
...
impl RelaySyncConfig {
    ...
    pub fn is_enabled(&self) -> bool {
        if let Ok(env_val) = std::env::var("AXON_RELAY_SYNC_ENABLED") {
