...
//!
//! In-process SDK MCP servers (the official `axon-agent-sdk`'s `@tool` /
...
//! reverse `axon/mcp/sdk_call` request and feeding the response back. This module
...
/// channel (`axon/mcp/sdk_call`) and returns its JSON-RPC response. The `Err` string is
...
/// `axon/mcp/sdk_call` and writes the server→client response back. The pump exits when
...
async fn read_requests(
    ...
    loop {
        ...
        // `axon/mcp/sdk_call`s without a JSON-RPC id, so id-less messages (e.g. rmcp's
