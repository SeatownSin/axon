...
use axon_tools::util::ProcessGroup;

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `axon_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use axon_workspace_types::MCP_TOOL_NAME_DELIMITER;
...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"axon_com_linear"`).
...
/// SDK-side id echoed back in `axon/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["axon/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["axon/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `axon/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    event_writer: axon_file_utils::events::EventWriter,
    ...
    /// `axon/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: axon_file_utils::events::EventWriter::noop(),
    ...
    /// `AxonClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: axon_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &axon_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `axon-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use axon_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`axon_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(axon_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((axon_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> axon_file_utils::events::McpErrorCategory {
        use axon_file_utils::events::McpErrorCategory;
        match self {
...
///   the frontend via `axon/mcp/tools_changed` notifications and callable via
///   `axon/mcp/call`.
...
/// `axon_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl axon_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> axon_tool_protocol::ToolId {
        ...
        axon_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| axon_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        axon_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
    ...
    ) -> Result<ToolOutput, axon_tool_runtime::ToolError> {
        ...
        event_writer.emit(axon_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(axon_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            axon_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(axon_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(axon_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        axon_telemetry::session_ctx::log_event(axon_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &axon_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, axon_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| axon_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(axon_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(axon_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(axon_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(axon_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(axon_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(axon_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(axon_file_utils::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`axon/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`AxonClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, AxonClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`AxonClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `axon/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`AxonClientHandler`]** constructed by
...
/// and the [`AxonClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to axon-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `axon/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: axon_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, AxonClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `axon/mcp/sdk_call`: the larger of the
    ...
    /// Build the [`AxonClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> AxonClientHandler {
        AxonClientHandler {
    ...
    /// references (the [`AxonClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _list_tools_timer = axon_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = axon_config::axon_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`axon_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "axon_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("axon-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = axon_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = axon_telemetry::instrumentation::timer("mcp_stdio_spawn");
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    axon_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            axon_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = axon_telemetry::instrumentation::timer("mcp_start_servers");
...
impl ClientHandler for AxonClientHandler {
    ...
    // See the [`AxonClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            axon_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(!is_figma_mcp("axon_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_axon_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use axon_computer_hub_sdk::LocalRegistry;
        use axon_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = axon_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = axon_file_utils::events::EventWriter::open(tmp.path());
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'axon_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            axon_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
