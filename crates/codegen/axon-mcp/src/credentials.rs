...
///
/// Local helper (not shell-base): `axon-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$AXON_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$AXON_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$AXON_HOME/mcp_credentials.json`).
    ///
...
mod tests {
    ...
    /// `$AXON_HOME/mcp_credentials.json`. Existing credential files must keep
