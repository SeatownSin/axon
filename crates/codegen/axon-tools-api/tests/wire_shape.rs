...
//! field rename/retype in `axon-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use axon_tools_api::ToolConfigEntry;
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "AxonBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "AxonBuild:read_file");
