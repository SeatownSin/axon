...
mod links {
    ...
    fn emits_osc8_around_linked_cells() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://blocked.invalid", None)]);
        assert!(
            out.contains("\x1b]8;;https://blocked.invalid\x07"),
            "missing open: {out:?}"
        );
    ...
    fn link_removed_next_frame_rewrites_cells_without_osc8() {
        let mut t = term(20, 3);
        let _ = frame(&mut t, "AB", &[span(0, 2, "https://blocked.invalid", None)]);
    ...
    fn wide_char_under_link_wraps_lead_cell_only() {
        ...
        // the OSC 8 wraps it.
        let out = frame(&mut t, "世", &[span(0, 2, "https://blocked.invalid", None)]);
        assert!(
            out.contains("\x1b]8;;https://blocked.invalid\x07世\x1b]8;;\x07"),
            "wide-char run: {out:?}"
        );
    }
    ...
    fn nonzero_origin_viewport_maps_links() {
        ...
        assert!(
            out.contains("\x1b]8;;https://blocked.invalid\x07AB\x1b]8;;\x07"),
