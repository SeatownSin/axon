//! Unified PTY harness for axon-pager.
//!
...
//!    `axon-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `AXON_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use axon_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
