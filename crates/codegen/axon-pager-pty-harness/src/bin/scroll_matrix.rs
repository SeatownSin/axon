//! `scroll-matrix` — scroll validation matrix sweep for axon-pager.
//!
...
use axon_pager_pty_harness::pager_binary;
use axon_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against axon-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_axon-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves AXON_SCROLL_LOG against ITS cwd (the
