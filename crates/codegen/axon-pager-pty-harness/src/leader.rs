...
//! elected leader) plus a fixed leader socket beneath its `AXON_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `AXON_HOME`.
    ...
    /// The cluster's sessions root: `AXON_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".axon").join("sessions")
    }
