...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build axon-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build axon-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "axon-pager build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_axon-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p axon-pager-bin` (the composition-
///    root package that owns the `axon-pager` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_axon-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
