...
//! [`MockInferenceServer`] from `axon-test-support` and provides the
...
pub use axon_test_support::mock_server::LogEntry;
pub use axon_test_support::mock_server::MockModelEntry as MockModel;
pub use axon_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use axon_test_support::sse;
pub use axon_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.axon
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ...
            ("AXON_PROMPT_SUGGESTIONS".into(), "false".into()),
            // Compatibility set_turns remains request-FIFO, so retries stay off.
            ("AXON_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(get("AXON_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("AXON_MAX_RETRIES").as_deref(), Some("0"));
