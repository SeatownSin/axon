//! `paste-latency` — clipboard paste latency benchmark for `axon-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p axon-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p axon-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/axon-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for axon-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
