...
//! `axon-workspace`. Wire types live in
//! `axon_workspace_types::rpc`; this crate adds the connected-state
...
use axon_workspace_types::rpc::fs::{
...
use axon_workspace_types::rpc::git::{
...
use axon_workspace_types::rpc::hunks::{
...
use axon_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use axon_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use axon_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
...
use axon_workspace_types::rpc::worktree::{
...
pub async fn consume_stream_terminal(
    stream: &mut axon_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, axon_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](axon_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &axon_tool_runtime::ToolError) -> bool {
    match err.kind {
        axon_tool_runtime::ToolErrorKind::NetworkError => true,
        axon_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = axon_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    impl axon_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::axon_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.axon/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = axon_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: axon_tool_runtime::ToolStream<TypedToolOutput> =
            axon_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = axon_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
