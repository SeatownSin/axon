//! Headless markdown analysis sharing Axon Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `axon-markdown` so analysis matches what Axon
//! Build actually renders 1:1.
//!
//! After parsing, Axon applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Axon Build uses to render markdown.
///
...
/// Offset event stream from Axon's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Axon Build's options; count elements and flag structural issues.
