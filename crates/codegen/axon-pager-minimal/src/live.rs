...
use axon_pager::render::Renderable;
use axon_pager::scrollback::state::ScrollbackState;
use axon_pager::scrollback::wrappers::EntryRenderer;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &axon_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut axon_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        axon_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    axon_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = axon_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                axon_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            axon_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(axon_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a axon_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
...
fn minimal_advance_phase_timer(
    agent: &mut axon_pager::app::agent_view::AgentView,
) -> Option<axon_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &axon_pager::app::agent_view::AgentView,
    activity: &Option<axon_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = axon_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = axon_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use axon_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = axon_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<axon_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> axon_pager::app::agent_view::AgentView {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use axon_pager::app::agent::AgentState;
        use axon_pager::scrollback::RenderBlock;
        use axon_pager::scrollback::entry::ScrollbackEntry;
        use axon_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &axon_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use axon_pager::acp::tracker::TurnActivity;
        use axon_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        axon_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        axon_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            axon_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(axon_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &axon_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
