...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("AXON_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "AXON_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    axon_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`axon_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    axon_tty_utils::detach_std_command(&mut command);
    command
}
