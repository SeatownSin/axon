...
//! The default theme is AxonNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! AxonNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::AxonNight, ThemeKind::AxonDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::AxonNight => "axonnight",
            Self::TokyoNight => "tokyonight",
            Self::AxonDay => "axonday",
    ...
    /// when quantized to 256 or 16 colors. AxonNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::AxonNight => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "axonnight" | "axon-night" | "dark" => Some(Self::AxonNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "axonday" | "axon-day" | "light" | "day" => Some(Self::AxonDay),
...
/// `"axonnight"` → `"Axon Night"`). Falls back to `value` verbatim.
///
/// The canonical machine names (`"axonnight"`/`"axonday"`) are retained for
/// config-serialization compatibility; only the human-facing labels are
/// rebranded to Axon.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "axonnight" => "Axon Night",
        "axonday" => "Axon Day",
...
impl Theme {
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::AxonNight => Self::axonnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::AxonDay => Self::axonday(),
            ...
            // if reached, fall back to AxonNight.
            ThemeKind::Auto => Self::axonnight(),
        };
    ...
    ///    on axonnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., AxonNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // AxonNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    axon_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::AxonNight.is_auto());
        assert!(!ThemeKind::AxonDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::axonday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::axonnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::axonday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. AxonNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::AxonNight => Theme::axonnight(),
                ThemeKind::AxonDay => Theme::axonday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_axonnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark AxonNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::axonnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("axonnight"),
            Some(ThemeKind::AxonNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::AxonNight));
        assert_eq!(ThemeKind::from_name("axonday"), Some(ThemeKind::AxonDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::AxonDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("axonnight", ThemeKind::AxonNight),
            ("axon-night", ThemeKind::AxonNight),
            ("dark", ThemeKind::AxonNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("axonday", ThemeKind::AxonDay),
            ("axon-day", ThemeKind::AxonDay),
            ("light", ThemeKind::AxonDay),
            ("day", ThemeKind::AxonDay),
