...
/// in `config.toml`. When `None`, defaults to `AxonNight` / `AxonDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::AxonDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::AxonNight),
    }
}
