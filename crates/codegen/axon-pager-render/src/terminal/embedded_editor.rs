//! Detects whether axon is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` axon is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `axon-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
