...
impl TerminalContext {
    ...
    pub fn telemetry_snapshot(&self) -> axon_telemetry::events::TerminalTelemetry {
        ...
        axon_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: axon_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> axon_shared::session::FeedbackTerminalInfo {
        use axon_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(axon_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `axon ssh` can
    // forward env vars.
...
/// `axon-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.axon/pager.toml` and
