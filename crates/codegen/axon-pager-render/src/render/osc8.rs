...
//! frame diff (see `axon_ratatui_inline::Terminal::flush_with_links`).
...
/// Relative file path (`images/1.png`, `.axon/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.axon/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn scan_detects_axon_session_media_path() {
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.axon/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.axon/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.axon/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.axon/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/axon-pager/src/render` should
        // NOT produce a link for the `/axon-pager/src/render` substring.
        let line = make_line("find crates/codegen/axon-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
