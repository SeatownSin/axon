...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,axon_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        #[cfg(feature = "audio")]
        {
            let (bytes, chunks) = axon_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    if let Ok(home) = std::env::var("AXON_HOME")
