...
mod imp {
    ...
    pub fn install(crash_dir: &Path, axon_version: &str) -> bool {
        ...
        unsafe {
            ...
            version.fill(0);
            let copy_len = axon_version.len().min(format::VERSION_STRING_LEN);
            version[..copy_len].copy_from_slice(&axon_version.as_bytes()[..copy_len]);
        }
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("axon-crash-handler-test-replace");
    ...
    fn install_tightens_preexisting_0644_crash_blob() {
        ...
        let _guard = SIGNAL_STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!(
            "axon-crash-handler-test-tighten-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create crash dir");
