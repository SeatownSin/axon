...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `axon-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `axon-shell::session::events`.
    ...
    /// discriminator constants in `axon-shell::session::events`.
    ...
    /// `axon-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `axon-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `axon/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `axon/interject`, or "Send now" on a
...
/// Telemetry mirror of `axon-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `axon-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `axon-shell`'s `GoalPauseReason`. The two types
