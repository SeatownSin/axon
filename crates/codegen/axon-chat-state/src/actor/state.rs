...
pub fn estimate_system_message_tokens(item: &ConversationItem) -> u64 {
    match item {
        ConversationItem::System(s) => axon_token_estimation::estimate_tokens(&s.content),
        _ => 0,
    }
}
...
pub fn estimate_tool_definition_tokens(td: &axon_sampling_types::ToolDefinition) -> u64 {
    ...
    ((name_len + desc_len + params_len) as u64) / axon_token_estimation::BYTES_PER_TOKEN
}

/// Sum [`estimate_tool_definition_tokens`] across a slice.
pub fn estimate_tool_definitions_tokens(tds: &[axon_sampling_types::ToolDefinition]) -> u64 {
    tds.iter().map(estimate_tool_definition_tokens).sum()
}

/// Bytes/4 estimate for a single [`ConversationItem`].
///
/// Images are counted at [`axon_token_estimation::IMAGE_TOKEN_ESTIMATE`] each.
...
pub fn estimate_item_tokens(item: &ConversationItem) -> u64 {
    ...
    match item {
        ...
        ConversationItem::User(u) => {
            ...
            (bytes as u64) / axon_token_estimation::BYTES_PER_TOKEN
                + axon_token_estimation::estimate_image_tokens(images)
        }
        ...
        ConversationItem::ToolResult(tr) => axon_token_estimation::estimate_tokens(&tr.content),
        ConversationItem::BackendToolCall(b) => {
            axon_token_estimation::estimate_tokens(&b.text_summary())
        }
        ConversationItem::Reasoning(r) => {
            ...
            let text_bytes = axon_sampling_types::reasoning_item_text(r).len();
            let enc_bytes = r.encrypted_content.as_deref().map(str::len).unwrap_or(0);
            ((text_bytes + enc_bytes) as u64) / axon_token_estimation::BYTES_PER_TOKEN
        }
    }
}

/// Estimate token footprint: text bytes / 4, images at the per-image
/// constant defined by [`axon_token_estimation::IMAGE_TOKEN_ESTIMATE`].
...
impl axon_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use axon_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = axon_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
