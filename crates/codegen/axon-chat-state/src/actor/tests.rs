...
use axon_sampling_types::{ConversationItem, SamplingConfig};
use tokio::sync::mpsc;
...
async fn record_last_turn_usage_round_trip() {
    use axon_sampling_types::TokenUsage;
...
async fn build_request_with_tool_definitions() {
    use axon_sampling_types::ToolSpec;
...
/// Simulates the exact sequence that `axon-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools = ConversationItem::Assistant(axon_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use axon_sampling_types::{ContentPart, UserItem};
...
fn serialize_via_public_api(req: &axon_sampling_types::ConversationRequest) -> serde_json::Value {
    ...
    axon_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &axon_sampling_types::ConversationRequest,
    extended: &axon_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(axon_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(axon_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use axon_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
