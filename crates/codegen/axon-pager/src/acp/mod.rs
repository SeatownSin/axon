...
use axon_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    pub auth_manager: std::sync::Arc<axon_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<axon_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    axon_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
mod tests {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.axon/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use axon_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
