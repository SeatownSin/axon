...
///
/// Defined in `axon-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use axon_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "axon_shell=info,axon_pager=trace,axon_tools=info,axon_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    // `build_otel_layer` returns `Identity` -- the first-party span exporter is
    // gone, so this layer is inert and both arguments are discarded.
    //
    // DO NOT delete the `build_default_otel_layer_config()` call as dead code.
    // Its *return value* is dead, but it is called for its **side effect**: it
    // populates the `OTEL_PROVIDER` global, which is live and consumed by
    // `wire_otel_auth_manager`, `wire_otel_deployment_key` and
    ...
    let instrumentation_layer = axon_telemetry::instrumentation::layer();
    let sampling_log_layer = axon_telemetry::sampling_log::layer();
    let hooks_log_layer = axon_telemetry::hooks_log::layer();
