//! `axon plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `axon_shell::plugin` (shared orchestration)
//! and lower crates (`axon-agent`, `axon-plugin-marketplace`). This
...
use axon_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use axon_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use axon_plugin_marketplace::SourceKind;
use axon_plugin_marketplace::git::SourceCacheLease;
use axon_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `axon plugin list --json`. The `status` field acts as a
...
/// Typed entry for `axon plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `axon plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  axon plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `axon plugin install --help` to get started.");
    } else {
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = axon_shell::config::load_effective_config()
        .ok()
    ...
    for source in &sources {
        ...
        for plugin in axon_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                axon_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &axon_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    source: &axon_plugin_marketplace::MarketplaceSource,
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = axon_plugin_marketplace::git::default_cache_root();
            axon_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
                axon_plugin_marketplace::git::SyncMode::UseTtl,
            )
...
fn install_kind(is_git: bool) -> axon_telemetry::events::InstallKind {
    if is_git {
        axon_telemetry::events::InstallKind::Git
    } else {
        axon_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: axon_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    axon_telemetry::session_ctx::log_event(axon_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    ...
    if !trust {
        use axon_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &axon_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `axon plugin update {}` to update it.",
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               axon plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `axon plugin list` to see installed plugins."
        );
    }
    if let Err(e) = axon_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    axon_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = axon_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    axon_shell::config::add_disabled_plugin(name)
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `axon plugin tag`."
            )
        })?,
...
fn marketplace_list(
    sources: &[axon_plugin_marketplace::MarketplaceSource],
    json: bool,
) -> Result<()> {
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `axon plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use axon_shell::plugin::MarketplaceAddInput;
    ...
    let allowlist =
        &axon_workspace::permission::resolution::managed_settings().marketplace_allowlist;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        axon_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &axon_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match axon_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use axon_plugin_marketplace::MarketplaceSource;
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  axon plugin install u/r --trust"), "{git}");
    ...
    fn current_head(repo: &Path) -> String {
        let output = axon_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
