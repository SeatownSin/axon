//! `axon mcp` — manage MCP server configurations from the command line.
...
Examples:
  ...
  axon mcp add xcode -- xcrun mcpbridge

  # Add a stdio server with environment variables
  axon mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

  # Add a remote HTTP server
  axon mcp add --transport http sentry https://mcp.sentry.dev/mcp

  # Add a remote server with an authentication header
  axon mcp add --transport http api https://mcp.example.com/mcp --header \"Authorization: Bearer YOUR_TOKEN\"

  # Add to the project config (./.axon/config.toml) instead of ~/.axon/config.toml
  axon mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.axon/config.toml`, available in all your projects
    User,
    /// `./.axon/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.axon/config.toml) or project (./.axon/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = axon_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `axon mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    axon_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: axon mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: axon mcp add --transport {label} <name> <url>"
                );
            };
...
fn select_remove_site(
    ...
    use axon_shell::util::config::user_config_path;
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        axon_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .axon/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = axon_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        axon_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = axon_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.axon/config.toml");
