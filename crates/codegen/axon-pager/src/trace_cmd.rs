...
async fn run_upload(
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            axon_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = axon_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &axon_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
    ...
    (|| async {
        tokio::time::timeout(
            UPLOAD_TIMEOUT,
            axon_file_utils::gcs::upload_bytes(config, object_path, archive, "application/gzip"),
        )
        .await
