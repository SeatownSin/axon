...
//! pattern as axon-shell's `compaction.rs`): entry from the queue pane,
...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// `axon/queue/changed` rebroadcast for the visual result) instead
...
impl AgentView {
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`axon/queue/interject` newText)
