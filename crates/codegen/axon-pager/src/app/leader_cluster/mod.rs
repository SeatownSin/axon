...
struct PagerLeaderCluster {
    ...
    /// agent on the same AXON_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(AXON_HOME)]`
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (axon_version), not
        ...
        // axon-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: axon-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same AXON_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `AXON_LEADER_SOCKET`, flock held
