#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: a machine-wide `axon/models/update` broadcast
    ...
    fn models_update_preserves_active_agent_model() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_3 = acp::ModelId::new(std::sync::Arc::from("axon-3"));
        agent
            .session
            .models
            .available
            .insert(id_3.clone(), make_model_info("axon-3"));
        agent.session.models.current = Some(id_3);

        let notif = make_models_update_notif("axon-4", &["axon-3", "axon-4"]);
    ...
    fn models_update_uses_shell_default_when_agent_model_removed() {
        ...
        // axon-3 removed from catalog.
        let notif = make_models_update_notif("axon-4.3", &["axon-4.3", "axon-4.5"]);
        handle_models_update(&notif, &mut app);
    ...
    fn models_update_noop_when_agent_matches_shell_default() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_4 = acp::ModelId::new(std::sync::Arc::from("axon-4"));
        agent
            .session
            .models
            .available
            .insert(id_4.clone(), make_model_info("axon-4"));
        ...
        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("axon-4"),
            "app.models.current must be axon-4 when agent and shell agree"
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
            agent
                ...
                .map(|id| id.0.as_ref()),
            Some("axon-4"),
            "agent model must remain axon-4"
        );
    }
    ...
    fn models_update_non_active_agent_uses_shell_fallback_not_active_model() {
...

        {
            ...
            let id_5 = acp::ModelId::new(std::sync::Arc::from("axon-4.5"));
            agent_b
                .session
                .models
                .available
                .insert(id_5.clone(), make_model_info("axon-4.5"));
            agent_b.session.models.current = Some(id_5);
        }

        // axon-5 removed from catalog.
        let notif = make_models_update_notif("axon-4", &["axon-3", "axon-4"]);
        handle_models_update(&notif, &mut app);
        ...
        // B's axon-5 was removed — must fall back to shell's axon-4, not A's axon-3.
    ...
    fn model_changed_updates_state_silently_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "axon-4", None);
    ...
    fn model_changed_dropped_when_model_unknown_to_catalog() {
        ...
        let notif = model_changed_ext("sess-1", "axon-99-unknown", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(
            !changed,
    ...
    fn model_changed_applies_reasoning_effort_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "axon-4", Some("high"));
    ...
    fn model_changed_dropped_for_unknown_session_id() {
        ...
        let notif = model_changed_ext("sess-OTHER", "axon-4", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(!changed);
