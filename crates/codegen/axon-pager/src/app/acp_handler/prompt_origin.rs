...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    axon_shell::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `axon/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        axon_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        axon_shell::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        axon_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        axon_shell::session::PromptOrigin::TaskCompleted { .. }
            | axon_shell::session::PromptOrigin::SubagentCompleted { .. }
            | axon_shell::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `axon/session/prompt_complete`, the
