...
/// Inclusive bounds for motion cadence env knobs (`AXON_MIN_DRAW_MS`,
/// `AXON_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("AXON_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("AXON_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        axon_telemetry::session_ctx::log_event(t.clone());
        ...
        axon_telemetry::session_ctx::log_event(axon_telemetry::events::DisplayRefreshProbe {
            terminal: t,
