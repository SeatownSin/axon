...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: axon_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`axon --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: axon_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(axon_hooks_plugins_types::HooksAction),
    ...
    AnnouncementsOpenCta(axon_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"axonnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `axon dashboard`).
    OpenDashboard,
...
pub enum Effect {
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.axon/config.toml`.
    ...
    /// Persist a typed setting to `~/.axon/config.toml`. On failure,
    ...
    /// registry (`~/.axon/active_sessions.json`).
...
pub enum TaskResult {
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<axon_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<axon_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<axon_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<axon_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<axon_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<axon_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<axon_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<axon_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<axon_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    GateRefreshed {
        settings: Option<axon_shell::util::config::RemoteSettings>,
    },
