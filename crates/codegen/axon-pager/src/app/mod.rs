...
pub use axon_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `AXON_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    axon_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use axon_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("AXON_VOICE_MODE")
...
///
/// Uses [`axon_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `axon-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`axon-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&axon_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<axon_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<axon_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
pub async fn run(
    ...
    axon_tty_utils::redirect_native_stderr();
    ...
    axon_shell::util::config::cache_remote_auto_mode(
    ...
    let raw_config = match first_run::maybe_run(&raw_config).await {
        first_run::Outcome::Configured => axon_shell::config::load_effective_config()
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(axon_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = axon_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("AXON_HUNK_TRACKER").ok();
    ...
    cancel.cancel();
    axon_tty_utils::global_process_scope().kill_all();
...
fn disable_mouse_paste_raw() {
    axon_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(axon_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    axon_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        axon_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(axon_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                axon_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = axon_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = axon_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        axon_crash_handler::disable_terminal_escape_restore();
    })
}
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = axon_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = axon_shell::util::config::RemoteSettings {
