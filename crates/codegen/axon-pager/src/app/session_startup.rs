...
pub fn fork_session_params(
    ...
    let source_cwd = axon_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    axon_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = axon_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if axon_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = axon_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    let agent_config = axon_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
    ...
    let storage_client = axon_shell::auth::credential_provider::build_storage_client_for_proxy(
