...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        axon_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(axon_telemetry::events::CreditLimitUpsellShown {
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            axon_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<axon_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<axon_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
