...
//! `axon/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `axon/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `axon/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`axon/session/interjection`) is
