...
/// Start (if needed) and submit the initial prompt from `axon "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::SmallScreen,
            action: axon_telemetry::events::ContextualTipAction::Shown,
        });
    }
}

/// Show the one-shot "Over SSH? Run `axon wrap ssh <host>` locally…" hint at
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::SshWrap,
            action: axon_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::PlanMode,
            action: axon_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::WordSelect,
            action: axon_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(axon_telemetry::events::ContextualTip {
        tip: axon_telemetry::events::ContextualTipKind::WordSelect,
        action: axon_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(axon_telemetry::events::ContextualTip {
            tip: axon_telemetry::events::ContextualTipKind::SendNow,
            action: axon_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use axon_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use axon_telemetry::session_ctx::log_event;
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        let free_usage_blocked = agent.session.free_usage_blocked
            || result
                .as_ref()
                .err()
                .is_some_and(|e| axon_shell::sampling::error::is_free_usage_exhausted_error(e));
