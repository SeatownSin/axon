...
pub(super) fn cta_install_relative_path(
    candidates: &[axon_hooks_plugins_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn cta_install_error_category(
    ...
    use axon_hooks_plugins_types::OutcomeStatus;
    match result {
...
pub(super) fn plugin_cta_phase_for(
    ...
    use axon_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(axon_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == axon_hooks_plugins_types::OutcomeStatus::Success => {
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == axon_hooks_plugins_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::effects::sanitize_user_error(&outcome.message)),
        Err(e) => Err(e),
    };
    match reload_result {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(axon_telemetry::events::ExtensionsModalOpened {
                    trigger: axon_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = axon_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(axon_telemetry::events::PluginCtaImpression {
