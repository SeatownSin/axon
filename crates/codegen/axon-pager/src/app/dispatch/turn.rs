...
fn effective_cancel_subagents_preference(
    ...
    ui: &axon_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `axon/queue/changed`
...
/// Grace window between a driver-side `axon/session/prompt_complete`
...
/// Finish turns whose end was announced by `axon/session/prompt_complete`
...
// TODO: Add dispatch_cancel_command() once axon-shell supports proper
