...
fn critical_announcement(id: &str) -> axon_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> axon_announcements::RemoteAnnouncement {
    axon_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
...
/// the promo url (observed via the `AXON_TEST_OPEN_URL_FILE` seam, from every
...
#[serial_test::serial(AXON_TEST_OPEN_URL_FILE)]
#[test]
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    unsafe { std::env::remove_var("AXON_TEST_OPEN_URL_FILE") };
    let _ = std::fs::remove_file(&url_file);
}
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.axon/skills/pick-best/SKILL.md", }
    );
