...
///    `axon/yolo_mode_changed` ACP notification back to the shell.
...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.axon/config.toml AND fires blocked.invalid/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
fn enable_always_approve_is_idempotent_when_yolo_already_on() {
    ...
    assert!(
        ...
        "redundant PersistPermissionMode when YOLO already on — the dispatcher \
             ...
             blocked.invalid/yolo_mode_changed",
    );
}
...
/// guard MUST precede the `axon_telemetry::log_event` call
