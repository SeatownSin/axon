...
/// A free-tier user hitting the voice keybinding gets the SuperAxon upsell
...
fn voice_keybinding_on_restricted_tier_opens_upsell() {
    ...
    assert!(
        ...
        "restricted-tier voice keybinding must open the SuperAxon upsell"
    );
    assert!(
        !app.voice_listening(),
...
fn voice_interim_sets_then_error_clears_state() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        axon_voice::VoiceEvent::InterimTranscript {
            text: "partial".into(),
        },
    );
...
fn voice_stt_language_change_recycles_pipeline() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(axon_voice::VoiceCommand::Shutdown)
    ));
    assert!(matches!(
        effects.as_slice(),
