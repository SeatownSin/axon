...
use super::*;
use axon_shell::sampling::error::is_free_usage_exhausted_error;
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use axon_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// `AXON_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    unsafe { std::env::set_var("AXON_TEST_OPEN_URL_FILE", &bad) };
