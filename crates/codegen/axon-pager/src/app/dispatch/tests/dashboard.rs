...
/// `axon dashboard` before login: the startup hook consumes the
/// `AXON_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `axon dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(axon_shell::sampling::types::ReasoningEffort::High)
        )),
...
/// `axon dashboard`, and the startup hook.
#[serial_test::serial(AXON_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("AXON_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
