...
/// immediately, before the confirming `axon/queue/changed` broadcast.
...
/// that bridges the round-trip before the confirming `axon/queue/changed`
...
///
/// Delegates to the shared implementation in `axon_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    axon_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
pub(crate) fn apply_turn_start_shim(
    ...
    // seen ring, so a buffer-replayed `axon/follow_ups` for an older response
...
mod tests {
    ...
    fn drain_scroll_honors_page_flip_setting() {
        ...
        crate::appearance::cache::set_page_flip_on_send(
            axon_shell::agent::config::UiConfig::PAGE_FLIP_ON_SEND_DEFAULT,
        );
    }
    ...
    /// `axon/follow_ups` for a PRIOR turn's response stays rejected (its
