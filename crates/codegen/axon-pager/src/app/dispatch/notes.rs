...
pub(super) fn dispatch_send_feedback(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    agent.scrollback.push_block(RenderBlock::system(
        "Thanks for the feedback! The Axon Build team is on it.".to_string(),
    ));
...
/// Send a raw remember note for LLM-powered rewriting via `axon/memory/rewrite`.
...
/// agent is mid-turn. Fires the `axon/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `axon/recap`.
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_axon_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
