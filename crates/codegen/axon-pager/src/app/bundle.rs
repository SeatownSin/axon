...
//!
//! Pager-side cache of what `axon-shell` reports from
//! `axon/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `axon/bundle/status` ACP responses.
...
/// Deserialized response from `axon/bundle/status`.
...
/// Deserialized response from `axon/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
