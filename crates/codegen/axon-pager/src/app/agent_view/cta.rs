...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        axon_telemetry::session_ctx::log_event(axon_telemetry::events::PromptSuggestion {
            action: axon_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `axon/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `axon/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `axon/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `axon/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `axon/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `axon/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        axon_telemetry::session_ctx::log_event(
            axon_telemetry::events::PluginCtaConnectClicked {
        ...
        self.pending_effects
            .push(super::actions::Effect::InstallPluginFromCta {
                agent_id: self.session.id,
                session_id,
                source_url_or_path: axon_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
                    .to_string(),
                plugin_relative_path,
            });
    }
