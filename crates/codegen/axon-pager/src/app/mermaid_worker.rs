...
//! inside [`axon_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `axon-pager __mermaid-render` (see
...
//!    via [`axon_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`axon_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `axon-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, axon_mermaid::MermaidError> {
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `AXON_HOME` mutation).
        #[cfg(test)]
