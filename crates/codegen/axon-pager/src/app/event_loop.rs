...
/// [`decide`](axon_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<axon_update::auto_update::UpdateAvailable>>,
    >,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("AXON_LOG_SAMPLING", "1") };
    }
    ...
    let launch_auto = axon_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = axon_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(axon_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = axon_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match axon_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.session_picker_grouped = std::env::var("AXON_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            axon_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    axon_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<axon_shell::auth::AuthMeta>(meta.clone()) {
    ...
    app.hidden_announcement_ids = axon_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = axon_shell::config::load_merged_requirements();
    let user_config = axon_shell::config::load_from_disk().ok();
    let managed_config = axon_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match axon_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = axon_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    app.zdr_access_enabled = axon_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        axon_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        axon_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        axon_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use axon_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = axon_announcements::filter_expired(announcements);
    ...
    let hints = axon_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.axon/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        axon_shell::util::config::clamped_display_permission_mode(
            axon_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        axon_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = axon_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = axon_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // AXON_SCROLL_SPEED/_MODE/_LINES + AXON_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<axon_voice::VoiceEvent>>;
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(axon_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.axon/config.toml [ui]`
                // rather than `~/.axon/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> axon_shell::agent::config::UiConfig {
    use axon_shell::agent::config::UiConfig;
    let Ok(root) = axon_shell::config::load_effective_config() else {
