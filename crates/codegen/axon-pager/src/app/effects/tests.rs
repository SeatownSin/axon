...
use super::*;
use axon_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use axon_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use axon_workspace::session::git::RestoreDegree;
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use axon_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        axon_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        axon_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use axon_acp_lib::AcpAgentMessage;
    use axon_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
...
/// Neutralize `AXON_AGENT` for the profile-matrix tests below: agent-driven
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> axon_shell::session::SessionInfoResponse {
    use axon_shell::session::acp_types::{ContextInfo, SessionInfoData};
    axon_shell::session::SessionInfoResponse {
