...
/// Run the post-CTA-install `axon/mcp/list` read (uncached, which also nudges
...
/// rewrite when the body pushes personal SuperAxon), else auth-aware fallback
...
) -> (bool, Option<String>, Option<axon_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["axon/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("axon/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(axon_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(axon_fast_worktree::ENOSPC_OS_MESSAGE)
    {
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `axon-build` (default)         | `false`            |
/// | false | true      | false    | `axon-build` (default)         | `false`            |
/// | false | false     | true     | `axon-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `axon-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `axon-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `axon-build-plan`              | `false`            |
/// | true  | false     | true     | `axon-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `axon-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["axon/session"].kind`
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `axon/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `axon-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("axon-build-plan"),
            (true, false, _) => Some("axon-build-plan-no-subagents"),
            (false, _, true) => Some("axon-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `AXON_AGENT` — the env var chooses the *agent*, but the tool-strip is
    /// independent. Chat mode additionally stamps `axon/session.kind`.
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        let mut meta = serde_json::Map::new();
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("AXON_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("AXON_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("axon/session".into(), serde_json::json!({ "kind" : "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "axon/cloud_server_id",
    "axon/cloud_existing_workspace",
];
/// Stamp `_meta["axon/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("axon/session".into(), serde_json::json!({ "kind" : "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &axon_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = axon_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use axon_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// Degraded conversations lane on `axon/session/list`, parsed from the
/// response's `_meta["axon/partial"]` envelope.
...
/// Read `_meta["axon/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("axon/partial")?;
...
/// Parse the `axon/session/list` response payload (the unwrapped
...
pub(super) fn parse_session_picker_entries(
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            use axon_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = axon_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && axon_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
/// Best-effort `axon/auth/cancel`: stops the shell's device/loopback wait so a
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            axon_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            axon_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "page_flip_on_send" => {
            ...
            axon_shell::util::config::set_page_flip_on_send(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            axon_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            axon_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            axon_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            axon_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            axon_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            axon_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            axon_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            axon_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.ssh_wrap" => {
            ...
            axon_shell::util::config::set_contextual_hint_ssh_wrap(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            axon_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            axon_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            axon_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            axon_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            axon_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            axon_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            axon_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            axon_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            axon_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            axon_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            axon_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            axon_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            axon_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            axon_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            axon_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            axon_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            axon_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            axon_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            axon_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            axon_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            axon_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            axon_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            axon_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            axon_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            axon_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            axon_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            axon_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            axon_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
/// 2. Fire ACP `axon/yolo_mode_changed` (gated on disk success for
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = axon_shell::util::config::update_config(|cfg| {
...
/// Whether to fire the ACP `axon/yolo_mode_changed` notification.
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &axon_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == axon_hooks_plugins_types::OutcomeStatus::Success
}
/// Extract the typed kill outcome from an `axon/task/kill` ext response.
///
...
/// serializes (`axon_shell::extensions::task::KillTaskResponse` +
/// `axon_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<axon_tools::types::KillOutcome> {
    use axon_shell::extensions::task::KillTaskResponse;
    use axon_shell::session::result::ExtMethodResult;
    serde_json::from_str::<ExtMethodResult<KillTaskResponse>>(resp)
        .ok()
        .and_then(|envelope| envelope.result)
        .map(|payload| payload.outcome)
}
/// Map an `axon/subagent/cancel` response (payload under `result`) to a kill
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use axon_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: axon_shell::extensions::billing::BillingConfig,
...
/// Fetch the user's auto top-up rule via the `axon/auto-topup-rule` extension.
...
/// Map an `axon/auto-topup-rule` payload to an [`AutoTopupFetch`]. A body that
...
pub(super) fn parse_auto_topup_response(
    ...
    use axon_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.axon` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match axon_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
