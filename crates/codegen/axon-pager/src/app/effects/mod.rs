...
use axon_shell::sampling::error::http_status_from_error;
use axon_shell::session::{ExtMethodResult, SessionInfoResponse};
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = axon_shell::active_sessions::register(axon_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = axon_tools::types::compat::CompatConfig::default();
            let mcp_servers = axon_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = axon_shell::util::config::worktree_type();
                        ...
                        let ext_req = acp::ExtRequest::new(
                            "axon/git/worktree/resume_session",
                    ...
                    let ext_req = acp::ExtRequest::new(
                        "axon/git/worktree/create_from_worktree_sync",
        ...
        Effect::LoadSession { agent_id, session_id, session_cwd, chat_kind } => {
            ...
            if let Some(true) = session_flags.restore_code {
                meta.get_or_insert_with(acp::Meta::new)
                    .insert("axon/restore_code".into(), serde_json::Value::Bool(true));
            }
        ...
        Effect::ScanForeignSessions { cwd, compat, axon_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            axon_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &axon_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || axon_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
                    ...
                    match acp_send(request, &tx).await {
                        Ok(resp) => {
                            ...
                            match parsed {
                                ...
                                None => {
                                    tracing::warn!(
                                        "failed to parse blocked.invalid/sessions/list response"
                                    );
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            ...
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = axon_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = axon_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        axon_shell::session::restore::ProgressCallback,
                    > = {
                        use axon_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = axon_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use axon_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = axon_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use axon_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            axon_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = % e, "changelog fetch task failed");
                            axon_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    axon_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = axon_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                axon_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                axon_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = axon_hooks_plugins_types::HooksActionRequest {
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = axon_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                axon_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = axon_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = axon_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = axon_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = axon_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = axon_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: axon_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "axon/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: axon_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use axon_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use axon_shell::session::ClientType;
            use axon_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = axon_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    axon_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            axon_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = axon_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: axon_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use axon_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "axon/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !axon_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let axon_home = axon_shell::util::axon_home::axon_home();
                            let store = axon_shell::auth::read_auth_json(
                                    &axon_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = axon_shell::auth::AxonComConfig::default()
                                .auth_scope();
                            let auth = axon_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            ...
                            axon_shell::remote::fetch_settings_blocking(
                                &proxy_base,
                                &auth,
                                None,
                            )
...
/// Fetch session info from ACP via `axon/session/info`.
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = axon_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = axon_shell::session::model_display_name(
    ...
    let model_hash_line = if axon_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = axon_sandbox::profile_name()
        .map(|profile| format!("\n  Sandbox: {profile}"))
        .unwrap_or_default();
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = axon_version::display_version(
...
/// Build the `axon/interject` params. The optional structured `content`
