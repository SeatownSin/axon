...
//!
//! - Request/response `axon/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `axon/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `axon/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `axon/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `axon/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `axon-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `axon/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `axon/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> axon_shell::agent::roster::RosterEntry {
        use axon_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `axon/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("axon-4"));
        assert!(e.yolo);
    ...
    /// `axon/sessions/changed` round-trip: serialize the agent's `RosterChanged`
