...
/// [`axon_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<axon_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: axon_shell::util::config::WorktreeHintMode) -> Self {
        use axon_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        axon_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
...
/// [`axon_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `AXON_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `AXON_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("AXON_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`axon_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => axon_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: axon_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`axon_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.axon/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`AXON_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `AXON_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<axon_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<axon_announcements::RemoteAnnouncement>,
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `axon dashboard`, the
    ...
    /// non-selectable headers. Gated by `AXON_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `AXON_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `axon-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<axon_shell::extensions::session_search::SearchSessionHit>>,
    ...
    /// `axon_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `AXON_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: axon_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<axon_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `axon wrap` tip: set after the first
    ...
    pub pending_gate_verification: Option<axon_shell::auth::GateInfo>,
    ...
    /// `~/.axon/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `AXON_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<axon_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &axon_shell::util::config::RemoteSettings,
    ) -> Option<axon_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(axon_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &axon_shell::auth::AuthMeta) {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: axon_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: axon_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: axon_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<axon_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && axon_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: axon_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(axon_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(axon_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `AXON_FPS` overlay occupies (0 in non-dev builds), so
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    axon_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        axon_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    /// Log [`axon_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        ...
        let key = axon_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                axon_telemetry::session_ctx::log_event(
                    axon_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: axon_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `axon wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            axon_telemetry::session_ctx::log_event(axon_telemetry::events::ContextualTip {
                tip: axon_telemetry::events::ContextualTipKind::ImageInput,
                action: axon_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = axon_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![axon_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![axon_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: axon_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = axon_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&axon_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = axon_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(axon_workspace::foreign_sessions::RecentForeignSession {
                tool: axon_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![axon_workspace::file_system::FuzzyMatchResult {
