...
use axon_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use axon_shell::agent::auth_method::AuthMethodKind;
use axon_shell::agent::config::Config as AgentConfig;
use axon_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<axon_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use axon_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    ...
    for (name, mut value) in map {
        ...
        let mut def = axon_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut axon_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    axon_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `axon login` to authenticate \
         (or `axon login --device-code` if no browser is available)."
            .to_string()
    } else {
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &axon_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    axon_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = axon_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `axon workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = axon_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        axon_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(axon_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(axon_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    match prompt_result {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = axon_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &axon_acp_lib::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
...
mod tests {
    ...
    use super::*;
    use axon_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            axon_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> axon_acp_lib::AcpArgsBox<acp::ExtNotification> {
