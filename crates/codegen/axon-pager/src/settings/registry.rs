...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"axonnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"Axon Night"`).
...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: axon_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `AXON_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = axon_shell::models::default_model();
...
mod tests {
            ...
            match (meta.key, &meta.kind) {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in axon-tools"
                    );
                }
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        axon_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `axon_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = axon_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = axon_voice::STT_LANGUAGES.iter().map(|l| l.code).collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match axon_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: axon_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
