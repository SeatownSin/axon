//! `axon completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "axon" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("axon");
    if shell != Shell::Zsh {
    ...
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "axon", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `axon worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:axon-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:axon-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("axon");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "axon", &mut buf);
    ...
    // so `axon worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
