...
/// User-level vs project-level config files (`~/.axon` vs `{cwd}/.axon`).
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> axon_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = axon_agent::discovery::discover(cwd);
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match axon_shell::config::load_effective_config() {
        Ok(r) => r,
...
/// True when `path` is a deletable local persona file (user or project `.axon/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.axon/{subdir}` or `{cwd}/.axon/{subdir}`, not bundled.
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    axon_shell::config::load_effective_config()
        .ok()
        .and_then(|root| axon_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in axon-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    axon_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        axon_agent::config::PromptMode::Extend => "extend",
        axon_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use axon_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".axon").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".axon").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.axon/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".axon").join("personas").join("gone.toml");
