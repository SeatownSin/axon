...
use axon_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: axon_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `axon-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Axon what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from axon\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from axon\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p axon-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/axon-pager
