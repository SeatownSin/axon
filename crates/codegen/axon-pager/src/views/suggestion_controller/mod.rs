...
//! `axon/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `axon/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `axon/suggest` response.
...
// (e.g. axon-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `axon/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `axon/suggest` response.
...
/// Wire `limit` for `axon/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `AXON_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `AXON_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `axon/suggest` request.
    /// Resolved at construction from `AXON_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: axon_config::env_bool("AXON_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: axon_config::env_bool("AXON_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("AXON_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `axon/suggest` response arrives, with the text and
