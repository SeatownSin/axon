...
use axon_announcements::visible_announcements;
...
fn is_critical(a: &axon_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &axon_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    ...
    is_critical(a) && !axon_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !axon_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &axon_announcements::RemoteAnnouncement) -> bool {
...
fn usable_cta(a: &axon_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &axon_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    ...
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a axon_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [axon_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a axon_announcements::RemoteAnnouncement, &'a str)> {
...
pub fn session_announcement_hide_keys_at(
    ...
    visible_announcements(announcements)
        ...
        .map(axon_announcements::announcement_hide_key)
        .collect()
}
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &axon_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| axon_announcements::AnnouncementCta {
    ...
    fn session_hide_keys_cover_live_criticals_and_promos_only() {
        ...
        let list = vec![
            ...
            promo("promo-1", "upsell", Some(("Go", "https://blocked.invalid"))),
            expired_promo,
        ];
    ...
    fn first_promo_selection_filters_severity_and_hidden() {
        let list = vec![
            ...
            promo("p-a", "A promo", Some(("Go", "https://blocked.invalid"))),
    ...
    fn first_session_announcement_prefers_critical_over_promo() {
        let list = vec![
            promo("p", "upsell", Some(("Go", "https://blocked.invalid"))),
            RemoteAnnouncement {
                id: Some("c".into()),
                severity: Some("critical".into()),
    ...
    fn promo_cta_target_requires_usable_pair() {
        ...
        label_only.cta = Some(axon_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn usable_cta_caption_trims_and_never_resurrects_unusable_cta() {
        let mut p = promo("p", "msg", Some(("Go", "https://blocked.invalid")));
    ...
    fn promo_cta_returns_label_and_pinned_flag() {
        ...
        let dismissible = [promo("d", "msg", Some(("Go", "https://blocked.invalid")))];
    ...
    fn promo_cta_target_yields_to_critical_slot_owner() {
        let promo_ann = promo("p", "upsell", Some(("Go", "https://blocked.invalid/promo")));
        ...
        assert_eq!(
            ...
            Some((Some("p"), "https://blocked.invalid/promo"))
        );
    ...
    fn render_promo_row_button_and_hide_affordances() {
        let mut ann = promo(
            "p",
            "New promo",
            Some(("Get SuperAxon", "https://blocked.invalid/axon")),
        );
        ann.cta.as_mut().unwrap().caption = Some("or use Ctrl+O".into());
        let anns = [ann];
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(row0.starts_with("[Get SuperAxon]"), "row0={row0:?}");
        assert!(
    ...
    fn render_promo_row_hover_styles() {
        let anns = [promo("p", "msg", Some(("Go", "https://blocked.invalid")))];
        let area = Rect::new(0, 0, 80, 1);
        let theme = Theme::current();
    ...
    fn render_promo_row_truncates_button_never_affordances() {
        let anns = [promo(
            ...
            "msg",
            Some((
                "Upgrade to SuperAxon Heavy for the exclusive preview",
