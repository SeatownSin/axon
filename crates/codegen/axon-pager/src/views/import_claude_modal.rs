...
//! are written to `.axon/config.toml`.

use axon_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.axon/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".axon").join("config.toml");
...
mod tests {
    use super::*;
    use axon_shell::claude_import::PathKind;
    use axon_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use axon_shell::claude_import::ImportableItem;
