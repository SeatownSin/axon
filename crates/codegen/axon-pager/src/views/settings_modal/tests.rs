...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "Axon Test".to_string(),
            acp::ModelId::new(Arc::from("axon-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Axon Test", 4); // cursor between "Axon" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    ...
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"axonnight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether SpaceAXON may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow SpaceAXON to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether SpaceAXON may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
    ...
    assert!(
        ...
        "validation_error must be Some for unknown model 'a' \
         (catalog has 'Axon 4 Fast' only)",
    );

    // Enter on a buffer that fails the KnownModel validator
    // (catalog has 'Axon 4 Fast'; "a" doesn't match) is
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Axon Tes", "Axon Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Axon Test"));
...
//    the "Tip · Ask Axon…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Axon Tst", "Axon T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask Axon to change a setting" (34 cells);
    ...
    assert!(
        ...
        "width=40 must NOT render LONG path (no `axonday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Axon") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"axonnight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `axonnight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `axonnight`; entering the picker
            // captures `original_value = current value = axonnight`,
...
/// misleading: on dark themes (AxonNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the AxonDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::AxonNight => crate::theme::Theme::axonnight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::AxonDay => crate::theme::Theme::axonday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::axonnight(),
    };
    assert_ne!(
