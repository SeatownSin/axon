...
/// `~/.axon/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<axon_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    axon_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.axon/config.toml`.
///
...
/// `~/.axon/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no axon home"))?;
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/axon");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/axon");
    }
    ...
    fn peek_test_image() -> crate::prompt_images::PastedImage {
        crate::prompt_images::PastedImage {
            element_id: axon_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
    ...
    /// per-key serial lock. The `AXON_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(AXON_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `AXON_AGENT_DASHBOARD` key reads it concurrently.
