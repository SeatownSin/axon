...
/// Env override for the whole feature: `AXON_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "AXON_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `AXON_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "AXON_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "axon-build-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `AXON_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| axon_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `axon-build-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `axon-build-0.1` default): the shell
