...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a axon_announcements::RemoteAnnouncement>,
...
pub(super) fn render_version_badge(
    ...
    let channel = axon_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", axon_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a axon_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a axon_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [axon_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a axon_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("your provider");
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Axon may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &axon_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("A subscription is required for this model.");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://github.com/SeatownSin/axon");
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        axon_ratatui_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use axon_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = axon_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn long_ann() -> axon_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = axon_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
