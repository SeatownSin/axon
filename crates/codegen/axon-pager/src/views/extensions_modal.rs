...
use axon_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &axon_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &axon_hooks_plugins_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[axon_hooks_plugins_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<axon_hooks_plugins_types::PluginOrigin>,
) -> axon_hooks_plugins_types::PluginInfo {
    axon_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: axon_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: axon_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: axon_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a axon_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &axon_hooks_plugins_types::PluginInfo) -> PluginGroup {
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [axon_hooks_plugins_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a axon_hooks_plugins_types::MarketplaceScanResult,
        plugin: &'a axon_hooks_plugins_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [axon_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[axon_hooks_plugins_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> axon_telemetry::events::ExtensionsModalTab {
        use axon_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(axon_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(axon_hooks_plugins_types::PluginsAction),
    ...
    MarketplaceAction(axon_hooks_plugins_types::MarketplaceAction),
...
impl ModalInputField {
    ...
    pub(crate) fn viewport(&self, width: usize) -> axon_ratatui_textarea::SingleLineViewport {
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use axon_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use axon_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            axon_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<axon_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<axon_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<axon_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<axon_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[axon_hooks_plugins_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // Project-scoped `.axon/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".axon" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Project hooks
    if source_dir.ends_with("/.axon/hooks") || source_dir.contains("/.axon/hooks/") {
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            ...
            axon_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            axon_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &axon_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use axon_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &axon_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[axon_hooks_plugins_types::ComponentItem]); 6] {
    use axon_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &axon_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &axon_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        ...
        let (label, is_custom) =
            derive_source_label("/repo/work/.axon/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(
        ...
        axon_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: axon_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = axon_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(axon_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> axon_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: axon_hooks_plugins_types::PluginOrigin,
    ) -> axon_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> axon_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(axon_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    axon_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    axon_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                axon_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> axon_hooks_plugins_types::HookInfo {
        axon_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: axon_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: axon_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> axon_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> axon_hooks_plugins_types::MarketplaceScanResult {
        axon_hooks_plugins_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.axon/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<axon_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> axon_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                axon_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                axon_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(axon_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = axon_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> axon_hooks_plugins_types::ComponentItem {
        axon_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> axon_hooks_plugins_types::PluginComponents {
        axon_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(axon_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = axon_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<axon_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(axon_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = axon_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = axon_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = axon_hooks_plugins_types::PluginScope::Config;
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            axon_hooks_plugins_types::PluginOrigin::Unknown,
        );
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(axon_hooks_plugins_types::PluginComponents::default());
