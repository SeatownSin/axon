...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `axon wrap` on the local end, so
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(axon_sandbox::sandbox_profile_conflicts(workspace))
}
...
/// Pure SSH `axon wrap` recommendation — suggests launching the session
/// through `axon wrap ssh <host>` on the user's local machine, which gives a
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && axon_shell::util::clipboard::wayland_data_control_supported(),
        is_wayland && axon_shell::util::clipboard::native_tool_name() == "wl-copy",
    )
}
...
pub fn format_clipboard_diagnostics(input: ClipboardDiagnosticsInput<'_>) -> ClipboardDiagnostics {
    ...
    let fix = match delivery {
        ...
        ClipboardDelivery::Unverified if input.is_ssh => {
            Some("axon wrap <ssh command> or /minimal")
        }
        ClipboardDelivery::Unverified if input.container_no_display => {
            Some("axon wrap <command> or /minimal")
        }
        ClipboardDelivery::Unverified => Some("axon wrap or /minimal"),
        ClipboardDelivery::Failed if input.is_ssh => Some("axon wrap <ssh command> or /minimal"),
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart Axon.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart Axon.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn clipboard_diagnostics_unknown_ssh_is_unverified() {
        ...
        for expected in [
            ...
            "wrap         off",
            "status       unverified",
            "fix          axon wrap <ssh command> or /minimal",
        ] {
            assert!(
    ...
    fn clipboard_diagnostics_known_terminal_status() {
        ...
        assert!(
            unsupported
                .text
                .contains("fix          axon wrap <ssh command> or /minimal")
        );
    ...
    fn clipboard_diagnostics_tmux_wrap_and_container() {
        ...
        assert!(
            container
                .text
                .contains("fix          axon wrap <command> or /minimal")
        );
    ...
    // -- ssh_wrap_hint: `axon wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("axon wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `axon wrap` — adoption silences the hint by itself.
