...
//! the generic [`CodeBlockSpan`](axon_markdown::CodeBlockSpan) API) and
...
use axon_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](axon_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](axon_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a axon_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `AxonDay` is the only light theme; every other concrete theme (and the
/// `AxonNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `axon_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::AxonDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_axonday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::AxonDay),
            "AxonDay is the light theme"
        );
        for dark in [
            ThemeKind::AxonNight,
