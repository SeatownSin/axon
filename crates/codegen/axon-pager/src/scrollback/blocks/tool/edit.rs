...
mod tests {
    ...
    /// Pins AxonNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_axonnight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::axonnight().diff_uses_line_fg(),
            "AxonNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_axonnight_syntect` (lock is not reentrant).
...
"
    ...
    fn syntax_highlight_splits_keyword_and_string_fg() {
        let _guard = pin_axonnight_syntect();
