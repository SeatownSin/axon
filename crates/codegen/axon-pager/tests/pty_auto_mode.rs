...
//!
//! Uses `axon-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.axon/auth.json` from `AXON_AUTH_JSON` (path) or the
//! developer's `~/.axon/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p axon-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use axon_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.axon/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("AXON_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.axon/auth.json — may hit device login");
    }
    ...
    // layer below requirements; "1"/"0" parse to on/off (axon_config::
    ...
    env.push((
        "AXON_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("AXON_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (axon-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "AXON_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
