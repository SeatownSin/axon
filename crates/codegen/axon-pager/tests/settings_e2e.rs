...
fn make_state() -> SettingsModalState {
    ...
    axon_pager::app::set_voice_mode_enabled_for_test(true);
...
fn defaults_round_trip_through_registry() {
    use axon_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    axon_pager::appearance::cache::set_keep_text_selection(
        axon_pager::appearance::TextSelection::Flash,
    );
    axon_pager::appearance::cache::set_show_thinking_blocks(true);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("axonnight"),
            "auto_dark_theme" => SettingValue::Enum("axonnight"),
            "auto_light_theme" => SettingValue::Enum("axonday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = axon_pager::settings::default_value_for(meta);
...
// `crates/codegen/axon-pager/src/app/dispatch.rs::tests` (next to
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use axon_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use axon_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("change theme to axonday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    axon_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use axon_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use axon_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use axon_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use axon_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use axon_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        axon_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        axon_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_parses_aliases() {
    use axon_pager::slash::commands::privacy::parse_privacy_arg;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use axon_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use axon_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use axon_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use axon_pager::appearance::TextSelection;
    ...
    axon_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    axon_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    axon_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    axon_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    axon_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    axon_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    axon_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    axon_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
