...
/// `AXON_OSC52_SINK=1` (the signal an inner `axon` uses to trust OSC 52 over
...
fn wrap_osc52_sink_env_advertised_through_shell() {
    let (code, raw) = run_wrap(
        &["echo sink=$AXON_OSC52_SINK"],
        &[("SHELL", "/bin/sh"), ("AXON_OSC52_SINK", "0")],
    );

    assert!(
        raw.contains("sink=1"),
        "wrapped child must see AXON_OSC52_SINK=1 through the shell route\nraw:\n{raw}"
    );
    assert!(
        !raw.contains("sink=0"),
