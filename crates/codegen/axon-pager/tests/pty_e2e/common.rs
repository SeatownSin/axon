...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("AXON_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `axon_pager_pty_harness::flows` (re-exported above).
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("AXON_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`AXON_TEST_VERSION`) and an explicit `AXON_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/AXON_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    ...
    // auto-trust.
    env.push(("AXON_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set AXON_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("AXON_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".axon")
        .join(axon_workspace::trust::TRUST_FILE_NAME);
    let store = axon_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&axon_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `axon_pager_pty_harness::flows`
// (re-exported above).
...
// unit tests in axon-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.axon/config.toml` with a `[ui]` section body (e.g.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("AXON_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "AXON_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        axon_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `axon wrap` run budget. Same contention math as the requirements-version
...
/// Run `axon wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `AXON_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("AXON_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env).expect("spawn axon wrap");
...
/// `axon wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$AXON_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("AXON_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use axon_pager_pty_harness::host_clipboard::{
...
pub(crate) use axon_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
