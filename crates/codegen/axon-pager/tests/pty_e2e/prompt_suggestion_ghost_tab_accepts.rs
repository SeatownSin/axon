...
/// turn and the turn-end `axon/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`axon-build-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "AXON_PROMPT_SUGGESTIONS");
    env.push(("AXON_PROMPT_SUGGESTIONS".into(), "true".into()));
