...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "AXON_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// safe-open path — observed via the `AXON_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("AXON_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `AXON_ANNOUNCEMENTS_OVERRIDE`.
...
/// `AXON_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
