...
/// should make a mode sticky across plain `axon` launches.
...
async fn minimal_cli_screen_mode_does_not_persist() {
    ...
    let config_path = content.home().join(".axon").join("config.toml");
    ...
    second
        ...
        .unwrap_or_else(|e| {
            panic!(
                "plain axon should open fullscreen after --minimal (no sticky write): {e}\nscreen:\n{}",
