...
//! `axon_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `AXON_POOL_*` /
//! `AXON_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `AXON_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `axon-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `axon-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `axon-telemetry` consumes
// the same type via `axon_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use axon_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `AXON_CLIENT_NAME` /
/// `AXON_CLIENT_VERSION` env vars. Returns `None` when
/// `AXON_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("AXON_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("AXON_CLIENT_VERSION").ok(),
        })
}
...
/// `axon agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn axon_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(axon_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
