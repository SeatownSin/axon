...
//! `axon-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.axon/skills
    Local,
    /// repo_root/.axon/skills
    Repo,
    /// ~/.axon/skills
    User,
    /// ~/.axon/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.axon/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the axon-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.axon/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.axon/skills"},
