...
use axon_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = axon_sampling_types::reported_cost_ticks(
        value
...
mod tests {
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = axon_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
