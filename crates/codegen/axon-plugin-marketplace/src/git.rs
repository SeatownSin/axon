...
//! Cache root: `~/.axon/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = axon_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(axon_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
