//! Parse marketplace sources from `~/.axon/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `AXON_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
/// instead of `~/.axon`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("AXON_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("AXON_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("AXON_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
