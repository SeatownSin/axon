...
//!
//! > `~/.axon/bin/axon` resolves to a binary that passes the smoke-test, OR it
...
use axon_update::auto_update::install_internal_from_base;
...
/// (`axon` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `axon` should resolve to after an install attempt.
...
/// to both `axon` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p axon-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("AXON_BLITZ_ITERS")
        .ok()
