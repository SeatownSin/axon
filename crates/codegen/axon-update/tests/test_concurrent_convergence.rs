...
//!   temp-name fix, every `0.1.x` download shared one `axon-0.1.tmp`
...
use axon_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use axon_update::version::installed_on_disk_version;

/// Assert the active `~/.axon/bin/axon` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("axon");
    assert!(link.is_symlink(), "axon must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active axon symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("axon-{version}-{platform}"),
        "active axon must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active axon must pass the smoke-test");
}

/// Lay down a managed-install layout in the test AXON_HOME:
/// `bin/axon -> ../downloads/axon-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("axon-{version}-{}", host_platform());
...
// installers that actually maintain the managed `~/.axon/bin/axon` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.axon/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("axon-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `axon-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/axon-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("axon-{version}-{platform}"));
        assert_eq!(
