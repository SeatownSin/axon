...
//!
//! The installer lives in the sibling `axon-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../axon-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".axon").join("downloads");
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("AXON_BIN_DIR", home.join(".axon").join("bin"))
        .env("AXON_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
