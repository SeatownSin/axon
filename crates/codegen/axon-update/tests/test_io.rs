...
//!
//! These tests touch global process state — `AXON_HOME` (a `OnceLock` in
//! `axon-config`), `AXON_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `AXON_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the AXON_HOME isolation used in other
//! integration tests.
...
use axon_update::write_version_cache;
