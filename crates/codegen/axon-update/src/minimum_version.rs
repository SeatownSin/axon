...
//!
//! When `cli.minimum_version` is set in any config layer, Axon refuses to
//! start below that floor. With auto-update on, we install
//! `max(latest, minimum)`; otherwise the user is asked to run `axon update`.
//!
//! Set `AXON_TEST_VERSION` to manually exercise either path without producing
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "The minimum version \"{value}\" in your Axon configuration \
    ...
    #[error(
        "This version of Axon ({current}) is no longer supported. \
         Run `axon update` to install version {minimum} or later."
    )]
    ...
    /// `axon update --version X` requested a version below the floor.
    #[error(
        "Cannot install Axon {target}: the configured minimum is {minimum}. \
         Run `axon update` to install the latest allowed version."
    )]
...
/// Used by `axon update` to keep the install target at or above the pin.
...
///
/// `AXON_TEST_VERSION` lets devs override the running version to skip
...
pub async fn enforce_minimum_version_or_exit(update_config: &UpdateConfig) {
    ...
    match enforce_minimum_version(Some(&min), update_config).await {
        ...
        Ok(EnforcementOutcome::Upgraded) => {
            // TODO: restart_axon uses exec() which carries the same
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("AXON_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("AXON_TEST_VERSION", "0.1.50") };
        let decision =
            evaluate_minimum_version(&get_installed_axon_version(), Some("0.1.100")).unwrap();
        assert!(matches!(
            decision,
            MinimumVersionDecision::BelowMinimum { .. }
        ));
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("AXON_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("AXON_TEST_VERSION") },
        }
    }
}
