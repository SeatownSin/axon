...
pub fn create_snapshot_with_symlink(btrfs_info: &BtrfsInfo, dest: &Path) -> Result<SnapshotResult> {
    ...
    if snapshot_path.exists() {
            ...
            .is_ok_and(|m| m.file_type().is_symlink())
        {
            ...
            bail!(
                "refusing to delete pre-existing snapshot {}: outside axon-managed \
                 btrfs storage",
                snapshot_path.display()
            );
        } else {
    ...
    // contains an empty .axon-snapshots/ directory (btrfs excludes nested subvolumes
    // from snapshots, leaving only empty directory placeholders). Remove it to
    // keep the worktree clean.
    let stale_snapshots_dir = snapshot_path.join(".axon-snapshots");
        ...
        && let Err(e) = std::fs::remove_dir(&stale_snapshots_dir)
    {
        tracing::debug!(
            ...
            error = %e,
            "failed to remove stale .axon-snapshots placeholder from snapshot"
        );
    }
...
/// root mount), snapshots go under `.axon-snapshots/` to stay hidden from git;
...
pub fn snapshot_dest_path(btrfs_mount: &Path, subvolume_root: &Path, dest: &Path) -> PathBuf {
    let subdir = if btrfs_mount == subvolume_root {
        BTRFS_SNAPSHOT_SUBDIRS[1] // ".axon-snapshots"
    } else {
...
///   (`worktrees` or `.axon-snapshots`),
/// - and that directory sits **directly under a real btrfs mount point** (from
///   the live mount table), anchoring the delete to axon-managed storage rather
...
fn is_safe_snapshot_delete_target_in(snapshot_path: &Path, btrfs_mounts: &[PathBuf]) -> bool {
    ...
    // Parent must be a snapshot-storage dir (`worktrees` / `.axon-snapshots`)...
...
/// `".axon-snapshots"` is used when the btrfs mount IS the repo subvolume
/// (dot-prefixed to stay hidden from git).
pub const BTRFS_SNAPSHOT_SUBDIRS: &[&str] = &["worktrees", ".axon-snapshots"];
...
mod tests {
    ...
    fn test_btrfs_snapshot_metadata_round_trip() {
        let meta = BtrfsSnapshotMetadata {
            ...
            mount_target: PathBuf::from("/home/user/.axon/worktrees/repo/session/wt-abc"),
    ...
    fn test_write_and_remove_btrfs_metadata() {
        ...
        let mount_target = Path::new("/home/user/.axon/worktrees/wt-abc");
    ...
    fn test_snapshot_dest_path_separate_root_mount() {
        ...
        let dest = Path::new("/home/user/.axon/worktrees/repo/session/wt-abc");
    ...
    fn test_snapshot_dest_path_subvol_mount() {
        // btrfs mount IS the subvolume root → snapshots under .axon-snapshots/.
        let mount = Path::new("/workspace/repo");
        let dest = Path::new("/home/user/.axon/worktrees/repo/session/wt-xyz");
        let got = snapshot_dest_path(mount, mount, dest);
        assert_eq!(
            got.parent().unwrap(),
            Path::new("/workspace/repo/.axon-snapshots")
        );
    ...
    fn test_snapshot_dest_path_disambiguates_same_basename_across_repos() {
        ...
        let btrfs_mount = Path::new("/mnt/btrfs");
        let subvolume_root = Path::new("/workspace/repo");
        let dest_a = Path::new("/home/user/.axon/worktrees/repo-a/session/wt-abc");
        let dest_b = Path::new("/home/user/.axon/worktrees/repo-b/session/wt-abc");
    ...
    fn test_snapshot_meta_targets_matches_only_same_dest() {
        ...
        let dest = Path::new("/home/user/.axon/worktrees/repo-a/session/wt-abc");
        ...
        let other = Path::new("/home/user/.axon/worktrees/repo-b/session/wt-abc");
