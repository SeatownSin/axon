...
pub(crate) fn git_command() -> Command {
    ...
    cmd.envs(axon_tty_utils::pager_env());
...
///
/// `ref_name` must be a fully-qualified ref (e.g. `refs/axon/subagents/<id>`);
...
fn snapshot_worktree_to_ref_inner(
    ...
    const NAME: &str = "Axon Snapshot";
    const EMAIL: &str = "axon-snapshot@example.com";
...
mod tests {
    ...
    fn test_snapshot_worktree_captures_tracked_and_untracked() {
        ...
        let ref_name = "refs/axon/snapshots/test";
    ...
    fn test_snapshot_excludes_ignored_files() {
        ...
        let ref_name = "refs/axon/snapshots/ignored";
    ...
    fn test_snapshot_captures_tracked_then_ignored_file() {
        ...
        let ref_name = "refs/axon/snapshots/tracked-ignored";
    ...
    fn test_snapshot_clean_tree_equals_head_tree() {
        ...
        let ref_name = "refs/axon/snapshots/clean";
    ...
    fn test_snapshot_overwrites_ref_on_second_call() {
        ...
        let temp = TempDir::new().unwrap();
        let (_repo, wt) = repo_with_worktree(&temp);
        let ref_name = "refs/axon/snapshots/overwrite";
    ...
    fn test_snapshot_survives_worktree_removal() {
        ...
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        std::fs::write(wt.join("untracked.txt"), "brand new").unwrap();
        let ref_name = "refs/axon/snapshots/survives";
    ...
    fn test_snapshot_does_not_mutate_real_index() {
        ...
        snapshot_worktree_to_ref(&wt, "refs/axon/snapshots/noindex", "no mutate").unwrap();
    ...
    fn test_snapshot_preserves_crlf_under_autocrlf_config() {
        ...
        let ref_name = "refs/axon/snapshots/crlf";
    ...
    fn test_snapshot_captures_non_ascii_spaced_path() {
        ...
        let ref_name = "refs/axon/snapshots/unicode";
    ...
    fn test_rehydrate_round_trip_restores_working_state() {
        ...
        let snap =
            snapshot_worktree_to_ref(&wt, "refs/axon/snapshots/roundtrip", "round trip").unwrap();
    ...
    fn test_rehydrate_base_missing_falls_back_to_snapshot() {
        ...
        let snap = snapshot_worktree_to_ref(&wt, "refs/axon/snapshots/orphan-src", "src").unwrap();
    ...
    fn test_rehydrate_self_heals_over_leftover_dest_dir() {
        ...
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        std::fs::write(wt.join("untracked.txt"), "brand new").unwrap();
        let snap = snapshot_worktree_to_ref(&wt, "refs/axon/snapshots/idem", "idem").unwrap();
        crate::remove_worktree(&wt).unwrap();
    ...
    fn test_transfer_snapshot_to_repo_makes_standalone_ref_durable() {
        ...
        let ref_name = "refs/axon/subagents/standalone";
    ...
    fn test_rehydrate_registers_worktree_in_db() {
        ...
        // Isolate the worktree DB (lock + AXON_HOME → private tmp + restore).
        let fx = crate::db::AxonHomeFixture::new();

        let (repo_path, wt) = repo_with_worktree(&temp);
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        let snap = snapshot_worktree_to_ref(&wt, "refs/axon/snapshots/db", "db test").unwrap();
        crate::remove_worktree(&wt).unwrap();
        ...
        // global AXON_HOME) DB and INSERT-OR-REPLACE our row.
        ...
        // other subagent rows since AXON_HOME is process-global.
