...
/// Serializes tests that mutate the process-global `AXON_HOME` env var so they
...
#[cfg(test)]
static AXON_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`AXON_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `AXON_HOME` at a fresh private tmp dir, and restores the prior value on drop.
...
impl AxonHomeFixture {
    pub(crate) fn new() -> Self {
        ...
        // until AXON_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("AXON_HOME");
        unsafe { std::env::set_var("AXON_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
    ...
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("AXON_HOME", p),
                None => std::env::remove_var("AXON_HOME"),
            }
        }
    }
}
