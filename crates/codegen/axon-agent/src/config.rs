//! Agent definition types — parsed from `.axon/agents/*.md` files.
...
/// Portable agent identity — parsed from .axon/agents/*.md.
...
pub struct AgentDefinition {
    ...
    pub capability_mode: Option<axon_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .axon/agents/ (project-level, highest priority)
    Project,
    /// ~/.axon/agents/ (user-level)
    User,
...
pub enum MemoryScope {
    /// `~/.axon/agent-memory/<name>/`
    User,
    /// `<project>/.axon/agent-memory/<name>/`
    Project,
    /// `<project>/.axon/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            ...
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".axon/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".axon/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `axon_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        if path_str.contains(".axon/agents/") || path_str.contains(".axon\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".axon/bundled/agents/")
            || path_str.contains(".axon\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: axon_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: axon_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: axon_tool_types::PLAN_SUBAGENT.description.to_string(),
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.axon/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.axon/agent-memory-local/a")
        );
