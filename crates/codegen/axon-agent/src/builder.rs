...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.axon/worktrees/project/fork-...-overlay`) that must stay
    ...
    memory_backend: Option<Arc<dyn axon_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: axon_tools::implementations::web_search::WebSearchConfig,
    ...
    attribution_callback: Option<axon_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in axon-shell).
...
fn merge_tool_params(
    tool_config: &mut axon_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(axon_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<axon_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn axon_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: axon_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn axon_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: axon_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&axon_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
        ...
        if task_stripped {
            use axon_tools::types::tool::ToolNamespace;
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                axon_tools::types::resources::TruncationCfg(
                    axon_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(axon_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(axon_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI [`axon_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: axon_tool_types::SubagentToolNaming<'static> =
    axon_tool_types::SubagentToolNaming {
...
/// shared [`axon_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => axon_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => axon_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => axon_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`axon_tool_types::SubagentDescriptor`] and defers to
/// [`axon_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<axon_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            axon_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = axon_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        axon_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: axon_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(axon_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(axon_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: axon_tools::types::config_source::ConfigSource::Project {
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, axon_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, axon_sampling_types::HostedTool::XSearch)),
