...
//!
//! Searches `.axon/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.axon/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.axon/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".axon/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.axon/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.axon/agents` / `.claude/agents`) under each
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.axon/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.axon/agents/` walking from `cwd` up to repo root
/// 2. `~/.axon/agents/` (user-level)
...
/// Load project agent definitions from every `.axon/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".axon").join("agents");
        let agents_dir_2 = inner_dir.join(".axon").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".axon").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".axon")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".axon")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".axon").join("agents");
        let bundled_dir = home.join(".axon").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".axon")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".axon")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".axon").join("agents");
        let bundled_dir = home.join(".axon").join("bundled").join("agents");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.axon/agents/) agent named "explore" should NOT shadow
