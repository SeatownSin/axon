...
/// Default install directory name under `~/.axon/`.
...
/// Lives here (not in axon-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("AXON_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.axon/installed-plugins/`
...
pub enum InstallError {
    ...
    #[error(
        "refusing unpinned remote plugin code for '{plugin}' from {url}: \
         marketplace.require_sha / AXON_MARKETPLACE_REQUIRE_SHA is enabled and \
