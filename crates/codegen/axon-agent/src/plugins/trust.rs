...
//!
//! Plugins from project directories (`.axon/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.axon/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.axon/`.
...
impl TrustStore {
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.axon/trusted-plugins`.
    ...
    /// rewrites `~/.axon/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.axon/trusted-plugins`.
