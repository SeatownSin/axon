...
//! - `Default` -- the legacy Axon Build prefix (built by the shell layer).
...
use axon_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Axon Build prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .axon/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `axon_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.axon/, ~/.claude/).
    ...
    /// (`~/.axon/projects/<encoded-cwd>/mcps`). Surfaced in
