...
//!
//! The tool runtime contract (`Tool` trait) lives in `axon_tool_runtime`.
...
///
/// Serializes to snake_case (`axon_build`, `mcp`, …) for the
...
/// (e.g. `"AxonBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "AxonBuild")]
    AxonBuild,
    #[serde(alias = "AxonBuildConcise")]
    AxonBuildConcise,
    #[serde(alias = "AxonBuildHashline")]
    AxonBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
