...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (axon build harness) ─────────────
...
///
/// `"/path/.axon/skills/my-skill/SKILL.md"` -> `"/path/.axon/skills/"`
...
/// Used by the budgeted (axon build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Axon build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the axon
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // axon-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: axon build harness (budgeted XML) ───────────
