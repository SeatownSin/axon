...
///
/// Use with `#[schemars(with = "AxonIntegerSchema")]` on `Option<usize>` fields.
pub struct AxonIntegerSchema;
impl schemars::JsonSchema for AxonIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "axon_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `axon-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use axon_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
