...
//! ## Context helpers
//!
//! Tools access session state through `axon_tool_runtime::ToolCallContext`
...
) -> Result<SharedResources, axon_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, axon_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<axon_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            axon_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> axon_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(axon_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = axon_tool_protocol::ToolCallId::new(call_id)
...
pub fn behavior_version(ctx: &axon_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
