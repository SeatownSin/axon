...
pub use axon_tty_utils::{detach_from_tty, pager_env};
...
/// Sentinel value for [`AXON_AGENT_ENV`] on agent tool terminals.
pub const AXON_AGENT_ENV_VALUE: &str = "1";
...
/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${AXON_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${AXON_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
#[cfg(test)]
mod tests {
    use super::{AXON_AGENT_ENV, AXON_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${AXON_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${AXON_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${AXON_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(AXON_AGENT_ENV, "AXON_AGENT");
        assert_eq!(AXON_AGENT_ENV_VALUE, "1");
    }
}
