//! Conversion from the gRPC wire config types (`axon-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`axon_tools_api::config_validation`] so every consumer (this
...
pub use axon_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`axon_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: axon_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let axon_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
...
pub fn tool_server_config_from_entries(
    entries: Vec<axon_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> axon_tools_api::ToolConfigEntry {
        axon_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
    ...
    fn minimal_entry_converts_with_defaults() {
        let cfg = tool_config_from_entry(0, entry("AxonBuild:read_file")).unwrap();
        assert_eq!(cfg.id, "AxonBuild:read_file");
    ...
    fn fully_populated_entry_converts_field_by_field() {
        let mut e = entry("AxonBuild:grep");
    ...
    fn name_override_valid_tool_id_charset_is_accepted() {
        for name in ["search", "AxonBuild:grep", "a-b_C9"] {
            let mut e = entry("AxonBuild:grep");
    ...
    fn name_override_outside_tool_id_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "emoji✨", "dot.name"] {
            ...
            assert_eq!(err.tool_id, "AxonBuild:grep");
    ...
    fn server_config_conversion_rejects_invalid_name_override_entry() {
        let mut bad = entry("AxonBuild:grep");
    ...
    fn server_config_conversion_preserves_valid_name_overrides() {
        let mut a = entry("AxonBuild:grep");
        a.name_override = Some("search".to_owned());
        let cfg = tool_server_config_from_entries(vec![a, entry("AxonBuild:bash")]).unwrap();
