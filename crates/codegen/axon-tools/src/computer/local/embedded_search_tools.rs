...
//! config helper `axon-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `AXON_TOOLS_FIND_BFS` / `AXON_TOOLS_GREP_UGREP` (+
//! `AXON_FIND_BFS` / `AXON_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.axon/vendor/<name>-<ver>-<target>`) →
//! `~/.axon/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("AXON_TOOLS_BFS_VER"),
    "-",
    env!("AXON_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("AXON_TOOLS_UGREP_VER"),
    "-",
    env!("AXON_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "AXON_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "AXON_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.axon/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.axon/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// `AXON_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.axon/vendor` and the extracted `bfs` runs.
    ...
    /// mode-0644 `AXON_TOOLS_*_PATH` / vendor copy) must fall through to the OS
