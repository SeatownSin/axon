...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        #[cfg(target_os = "linux")]
        if axon_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| axon_sandbox::child_net::install_child_network_filter());
            }
        }
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, AXON_AGENT=1, etc.)
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "AXON_LOGIN_ENV";
...
fn spawn_shell_command(
    ...
    let build_cmd = |with_breakaway: bool| {
        ...
        let inv = axon_config::shell::shell_command_argv(command);
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export AXON_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $AXON_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    /// `parse_login_env_capture` is `#[cfg(unix)]`; without the same gate here
    /// the whole crate's test binary fails to build on Windows.
    #[cfg(unix)]
    #[test]
