//! ToolBridge: adapter that wraps `axon-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the axon_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, axon_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, axon_tool_runtime::ToolError> {
        use crate::implementations::axon_build::scheduler::types::{
        ...
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    axon_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                axon_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx.await.map_err(|_| {
            axon_tool_runtime::ToolError::custom("process_manager", "Scheduler actor dropped reply")
        })
    }
...
mod tests {
    ...
    impl axon_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> axon_tool_protocol::ToolId {
            axon_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::axon_tool_runtime::ListToolsContext,
        ) -> axon_tool_types::ToolDescription {
            axon_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + axon_build's snake_case in one registry
