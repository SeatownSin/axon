...
impl axon_tool_runtime::Tool for ReadTool {
    type Args = ReadInput;
    type Output = ReadFileOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("read").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if let Ok(meta) =
            crate::implementations::axon_build::read_file::bytes_to_metadata(&file_bytes)
            && meta.is_image()
        {
