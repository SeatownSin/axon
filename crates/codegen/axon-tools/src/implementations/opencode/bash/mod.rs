...
impl axon_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(axon_tool_runtime::ToolError::execution(
                    axon_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = axon_tool_protocol::ToolCallId::new(call_id).unwrap();
        let mut ctx = axon_tool_runtime::ToolCallContext::new(id);
    ...
    async fn tool_metadata() {
        use crate::types::tool_metadata::ToolMetadata;
        let tool = BashTool;
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "bash");
