...
///
/// `skills` is the standard layout (`.axon/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Axon Build sessions.
...
/// name (e.g. `~/.axon/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Axon Build.
...
/// `cwd` (exclusive). At each directory, checks for `.axon/skills/`,
...
/// visits known config roots (`.axon`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.axon` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".axon", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.axon is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.axon/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.axon/ → kept (user content).
        let axon_shell = tmp.path().join(".axon").join("skills").join("shell");
        std::fs::create_dir_all(&axon_shell).unwrap();
        std::fs::write(
            axon_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (axon_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.axon/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .axon skill in an intermediate dir.
        ...
        .unwrap();
        let axon_skill = sub.join(".axon").join("skills").join("axon-dyn");
        std::fs::create_dir_all(&axon_skill).unwrap();
        std::fs::write(axon_skill.join("SKILL.md"), "---\nname: axon-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"axon-dyn"),
            "axon-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only axon-dyn discovered.
        ...
        assert!(
            names_off.contains(&"axon-dyn"),
            "axon-dyn missing: {names_off:?}"
        );
        assert!(
