...
register_resource!("axon_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, axon_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(axon_tool_runtime::ToolError::execution(
                        axon_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl axon_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = axon_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = axon_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
