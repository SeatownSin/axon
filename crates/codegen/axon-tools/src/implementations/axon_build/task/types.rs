...
//! (in `axon-tools`) and the subagent coordinator (in `axon-shell`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"axon-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`axon_tool_types::is_not_sentinel`] for existing call sites.
pub use axon_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `axon-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "AxonBuild:run_terminal_cmd" | "AxonBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `axon-shell` relies on
...
register_resource!("axon_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-axon-build
...
pub type MonitorEventBuffer = axon_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("axon_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// axon-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in axon-chat-state (via `RunningSubagentSummary`)
...
register_resource!("axon_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("axon_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("axon_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by axon-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by axon-shell at turn start.
...
register_resource!("axon_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("AxonBuild:run_terminal_cmd", ToolKind::Execute),
                tc("AxonBuild:read_file", ToolKind::Read),
                tc("AxonBuild:list_dir", ToolKind::List),
                tc("AxonBuild:grep", ToolKind::Search),
                tc("AxonBuild:kill_task", ToolKind::KillTaskAction),
                tc("AxonBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("AxonBuild:kill_task", ToolKind::KillTaskAction),
                tc("AxonBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("AxonBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("AxonBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
