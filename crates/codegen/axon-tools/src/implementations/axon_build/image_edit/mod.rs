//! `image_edit` tool — edits or transforms images via the Axon Imagine
...
use crate::implementations::axon_build::image_gen::{ImageGenClient, ImageGenResponse};
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), axon_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            axon_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
async fn resolve_to_data_url(value: &str) -> Result<String, axon_tool_runtime::ToolError> {
...
impl axon_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "image_edit");
