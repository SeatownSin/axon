...
impl axon_tool_runtime::Tool for WebFetchTool {
    type Args = WebFetchInput;
    type Output = WebFetchOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("web_fetch").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WebFetchInput,
    ) -> Result<WebFetchOutput, axon_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "web_fetch");
        assert_eq!(
