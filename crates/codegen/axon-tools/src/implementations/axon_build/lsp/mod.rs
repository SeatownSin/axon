...
//! `LspTool` (Tool trait impl) under the `AxonBuild` namespace.
...
impl axon_tool_runtime::ToolOutput for LspToolOutput {}
...
impl axon_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, axon_tool_runtime::ToolError> {
