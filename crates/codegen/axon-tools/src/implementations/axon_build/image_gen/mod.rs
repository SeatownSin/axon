//! `image_gen` tool — generates images via the Axon Imagine API and saves
...
const AXON_IMAGINE_MODEL: &str = "axon-imagine-image-quality";
// Some Imagine models (e.g. `axon-imagine-image`, selectable via `model_override`)
...
/// SuperAxon upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperAxon feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperAxon: https://blocked.invalid/superaxon?referrer=axon-build. Do not retry this tool.";

/// HTTP client for Axon Imagine API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    ...
    /// [`AXON_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    ...
    /// HTTP call and return the SuperAxon upsell prose instead. See
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| AXON_IMAGINE_MODEL.to_owned());
    ...
    /// with the SuperAxon upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`AXON_IMAGINE_MODEL`]). Driven by the remote
        ...
        /// SuperAxon upsell prose instead of a doomed request. Set by the
...
impl ImageGenConfig {
    ...
    /// default quality model ([`AXON_IMAGINE_MODEL`]) is used.
...
impl axon_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("axon-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("axon-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("axon-imagine-image")), None)
                .unwrap()
                .model,
            "axon-imagine-image"
        );
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperAxon upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperAxon"), "got: {}", t.text);
                assert!(t.text.contains("superaxon?referrer=axon-build"));
            }
