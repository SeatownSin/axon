...
//!
//! - **`axon-tools`** — tool blocks on a oneshot, formats the result.
//! - **`axon-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`axon-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `axon-tools`.
...
register_resource!("axon_build", "UserQuestionSender", UserQuestionSender);
