...
pub use axon_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl axon_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl axon_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, axon_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(axon_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
