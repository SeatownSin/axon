...
use crate::implementations::axon_build::task_output::{
...
use axon_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            axon_tool_types::build_wait_tasks_description(&axon_tool_types::WaitTasksToolNaming {
...
/// [`axon_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl axon_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::axon_build::task_output::capped_wait_timeout(input.timeout_ms);
