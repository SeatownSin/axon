...
impl axon_tool_runtime::Tool for WebSearchTool {
    type Args = WebSearchInput;
    type Output = WebSearchOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("web_search").expect("valid tool id")
    }
    ...
    ) -> Result<WebSearchOutput, axon_tool_runtime::ToolError> {
        ...
        let (content, citations) = client
            ...
            .map_err(|e| {
                axon_tool_runtime::ToolError::execution(
                    axon_tool_protocol::ToolId::new("web_search").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "web_search");
        assert!(
