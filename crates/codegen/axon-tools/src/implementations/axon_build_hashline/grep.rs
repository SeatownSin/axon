...
use crate::implementations::axon_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl axon_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = axon_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(axon_tool_runtime::Cwd(cwd));
        let mut result = axon_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                axon_tool_runtime::ToolError::execution(
                    axon_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
        assert_eq!(ToolMetadata::kind(&tool), ToolKind::Search);
        assert!(axon_tool_runtime::Tool::capabilities(&tool).is_read_only);
        assert!(matches!(
