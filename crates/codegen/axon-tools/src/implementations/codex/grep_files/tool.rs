...
use crate::implementations::axon_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the axon-build `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `AxonBuild` or `Codex` search,
...
impl axon_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, axon_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> axon_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = axon_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
