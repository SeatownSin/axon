...
use axon_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use axon_pager::client_identity::PAGER_CLIENT_VERSION;
use axon_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
...
/// Entrypoint tag for `axon -p`; keys the quiet stderr default in `init_tracing_simple`.
...
async fn run_setup_command(json: bool) {
    use axon_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `axon login`,");
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `axon setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = axon_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if let Err(e) = axon_shell::util::kill_process_by_pid(pid) {
...
) -> Result<(LeaderDescriptor, axon_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = axon_shell::leader::resolve_leader_target(target)
        .await
...
fn leader_info_json(
    ...
    info: Option<&axon_shell::leader::ControlPayload>,
...
/// Env override for the `axon workspace` gate: any truthy value enables the
...
/// Resolution of the `axon workspace` gate. `Unknown` is kept separate from
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&axon_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<axon_shell::util::config::RemoteSettings> {
    join_early_prefetch(axon_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`axon workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `axon workspace`. Check your \
             network connection (run `axon login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn workspace_control(
    ...
    let raw_config = axon_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use axon_shell::auth::ensure_authenticated;
    ...
    if !use_leader {
        anyhow::bail!(
            "`axon workspace` requires leader mode (the workspace is shared via the leader).\n\
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    axon_telemetry::sentry::flush_on_shutdown();
    axon_telemetry::otel_layer::shutdown_otel();
    axon_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<axon_pager::app::AgentArgs>,
    ...
    let _otel_guard = axon_telemetry::otel_layer::otel_guard();
    axon_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => axon_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = axon_shell::agent::models::start_early_prefetch(None);
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(axon_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = axon_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.resolve_runtime_fields(&axon_shell::agent::config::RuntimeResolutionContext {
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = axon_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = axon_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            axon_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(axon_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = axon_shell::util::config::load_config().await;
...
/// `axon dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `axon dashboard` does NOT force leader mode and
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<axon_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(axon_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<axon_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(axon_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    axon_shell::heap_profile::install(axon_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    axon_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    axon_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        axon_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        axon_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    if axon_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if !axon_crash_handler::install(axon_crash_handler::CrashHandlerConfig {
    ...
    let crashed = axon_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    axon_telemetry::debug_log::flush();
    if let Err(e) = result {
        axon_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(axon_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    ...
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return axon_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        axon_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        axon_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        axon_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
            ...
            Command::Inspect { json } => {
                ...
                axon_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return axon_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = axon_telemetry::otel_layer::otel_guard();
                return axon_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = axon_telemetry::otel_layer::otel_guard();
                let config = axon_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return axon_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return axon_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return axon_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return axon_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return axon_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return axon_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return axon_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                axon_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                axon_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = axon_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                axon_shell::auth::run_cli_logout(&config)?;
                axon_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = axon_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(axon_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == axon_pager::headless::OutputFormat::Plain {
                args.output_format = axon_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return axon_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            axon_pager::headless::HeadlessOptions {
    ...
    let result = axon_pager::app::run(args, bg_update_rx).await;
    axon_sandbox::flush();
    match result {
        Ok(true) => {
...
///
/// Prefers awaiting the parked waiter for the background `axon update` child
...
/// fall back to a fresh blocking `axon update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    ...
    if let Ok(root) = axon_shell::config::load_effective_config_disk_only()
        && let Some(ch) = axon_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
/// After a successful `axon update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in axon_shell::leader::discover_leaders().await {
        if d.classification != axon_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        match client
            ...
            .await
        {
            Ok(Ok(axon_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(axon_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            axon_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: axon_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        ...
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(axon_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(axon_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        axon_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    /// `axon dashboard` flags the startup hook without forcing leader mode —
    ...
    /// `axon dashboard --no-leader` is allowed — the dashboard does not
