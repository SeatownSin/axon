...
//! `axon_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `axon-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(
            ...
            Some(".axon/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("axon-3-fast"));
