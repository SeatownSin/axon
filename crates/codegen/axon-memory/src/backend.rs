...
use axon_tools::types::memory_backend::{MemoryBackend, MemorySearchResult};
...
pub struct EndpointScopedCredentials {
    ...
    auth_credentials: Option<Arc<dyn axon_auth::AuthCredentialProvider>>,
...
impl EndpointScopedCredentials {
    ...
    fn auth_credentials(&self) -> Option<&Arc<dyn axon_auth::AuthCredentialProvider>> {
        self.auth_credentials.as_ref()
    }

    fn api_key_provider(&self) -> Option<&axon_tools::types::SharedApiKeyProvider> {
...
pub struct MemoryBackendParams {
    ...
    pub embed_config: Option<axon_config_types::MemoryEmbeddingConfig>,
    ...
    pub search_config: axon_config_types::MemorySearchConfig,
...
async fn build_embedding_provider(
    config: Option<&axon_config_types::MemoryEmbeddingConfig>,
...
pub struct MemoryBackendImpl {
    ...
    embed_config: Option<axon_config_types::MemoryEmbeddingConfig>,
    ...
    search_config: axon_config_types::MemorySearchConfig,
...
impl MemoryBackendImpl {
    ...
    pub fn new(db_path: PathBuf, storage: MemoryStorage) -> Self {
        Self {
            ...
            embed_api_key: None,
            search_config: axon_config_types::MemorySearchConfig::default(),
            watcher: None,
            stale_claim_secs: 60,
    ...
    pub fn with_search_config(mut self, config: axon_config_types::MemorySearchConfig) -> Self {
    ...
    fn open_readonly(&self) -> Result<rusqlite::Connection, rusqlite::Error> {
        ...
        axon_sqlite_journal::JournalMode::for_db_path(&self.db_path).open_readonly(&self.db_path)
    }
...
impl MemoryBackendImpl {
    ...
    pub fn search_config_for_test(&self) -> &axon_config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        let mut index = super::index::MemoryIndex::open_or_create(
            &self.db_path,
            self.storage.clone(),
            axon_config_types::MemoryIndexConfig::default(),
            embed_dims,
        )
        ...
        if needs_release {
            ...
            if let Some((dirty_count, reindexed_count, sync_start)) = watcher_sync_stats {
                axon_telemetry::session_ctx::log_event(
                    axon_telemetry::memory_telemetry::MemoryWatcherSync {
        ...
        if results.is_empty() {
            axon_telemetry::session_ctx::log_event(
                axon_telemetry::memory_telemetry::MemorySearchEmpty {
                    session_id: self.session_id.clone(),
            ...
            axon_telemetry::session_ctx::log_event(
                axon_telemetry::memory_telemetry::MemorySearch {
                    session_id: self.session_id.clone(),
                    query_length: query.len(),
                    keyword_count,
...
mod factory_tests {
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: axon_config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: axon_config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
    ...
    async fn make_embedding_provider_uses_async_api_key_resolution() {
        ...
        let probe: axon_tools::types::SharedApiKeyProvider = Arc::new(AsyncProbe {
...
mod tests {
    ...
    struct PanicKey;
    impl axon_tools::types::ApiKeyProvider for PanicKey {
        fn current_api_key(&self) -> Option<String> {
    ...
    async fn test_trusted_endpoint_prefers_session_credential() {
        struct StubAuth;
        impl axon_auth::HttpAuth for StubAuth {
            fn apply(
                &self,
        ...
        #[async_trait::async_trait]
        impl axon_auth::AuthCredentialProvider for StubAuth {
            fn snapshot(&self) -> axon_auth::CredentialSnapshot {
                axon_auth::CredentialSnapshot::default()
            }
    ...
    async fn axon_embedding_endpoint_builds_no_provider() {
        ...
        let api_key: axon_tools::types::SharedApiKeyProvider = Arc::new(AnyKey);
        let config = axon_config_types::MemoryEmbeddingConfig {
            model: Some("test-embedding-model".to_string()),
            ..Default::default()
        };
    ...
    fn endpoint_scoped_credentials_trust_gate_and_url_match() {
        ...
        let key = || Arc::new(AnyKey) as axon_tools::types::SharedApiKeyProvider;
