...
//! `~/.axon/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
pub struct MemoryStorage {
    /// `~/.axon/memory/`
    global_dir: PathBuf,
    /// `~/.axon/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.axon/memory/`.
    ///
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see axon_sqlite_journal::JournalMode::open_readonly.
        axon_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.axon/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
