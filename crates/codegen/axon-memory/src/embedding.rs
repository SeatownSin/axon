...
pub(super) fn build_middleware_client(
    ...
    axon_http::with_auth_retry(axon_http::shared_client(), credentials)
}
...
fn build_static_middleware_client(
    ...
    let provider: std::sync::Arc<dyn axon_auth::AuthCredentialProvider> = std::sync::Arc::new(
        axon_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl axon_auth::HttpAuth for NoopHttpAuth {
