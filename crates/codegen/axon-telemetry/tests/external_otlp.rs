...
fn external_stream_end_to_end() {
    ...
    let mut cfg = axon_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "AXON_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = axon_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    axon_telemetry::external::init(Some(cfg));
    assert!(axon_telemetry::external::is_active());
    ...
    assert!(!axon_telemetry::is_enabled());
    axon_telemetry::log_event(axon_telemetry::events::SessionNew {
    ...
    axon_telemetry::log_event(axon_telemetry::events::ModelResponseReceived {
    ...
    axon_telemetry::external::shutdown();
    assert!(
