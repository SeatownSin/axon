...
//!
//! These structs were extracted from `axon-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `axon wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = axon_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: axon_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from axon-pager via log_event)
// ---------------------------------------------------------------------------
...
pub struct ClipboardCopy {
    ...
    /// An explicit `axon wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
