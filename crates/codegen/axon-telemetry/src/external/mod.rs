...
//!   headers.
//! - Default **off**: with `AXON_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &axon_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             AXON_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
