...
//! `AXON_EXTERNAL_OTEL=1` *and* at least one of `OTEL_METRICS_EXPORTER` /
...
mod tests {
    ...
    fn master_switch_alone_enables_nothing() {
        // RQ7: AXON_EXTERNAL_OTEL=1 without an explicit exporter is inert.
        assert!(
            ExternalOtelConfig::resolve_with(env(&[("AXON_EXTERNAL_OTEL", "1")]), None).is_none()
        );
    }
    ...
    fn file_config_layered_under_env() {
        ...
        let cfg =
            ExternalOtelConfig::resolve_with(env(&[("AXON_EXTERNAL_OTEL", "0")]), Some(&file));
        assert!(cfg.is_none());
    }
