...
/// This function caches the result in `$AXON_HOME/agent_id` so subsequent calls
...
/// Write `$AXON_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    axon_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
///
/// Enabled only when `AXON_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("AXON_TELEMETRY_SPECIAL_USER").as_deref(),
