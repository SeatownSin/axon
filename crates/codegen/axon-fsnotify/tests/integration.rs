...
use axon_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = axon_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = axon_fsnotify::stats();
    let b = axon_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let c = axon_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        axon_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = axon_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
///
/// ```bash
/// cargo test -p axon-fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = axon_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
