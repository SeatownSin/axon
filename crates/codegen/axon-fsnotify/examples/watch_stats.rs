...
//! startup latency, under either strategy (`AXON_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p axon-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! AXON_FSNOTIFY_PER_DIR=0 cargo run --release -p axon-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! AXON_FSNOTIFY_PER_DIR=1 cargo run --release -p axon-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "axon holds 55k inotify watches" report).
...
use axon_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("AXON_FSNOTIFY_PER_DIR").unwrap_or_default();
