// claude_import.rs
// Scans Claude settings and generates TOML patches for .axon/config.toml.
//
...
use axon_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use axon_workspace::permission::rules::parse_permission_rule;
use axon_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.axon/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.axon/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.axon/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.axon/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.axon/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.axon/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.axon/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.axon/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.axon/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".axon").join("config.toml");
        ...
        let hooks_dir = project_root.join(".axon").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .axon/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `axon-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.axon/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .axon/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_AXON_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = axon_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = axon_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.axon) don't interfere.
        let has_probe = |reg: &axon_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `AXON_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_AXON_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
