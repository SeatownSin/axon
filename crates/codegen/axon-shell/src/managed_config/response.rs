...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that AXON_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `axon login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.axon. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<axon_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<axon_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            axon_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[axon_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: axon_config::signed_policy::SignatureEnvelope,
    pub(super) payload: axon_config::signed_policy::SignedPayload,
}
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use axon_config::signed_policy::SignatureEnvelope;
