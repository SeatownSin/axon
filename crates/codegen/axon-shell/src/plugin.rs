...
use axon_agent::plugins::git_install::{self, UpdateStatus};
use axon_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use axon_plugin_marketplace::git::{self, SourceCacheLease};
use axon_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.axon/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &axon_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `axon plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `axon plugin install ./{name}`, or add a \
                     source with `axon plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `axon plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `axon plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    axon_config::load_effective_config_disk_only()
        .map(|c| axon_plugin_marketplace::load_require_sha(&c))
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &axon_workspace::permission::resolution::MarketplaceAllowlist,
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("axon plugin install ./sentry"), "{msg}");
        assert!(msg.contains("axon plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("axon plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> axon_workspace::permission::resolution::MarketplaceAllowlist {
        axon_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = axon_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
