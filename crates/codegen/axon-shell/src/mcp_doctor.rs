//! `axon mcp doctor` -- runtime health check for MCP servers.
...
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `axon login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use axon_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `axon mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `axon mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `axon mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `axon mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
