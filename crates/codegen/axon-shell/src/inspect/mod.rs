...
/// A single config layer entry for `axon inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = axon_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = axon_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = axon_agent::plugins::PluginRegistry::from_discovered(
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = axon_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        axon_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `axon_agent::prompt::agents_md::read_agents_config_with_paths`
...
fn list_hooks(
    ...
    discovered_plugins: &[axon_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = axon_tools::types::compat::CompatConfig::default();
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &axon_agent::plugins::PluginRegistry,
    skills_config: &axon_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = axon_agent::prompt::skills::list_skills_with_plugins(
...
fn list_agents(
    ...
    let agents = axon_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[axon_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                axon_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                axon_agent::plugins::PluginScope::Project => Scope::Project,
                axon_agent::plugins::PluginScope::User => Scope::User,
                axon_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: axon_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    axon_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = axon_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `axon mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
fn print_human(r: &InspectReport) {
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `axon mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use axon_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use axon_tools::implementations::skills::types::SkillScope;
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = axon_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
