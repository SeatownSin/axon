...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `AXON_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `AXON_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("AXON_WEB_FETCH_PROXY"))
...
/// from `axon_tools::registry::types::ToolsetConfig` which holds
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            axon_agent::config::AgentDefinition::plan(),
            axon_agent::config::AgentDefinition::explore(),
        ] {
