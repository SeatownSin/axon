...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(axon_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }
    ...
    fn make_auth_provider(
        ...
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(axon_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, axon_version::VERSION)
            .header(
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = axon_file_utils::trace_context::inject_trace_context_into_request(request);
