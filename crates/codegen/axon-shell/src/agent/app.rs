    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = axon_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`axon_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    axon_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    axon_telemetry::unified_log::set_version(axon_version::VERSION);
    ...
    // blocking stdin reads (see `axon_acp_lib::spawn_stdin_line_reader`): on
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    axon_telemetry::unified_log::set_version(axon_version::VERSION);
    // `axon agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `axon agent stdio` stays interactive.
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'axon login' to re-authenticate.")
            }
...
///
/// * `relay_on_demand == false` (default — explicit `axon agent leader`
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: axon_version::VERSION.to_string(),
    })
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `axon agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
