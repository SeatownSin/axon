...
/// Router for blocked.invalid/session/* and blocked.invalid/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "axon/session/info" => handle_session_info(agent, args).await,
        "axon/session/close" => handle_session_close(agent, args).await,
        "axon/session/list" => handle_session_list(agent, args).await,
        "axon/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("axon/session_summaries/") => {
...
/// `axon/sessions/list` — the FleetView roster. Returns every
...
/// `axon/sessions/changed` broadcast.
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "axon/session_summaries/session_list" => {
        ...
        "axon/session_summaries/workspace_list" => {
            tracing::debug!("axon/session_summaries/workspace_list is working");
        ...
        "axon/session_summaries/workspace_list_recent" => {
