...
//! (`axon/folder_trust/request`) that asks a GUI client (axon-desktop) to decide
...
//! `axon/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`axon_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.axon/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `axon/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `axon/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `axon/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("axon/folderTrust"))
    ...
    /// `axon/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = axon_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                axon_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("axon/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a axon_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a axon_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.axon/hooks`, `.cursor/hooks.json`),
