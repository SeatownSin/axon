...
mod hunk_tracking_mode {
    ...
    use axon_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<axon_sampling_types::conversation::ConversationItem> {
...
/// Anti-bypass guard: trace upload (per-turn request/response → GCS) is
/// removed in this build and pinned off at the config seam. Even with
/// telemetry fully `Enabled`, `trace_upload = true`, AND a bucket set, the
/// agent-side harness path must produce NO uploads and must NOT burn a turn
/// number — no configuration may re-enable egress. (The disabled-telemetry
/// path is covered separately by
/// `upload_harness_trace_turns_uploads_disabled_does_not_burn_counter`.)
#[tokio::test(flavor = "current_thread")]
async fn upload_harness_trace_upload_cannot_be_reenabled_by_config() {
    ...
    // Trace upload pinned off → no trace context → no per-turn uploads built.
    let built = agent
    ...
    assert!(
        built.is_empty(),
        "trace upload is removed; config must not re-enable it",
    );
    ...
    assert_eq!(
        ...
        Some(3),
        "no upload → the turn counter must not advance",
    );
    assert!(
        cmd_rx.try_recv().is_err(),
        "no upload → no counter persisted",
    );
}
...
/// The per-harness-turn manifest seam is reachable only through the removed
/// trace-upload path (`build_harness_trace_uploads` yields no contexts when
/// upload is pinned off), so there is nothing to build. This guards that the
/// seam stays inert even with telemetry + trace_upload + bucket all set.
#[tokio::test(flavor = "current_thread")]
async fn upload_harness_trace_manifest_seam_is_inert_when_upload_removed() {
    let agent = build_minimal_agent_for_tests();
    {
        let mut cfg = agent.cfg.borrow_mut();
        cfg.features.telemetry = Some(crate::agent::config::TelemetryMode::Enabled);
    ...
    assert!(
        built.is_empty(),
        "trace upload removed → no per-turn trace contexts to build manifests for",
    );
}
...
fn inherited_harness_template_skips_nonstrict_model() {
    use axon_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        axon_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    let builder = axon_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        system_reminder_tag: axon_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = axon_hunk_tracker::HunkTrackerActor::spawn(
        ...
        hunk_event_tx,
        axon_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: axon_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            axon_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(axon_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use axon_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<axon_acp_lib::AcpClientMessage>,
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), axon_chat_state::AuthType::ApiKey,);
}
...
/// Auth-diagnostics upload removed: the uploader must never be wired, even
...
async fn diagnostic_upload_never_wired_for_normal_user() {
    ...
    assert!(
        ...
        "diagnostics uploader must not exist — egress removed"
    );
}
#[tokio::test]
...
#[tokio::test]
async fn diagnostic_upload_never_wired_without_credentials() {
    let agent = build_minimal_agent_for_tests();
    enable_trace_upload_config(&agent);
    assert!(agent.diagnostic_upload_config().is_none());
}
/// Trace-upload removal: the live collection mirror must stay off even
...
async fn collection_config_gate_mirror_stays_off() {
    ...
    assert!(
        ...
        "mirror must stay off — trace upload egress removed"
    );
}
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = axon_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = axon_workspace::WorkspaceOps::for_test();
    let toolset =
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<axon_workspace::permission::PermissionEvent>();
    agent
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                axon_subagent_resolution::config::SubagentRole {
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, axon_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(axon_acp_lib::AcpClientMessage::ExtMethod(_)))),
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(axon_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> axon_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<axon_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = axon_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = axon_announcements::RemoteAnnouncement {
