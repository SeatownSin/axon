...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        axon_telemetry::unified_log::info("agent initialized", None, None);
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    axon_workspace::permission::cleanup_stale_permission_state(
        ...
        axon_workspace::trust::migrate_legacy_hook_trust();
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    serde_json::json!(
                        ...
                        axon_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::AXON_API_KEY_METHOD_ID => {
                ...
                log_event(axon_telemetry::events::Login {
    ...
    async fn new_session(
        ...
        axon_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || axon_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = axon_telemetry::context::collect_git_context(&cwd_str);
                let ev = axon_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                axon_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match axon_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        axon_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        axon_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        axon_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => axon_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            axon_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = axon_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : axon_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a axon worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            axon_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use axon_workspace::session::git::RestoreKind;
            let outcome = axon_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = axon_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    axon_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = axon_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = axon_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(axon_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            axon_file_utils::trace_context::link_current_span_to_meta(
        ...
        if let Some(ctx) = trace_context.clone() {
            ...
            let mut prompt_metadata = PromptMetadata {
                ...
                shell_version: Some(axon_version::VERSION.to_string()),
                workspace_type: None,
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: axon_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
                respond_to: tx,
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<axon_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = axon_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = axon_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = axon_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            axon_file_utils::trace_context::link_current_span_to_meta(meta);
        }
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "axon/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                axon_telemetry::session_ctx::log_event(axon_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "axon/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                axon_telemetry::session_ctx::log_event(axon_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| axon_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "axon/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                axon_telemetry::session_ctx::log_event(axon_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "axon/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                axon_telemetry::session_ctx::log_event(axon_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| axon_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == axon_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                axon_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            axon_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
