...
use axon_telemetry::id::{agent_id, agent_instance_id};
use axon_telemetry::session_ctx::log_event;
use axon_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use axon_workspace::permission::{ClientType, PermissionEvent};
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a axon_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[axon_announcements::RemoteAnnouncement]>,
    last_emitted: &[axon_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<axon_announcements::RemoteAnnouncement>> {
    let current = axon_announcements::filter_expired_at(
...
/// Override with `AXON_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AXON_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.axon/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<axon_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    pub(crate) plugin_registry_handle: axon_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<axon_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<axon_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &axon_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<axon_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !axon_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = axon_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use axon_agent::config::is_strict_harness_agent_type;
    match (
...
use axon_chat_state::conversation_util::replace_or_insert_system_head;
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<axon_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(axon_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<axon_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<axon_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<axon_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = axon_tools::types::TaskSnapshot {
    ...
    ///    against `~/.axon/bundled/` and the manifest.
