...
//! inside a cloned repository (`.mcp.json`, project `.axon/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.axon/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`axon_workspace::trust::TrustStore`]
//! reads/writes — lives in `axon-workspace` (client-side); this module keeps
...
//! on [`axon_workspace::folder_trust::decide`]; the consume-side nuance is
...
// `axon_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use axon_workspace::folder_trust::grant_folder_trust;
...
// NOTE: this folder-trust store (`~/.axon/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`axon_agent::plugins::TrustStore` at `~/.axon/trusted-plugins`, plus the
...
/// [`axon_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = axon_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`axon_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.axon/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    axon_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: axon_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != axon_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `axon mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `axon --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `axon mcp doctor`). It MUST
...
///
/// Sources: project `.axon/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.axon/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.axon/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            axon_tools::implementations::lsp::config::LspServerConfig,
...
mod tests {
    ...
    use axon_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(axon_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; AXON_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because AXON_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. AXON_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because AXON_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. AXON_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because AXON_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; AXON_HOME-isolated store; AXON_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".axon").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. AXON_HOME-isolated so the trust store is empty;
        // AXON_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = axon_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. AXON_HOME-isolated so the trust store is empty;
        // AXON_FOLDER_TRUST unset so the default-on feature flag applies.
        use axon_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). AXON_HOME-isolated so the trust store is empty.
        use axon_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. AXON_HOME-isolated (empty store).
        use axon_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".axon").join("agents");
        ...
        .unwrap();
        let def = axon_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.axon/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). AXON_HOME-isolated (empty
        // store); AXON_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".axon").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + AXON_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. AXON_HOME-isolated so the seeded store is the temp one;
        // AXON_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // AXON_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(axon_version::TEST_VERSION_ENV);
        if option_env!("AXON_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.axon/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. AXON_HOME-isolated (empty store);
        // AXON_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".axon").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // AXON_HOME-isolated (empty store); AXON_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".axon").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // AXON_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); AXON_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use axon_agent::plugins::discovery::DiscoveryConfig;
        use axon_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".axon").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // AXON_HOME-isolated so the grant writes to a temp store; AXON_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".axon").join("hooks");
        ...
        let has_project_hook = |reg: &axon_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = axon_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.axon/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.axon/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use axon_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use axon_tools::types::config_source::ConfigSource;

        // A `<cwd>/.axon/lsp.json` server must be tagged `Project` so the gate
    ...
    /// (`.mcp.json`) and `projtoml` (`.axon/config.toml [mcp_servers]`).
    ...
    /// `.axon/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `axon --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(axon_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("AXON_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no AXON_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(axon_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(axon_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("AXON_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // AXON_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("AXON_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".axon").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".axon").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with AXON_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. AXON_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(axon_version::TEST_VERSION_ENV);
        if option_env!("AXON_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. AXON_HOME-isolated so the store starts
        // empty; `#[serial]` because AXON_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // AXON_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(axon_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; AXON_HOME-isolated and
        // AXON_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("AXON_HOME", home.path());
        let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".axon").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".axon").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.axon/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".axon")).unwrap();
        std::fs::write(tmp.path().join(".axon").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
