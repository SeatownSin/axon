...
/// This stays in axon-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                axon_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: axon_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != axon_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == axon_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = axon_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &axon_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("axon-test"), || {
            ...
            assert!(
                msg.contains("axon logout"),
                "should suggest axon logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
