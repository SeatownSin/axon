...
//! attempt (or an explicit `axon/auth/cancel`) cancels the previous one, so
...
//! scopes explicit cancels so a delayed `axon/auth/cancel` cannot tear down a
//! successor login.
...
pub(crate) struct AttemptChannels {
    /// Forwards pasted codes from `axon/auth/submit_code` to the flow.
    code_tx: tokio::sync::mpsc::Sender<String>,
    /// Yields the auth URL to `axon/auth/get_url`. `Option` so
