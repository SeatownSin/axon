...
) -> axon_memory::EndpointScopedCredentials {
...
pub fn build_storage_client_for_proxy(
...
) -> axon_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn axon_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        axon_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(axon_version::VERSION, client_identifier)
...
/// Bridge that lets `StorageClient` (which lives in axon-file-utils)
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        axon_telemetry::external::set_identity(
            axon_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
mod tests {
    ...
    /// RAII guard: pins `AXON_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("AXON_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("AXON_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("AXON_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("AXON_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use axon_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
