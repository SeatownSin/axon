...
//!
//! 1. [`axon_telemetry::unified_log::warn`] for the local
//!    `~/.axon/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`axon_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`axon_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`axon_sampler::SamplerConfig::attribution_callback`] at every
...
use axon_sampler::{Auth401AttributionCallback, SamplingConsumer};
use axon_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`axon_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `axon_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn axon_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `axon_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `axon-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `axon-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.axon/logs/unified.jsonl) + scrubbed
    ...
    axon_telemetry::unified_log::warn("auth 401 attribution", session_id, Some(payload.clone()));
...
/// and the mint/expiry fields from the resulting `AxonAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `AxonAuth` (or
    ...
    // `axon_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.axon/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> AxonAuth {
        AxonAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `axon_tools::Auth401AttributionCallback`
