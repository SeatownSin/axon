...
pub struct AuthManager {
    ...
    power_listener: parking_lot::Mutex<Option<axon_system_power::SystemPowerListener>>,
...
impl AuthManager {
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: AxonAuth) -> std::io::Result<AxonAuth> {
        ...
        match &write_result {
            Ok(()) => axon_telemetry::unified_log::info(
            ...
            Err(e) => axon_telemetry::unified_log::error(
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<AxonAuth, AuthError> {
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`axon login` from
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<AxonAuth, AuthError> {
        ...
        axon_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `axon_system_power::PowerState` for the canonical
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<axon_telemetry::events::ManualAuth> {
...
impl axon_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
) -> axon_tools::types::SharedApiKeyProvider {
