...
pub struct SessionHandle {
    ...
    pub chat_state_handle: axon_chat_state::ChatStateHandle,
    ...
    /// `axon/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `axon/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `axon/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: axon_workspace::permission::PermissionHandle,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> axon_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            axon_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<axon_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<axon_hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<axon_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
