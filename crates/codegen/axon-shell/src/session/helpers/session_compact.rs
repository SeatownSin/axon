...
///
/// All other agents (axon-build, etc.) continue to use the detailed
...
pub(crate) use axon_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_axon_conv_id = Some(sid.clone());
            message.x_axon_req_id = Some(format!("axon-compact-{}", uuid::Uuid::new_v4()));
            message.x_axon_session_id = Some(sid);
            message.x_axon_agent_id = Some(axon_telemetry::id::agent_id());
            tracing::info!(
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_axon_conv_id: Some(session_id.to_string()),
                x_axon_req_id: Some(format!("axon-compact-{}", uuid::Uuid::new_v4())),
                x_axon_session_id: Some(session_id.to_string()),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            axon_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            axon_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: axon_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            axon_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, axon_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        axon_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            axon_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            axon_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            axon_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    /// Regression guard: axon-build must DROP the working
    /// tail post-compaction. A prior change routed axon-build to keep `recent_messages`,
    /// which survive only as `Tool call omitted...` stubs (dead tokens). Mirrors
    /// `summary_before_recent_compaction_with_no_user_query_yields_three_messages` for axon-build
    /// (`summary_before_recent = false`).
    #[tokio::test]
    async fn axon_build_compaction_drops_working_tail_regression_206460() {
        let conversation = vec![
            ConversationItem::system("You are a helpful assistant."),
            ConversationItem::user(
                "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
        ...
        assert!(
            ...
            "axon-build must drop recent_messages post-compaction",
        );
        let compacted = build_compacted_history(
            "You are a helpful assistant.",
            "<user_info>\nOS: macos\n</user_info>",
        ...
        assert!(
            !compacted
                ...
                .any(|i| matches!(i, ConversationItem::ToolResult(_))
                    || i.text_content() == "Tool call omitted..."),
            "no tail (ToolResult or stub) may leak into the axon-build compacted history",
        );
    }
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use axon_chat_state::compaction_utils::validate_compacted_history;
