...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input = axon_agent_lifecycle::TurnStartInput::new(
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = axon_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        axon_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        axon_telemetry::session_ctx::log_event(
                            axon_telemetry::events::PluginUsed {
        ...
        self.observability_bridge
            .emit(
                axon_tool_protocol::session_event::SessionEvent::TurnStarted {
                    turn_number,
                    model_id: model_id.clone(),
                    yolo_mode,
                },
            ...
            .await;
        self.send_before_turn_event(axon_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: axon_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        axon_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        axon_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = axon_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || axon_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = axon_telemetry::events::PromptSubmitted {
            ...
            axon_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(axon_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            axon_hooks::event::HookEventName::UserPromptSubmit,
            axon_hooks::event::HookPayload::UserPromptSubmit {
        ...
        self.observability_bridge
            .emit(axon_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                self.send_after_turn_event(axon_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: axon_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(axon_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: axon_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(axon_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: axon_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&axon_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = axon_agent_lifecycle::TurnAbortInput::new(
                    axon_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = axon_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let Some(body) = axon_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = axon_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use axon_tools::types::memory_backend::MemoryBackend as _;
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<axon_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                axon_telemetry::session_ctx::log_event(axon_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            self.observability_bridge
                .emit(
                    axon_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: axon_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            axon_telemetry::unified_log::info(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                axon_telemetry::session_ctx::log_event(
                    axon_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            for item in response.items {
                match item {
                    axon_sampling_types::ConversationItem::Assistant(_) => {
            ...
            self.observability_bridge
                .emit(
                    axon_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: axon_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
