...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => axon_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use axon_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
///
/// Uses `axon_acp_lib`'s TYPED [`AcpChannelFailure`](axon_acp_lib::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        axon_acp_lib::acp_channel_failure(err),
        Some(axon_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        axon_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    axon_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(axon_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => axon_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && axon_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(axon_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                        ...
                        .hook_event_active(axon_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        ...
                        let (tool_input_value, tool_input_truncated) =
                            axon_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            axon_hooks::event::HookEventName::PostToolUseFailure,
                            axon_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    axon_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    axon_hooks::event::HookEventName::PostToolUse,
                    axon_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                .emit(
                    axon_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if axon_telemetry::external::is_active() {
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(axon_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                axon_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                axon_hooks::event::HookEventName::PreToolUse,
                None,
                axon_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                .await;
                if let axon_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                axon_workspace::permission::AccessKind::Read(p) => (
                    axon_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                axon_workspace::permission::AccessKind::Edit(p) => {
                    (axon_telemetry::events::AccessKind::Edit, p.clone())
                }
                axon_workspace::permission::AccessKind::Bash(cmd) => {
                    (axon_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                axon_workspace::permission::AccessKind::Grep { path, glob } => (
                    axon_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                axon_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (axon_telemetry::events::AccessKind::Mcp, name.clone())
                }
                axon_workspace::permission::AccessKind::WebFetch(u) => {
                    (axon_telemetry::events::AccessKind::Web, u.clone())
                }
                axon_workspace::permission::AccessKind::WebSearch(q) => {
                    (axon_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            let edit_path_context = matches!(&access_kind, AccessKind::Edit(_)).then(|| {
                axon_workspace::permission::types::EditPathContext {
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        axon_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        axon_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        axon_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        axon_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (axon_telemetry::events::PermissionOutcome::Allow, None)
                }
            ...
            .in_scope(|| {});
            axon_telemetry::session_ctx::log_event(
                axon_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        axon_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        axon_hooks::event::HookEventName::PermissionDenied,
                        axon_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                axon_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        axon_tool_types::WaitMode::WaitAny => "wait_any",
                        axon_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
    ...
    async fn handle_tool_parse_error(
        ...
        call_id: &str,
        function_name: &str,
        err: axon_tool_runtime::ToolError,
        raw_arguments: &str,
        model_id: &str,
    ) -> Result<(), acp::Error> {
        tracing::error!(
    ...
    /// `axon_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use axon_telemetry::enums::PrCreationSource;
        use axon_tools::util::git_detect;
        match &result.output {
            axon_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
    ...
    fn record_pr_created(
        &self,
        pr: axon_tools::util::git_detect::PrRef,
        source: axon_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            axon_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                axon_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let axon_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                axon_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | axon_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            axon_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            axon_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`axon_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: axon_sampler::SamplingEvent,
    ) {
        use axon_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == axon_sampler::SamplingErrorKind::DoomLoopDetected {
...
fn execute_tool_call_parts(
    ...
    let display = axon_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use axon_workspace::permission::AccessKind;
    ...
    fn write(path: &str) -> ToolInput {
        use axon_tools::implementations::opencode::write::WriteInput;
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use axon_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&axon_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use axon_interjection_core::PendingInterjection;
