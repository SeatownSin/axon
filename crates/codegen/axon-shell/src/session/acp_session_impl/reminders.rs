...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> axon_agent::ReminderPolicy {
    let mut policy = axon_agent::ReminderPolicy::default();
...
/// Wrapped in axon's `<system-reminder>` shape by [`SessionActor::push_system_reminder`].
...
pub(super) fn todo_gate_active(
    policy: &axon_agent::system_reminder::ReminderPolicy,
    audience: axon_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        let reminder =
            axon_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(axon_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
