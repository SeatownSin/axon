...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
        ...
        self.chat_state_handle
            .update_sampling_config(axon_sampling_types::SamplingConfig {
        ...
        self.chat_state_handle
            .update_credentials(axon_chat_state::Credentials {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            axon_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    ///
    /// Builds a fresh [`axon_agent::Agent`] from the cached
    /// [`crate::session::agent_rebuild::AgentRebuildSpec`] + the supplied
    /// [`axon_agent::AgentDefinition`], replaces `self.agent`,
