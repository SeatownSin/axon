...
fn partition_rules_by_scope(
    ...
    Vec<axon_agent::prompt::user_message::RuleEntry>,
    Vec<axon_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = axon_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    fn paths(entries: &[axon_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(axon_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: axon_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use axon_agent::prompt::agents_md::read_agents_config_with_paths;
        use axon_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use axon_workspace::file_system::{git_status_short, jj_status};
        use axon_workspace::session::git::VcsKind;
    ...
    ) -> Vec<axon_agent::prompt::user_message::McpServerEntry> {
        use axon_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &axon_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = axon_sampler::SamplingClient::new(sampler_config).map_err(|e| {
