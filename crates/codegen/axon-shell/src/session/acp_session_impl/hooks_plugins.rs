...
impl SessionActor {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<axon_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(axon_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(axon_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use axon_hooks_plugins_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::Add { path } => {
                ...
                // CWE-427: add_hooks_path() validates path is under ~/.axon/.
            ...
            HooksAction::Disable { hook_name } => {
                match axon_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        axon_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        axon_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use axon_hooks_plugins_types::{ActionOutcome, OutcomeStatus, PluginsAction};
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                match axon_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                    crate::plugin::marketplace_require_sha(),
                ) {
                    Ok(result) => {
                        let repo = axon_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = axon_agent::plugins::InstallRegistry::load();
                match registry.find_plugin(plugin_name) {
                    ...
                    Some((repo_key, repo, _plugin)) => {
                        ...
                        if let Err(e) =
                            axon_agent::plugins::git_install::remove_repo_path(&repo_path)
                        {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                for (key, repo) in &repos_to_update {
                    match axon_agent::plugins::git_install::update_repo(
                        key,
                        repo,
                        crate::plugin::marketplace_require_sha(),
                    ) {
                        Ok(status) => {
                            use axon_agent::plugins::git_install::UpdateStatus;
                            match status {
        ...
        if let Some(ref pr) = plugin_registry_snapshot {
            for plugin in pr.active_plugins() {
                ...
                if let Some(ref inline_value) = plugin.inline_hooks {
                    let (specs, warnings) =
                        axon_agent::plugins::hooks_adapter::parse_plugin_hooks_from_value(
                            inline_value,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &axon_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        axon_telemetry::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<axon_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<axon_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(axon_mcp::servers::McpClientEvent::ConfigDiff {
