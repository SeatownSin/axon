...
fn drop_cli_catchall_allows(
    rules: Vec<axon_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<axon_workspace::permission::types::PermissionRule>,
    Vec<axon_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if axon_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use axon_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use axon_workspace::permission::rules::parse_permission_rule;
    use axon_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: axon_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<axon_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: axon_chat_state::CompactionMode,
    ...
    preloaded_skills: Option<Vec<axon_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    ...
    prompt_audience: axon_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<axon_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<axon_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<axon_workspace::permission::PermissionHandle>,
    api_key_provider: Option<axon_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<axon_hooks::discovery::HookRegistry>>,
    workspace_ops: axon_workspace::WorkspaceOps,
    cli_permission_rules: Vec<axon_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    axon_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(axon_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            axon_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = axon_workspace::permission::resolution::yolo_disabled_by_policy();
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(axon_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if axon_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    axon_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
        ...
        let (permissions, permission_events_rx) =
            axon_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let context_window_override = std::env::var("AXON_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "AXON_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = axon_sampling_types::SamplingConfig {
        ...
        chat_template_kwargs: sampling_config.chat_template_kwargs.clone(),
    };
    let actor_pruning_config = axon_chat_state::PruningConfig {
    ...
    let chat_state_handle = axon_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let task_completion_reservations =
        axon_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        axon_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        axon_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        axon_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn axon_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn axon_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn axon_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(axon_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(axon_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || axon_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                axon_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                axon_telemetry::enums::PermissionMode::Auto
            } else {
    ...
    let compaction_policy = axon_agent::CompactionPolicy {
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn axon_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : axon_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : axon_telemetry::memory_log::TARGET, removed,
        ...
        axon_telemetry::session_ctx::log_event(
            axon_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        axon_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        axon_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    ...
    let resolved_workspace_root = axon_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = axon_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<axon_sampler::SamplingEvent>();
    let sampler_handle = axon_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<axon_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<axon_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = axon_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let obs_bridge = {
        let sid = axon_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| axon_tool_protocol::SessionId::new("unknown").expect("valid"));
        axon_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: axon_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match axon_workspace::session::git::discover_git_root(root) {
                axon_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    axon_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => axon_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(axon_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(axon_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    let telemetry_ctx = axon_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            axon_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        axon_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = axon_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
