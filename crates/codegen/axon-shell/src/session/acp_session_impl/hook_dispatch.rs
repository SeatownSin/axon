...
) -> axon_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> axon_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use axon_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> axon_hooks::runner::RunContext<'_> {
        axon_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[axon_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, axon_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use axon_hooks::result::HookRunResult;
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: axon_hooks::event::HookEventName,
        payload: axon_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                axon_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    axon_telemetry::events::HookOutcome::Success,
                ),
                axon_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    axon_telemetry::events::HookOutcome::Error,
                ),
                axon_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            axon_telemetry::session_ctx::log_event(axon_telemetry::events::HookExecuted {
