...
impl SessionActor {
    ...
    pub(super) async fn handle_session_mode(&self, session_mode_id: acp::SessionModeId) {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_plan() {
            ...
            axon_telemetry::session_ctx::log_event(axon_telemetry::events::PlanModeToggled {
                enabled: true,
                trigger: axon_telemetry::events::PlanModeTrigger::User,
                turn_in_flight,
    ...
    /// first-entry/reentry distinction), or axon's reentry/full variant.
