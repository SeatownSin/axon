        ...
        match action {
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        axon_telemetry::session_ctx::log_event(
                            axon_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        axon_telemetry::session_ctx::log_event(
                            axon_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.axon/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.axon/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::HookAdded { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        axon_telemetry::session_ctx::log_event(
                            axon_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_slash_command_output(&msg).await;
                    }
                    None => {
                        axon_telemetry::session_ctx::log_event(
                            axon_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = axon_token_estimation::usage_percentage(ctx.used, ctx.total);
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::PluginAdded {
                                    source: axon_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
                    ...
                    if !trust {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                axon_telemetry::session_ctx::log_event(
                                    axon_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn axon_tools::types::memory_backend::MemoryBackend,
