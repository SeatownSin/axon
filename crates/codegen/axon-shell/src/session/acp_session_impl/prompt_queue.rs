...
impl SessionActor {
    ...
    pub(super) fn broadcast_queue_changed(&self, state: &State) {
        ...
        tracing::debug!(
            ...
            session = self.session_info.id.0.as_ref(),
            "broadcasting blocked.invalid/queue/changed to subscribers",
        );
    ...
    ///
    /// Always re-broadcasts `axon/queue/changed` so every client reconciles
    ...
    /// 3. Re-broadcast `axon/queue/changed` so every subscriber renders the
