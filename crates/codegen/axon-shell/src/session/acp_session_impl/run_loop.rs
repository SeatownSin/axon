...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<axon_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<axon_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            } else { tracing::debug!(target : axon_telemetry::memory_log::TARGET,
            ...
            axon_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(axon_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(axon_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            axon_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(axon_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(axon_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            axon_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            ...
            axon_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            axon_telemetry::session_ctx::log_event(axon_telemetry::events::SessionEnded
            ...
            false; axon_telemetry::unified_log::info("shell.task_wake.gate_cleared",
            ...
            axon_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(axon_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(axon_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            axon_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(axon_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(axon_file_utils::events::Event::McpServerToggled { server_name :
            ...
            session.fire_hook(axon_hooks::event::HookEventName::SessionStart, None,
            axon_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            axon_hooks::dispatcher::dispatch_non_blocking(& registry,
            axon_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[axon_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = axon_chat_state::compaction_utils::extract_user_query(&raw);
