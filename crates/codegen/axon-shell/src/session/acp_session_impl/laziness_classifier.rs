...
/// Prefix on `x_axon_req_id` for laziness-classifier sampler calls.
...
pub(crate) const LAZINESS_REQ_ID_PREFIX: &str = "axon-laziness-";
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let axon_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = axon_sampling_types::reasoning_item_text(r);
...
const CLASSIFIER_TURN_MAX_LEN: usize = axon_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<axon_workspace::permission::ClassifierTurn> {
    use axon_workspace::permission::ClassifierTurn;
