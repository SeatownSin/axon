...
pub use axon_telemetry::enums::PrCreationSource;
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &axon_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: axon_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    axon_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("axon-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("axon-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["axon-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("axon-4");
        handle.record_model_usage("axon-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"axon-3".to_string()));
        assert!(snapshot.models_used.contains(&"axon-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["axon-3".to_string(), "axon-4".to_string()],
        );
        ...
        handle.record_model_usage("axon-3"); // existing model
        handle.record_model_usage("axon-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // axon-3 not duplicated, axon-5 added
        assert!(snapshot.models_used.contains(&"axon-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("axon-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("axon-4");
        ...
        assert!(restored.models_used.contains(&"axon-3".to_string()));
        assert!(restored.models_used.contains(&"axon-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for axon-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the axon-critique bug describes: the
        ...
        handle2.record_model_usage("axon-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // axon-3 not duplicated
