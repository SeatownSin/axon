...
/// scratch root (`<temp_dir>/axon-goal-<id>`) and the verifier
...
/// Private per-goal scratch root: `<temp_dir>/axon-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("axon-goal-{verifier_id}"))
}
...
mod tests {
    ...
    /// `axon-goal-<12 hex chars>` scratch-root format (and the
    ...
    fn serde_skip_fields_reset_on_deserialize() {
        ...
        t.update_live_progress(
            100,
            vec![("axon-4".to_owned(), 60), ("axon-3".to_owned(), 40)],
            200_000,
            50,
            3,
            10,
    ...
    /// `temp_dir()` + `verifier_id`: a `axon-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("axon-goal-vid123"));
        assert_eq!(
            implementer_scratch_dir("vid123"),
            std::env::temp_dir()
                .join("axon-goal-vid123")
                .join("implementer"),
        );
        assert_eq!(
