...
/// attribution.
fn auth_error() -> axon_sampler::SamplingErrorInfo {
    axon_sampler::SamplingErrorInfo {
        kind: axon_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: axon_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `axon logout`, got: {msg}"
            );
            assert!(
                msg.contains("axon login"),
                "hint must mention `axon login`, got: {msg}"
            );
            assert!(
