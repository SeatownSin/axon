...
fn test_json_round_trip() {
    let ctx = axon_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: axon_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `axon_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = axon_agent::PromptContext::default();
    ...
    let loaded: axon_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a Axon Build subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: axon_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a Axon Build subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<axon_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/axon-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
fn build_truncated_preserves_small_query_truncates_context() {
    ...
    assert!(
        ...
        "axon ordering: query block first"
    );
...
//
// `axon_home()` is a process-wide `OnceLock`, so the real async method is
