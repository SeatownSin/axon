...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `axon/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let axon_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "axon/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an blocked.invalid/session/interjection broadcast");
                assert_eq!(
