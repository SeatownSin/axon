...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<axon_tools::types::resources::State<
                    axon_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = axon_tools::reminders::TaskCompletionReminder;
            let reminders = axon_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<axon_tools::types::resources::State<
                        axon_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = axon_tools::reminders::task_completion::format_monitor_completion(
                &axon_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: axon_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use axon_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<axon_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl axon_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: axon_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        axon_tools::computer::types::TerminalRunResult,
        axon_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<axon_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> axon_tools::computer::types::KillOutcome {
        axon_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<axon_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<axon_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> axon_tools::computer::types::TaskSnapshot {
    axon_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
