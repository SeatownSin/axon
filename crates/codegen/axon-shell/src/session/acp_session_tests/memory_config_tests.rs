...
async fn create_injection_ready_actor(
    initial_conversation: Vec<axon_sampling_types::ConversationItem>,
) -> SessionActor {
    ...
    local
        .run_until(async {
            let actor = create_injection_ready_actor(vec![
                axon_sampling_types::ConversationItem::system("You are a helpful assistant."),
                axon_sampling_types::ConversationItem::user(
            ...
            assert!(
                reminder.contains(axon_chat_state::MEMORY_CONTEXT_OPEN_TAG),
        ...
        .run_until(async {
            let persisted_block =
                crate::session::helpers::memory_context::format_memory_reminder(&[
                    axon_tools::types::memory_backend::MemorySearchResult {
                ...
                .unwrap();
            let actor = create_injection_ready_actor(vec![
                axon_sampling_types::ConversationItem::system(format!(
