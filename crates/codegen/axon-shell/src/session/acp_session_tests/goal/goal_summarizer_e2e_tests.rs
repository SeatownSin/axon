...
//!
//! Tests mutate `AXON_GOAL_CLASSIFIER` so they carry `serial`.
...
async fn answer_summarizer(
    ...
    req: Box<axon_tools::implementations::axon_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        axon_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
