...
//!
//! Tests that mutate `AXON_GOAL_CLASSIFIER` carry the
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(axon_tools::implementations::axon_build::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
async fn goal_classifier_env_override_disables_when_remote_enabled() {
    ...
    assert!(
        !resolved.value,
        "AXON_GOAL_CLASSIFIER=0 must disable even with remote=true",
    );
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::axon_build::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::axon_build::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::axon_build::update_goal::UpdateGoalAck::Accepted
                { .. },)
            );
        })
        .await;
...
fn tool_error_code(err: &axon_tool_runtime::ToolError) -> &str {
    err.details
        .as_ref()
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                axon_tools::implementations::axon_build::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
...
/// `axon-build-plan` (non-strict), and `opencode` (non-strict + unrepresentable
...
async fn resolve_role_override_harness_flavor_representability_gate() {
    ...
    local
        .run_until(async {
            ...
            let honored = ["axon-build-plan", "opencode"];
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                axon_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                axon_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
