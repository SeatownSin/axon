...
//!
//! These tests verify the parallel dispatch path (AXON_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and axon_build's
...
#[test]
fn lock_path_for_args_matches_axon_build_file_path() {
    // axon_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_axon_build_target_file() {
    // axon_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_axon_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. axon_build search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&axon), lock_path_for_args(&compat));
}
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use axon_sampling_types::{ConversationItem, SyntheticReason};
