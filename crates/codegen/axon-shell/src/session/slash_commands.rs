...
use axon_tools::implementations::skills::skill::format_skill_name;
...
// ── blocked.invalid/commands/list ext method ────────────────────────────────
...
///   matching today's Axon Build flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use axon_tools::implementations::skills::skill::{
...
/// `axon-tools` so it stays identical to the pager's `LoopCommand` and the
