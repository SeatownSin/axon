...
//! `axon-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use axon_file_utils::events::tracker::EventTracker;
pub(crate) use axon_file_utils::events::types::{
...
) -> Option<axon_sampling_types::PriorTurnInterrupt> {
    use axon_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run axon-build flavor. Distinct from
...
/// (`axon-file-utils`). The two enums are intentional mirrors and live in
