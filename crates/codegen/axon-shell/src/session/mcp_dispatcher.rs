...
pub enum McpServerStatus {
    /// Client is in [`axon_mcp::servers::ClientStateKind::Ready`]
...
/// [`axon_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && axon_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    fn discard_gateway() -> axon_acp_lib::AcpAgentGatewaySender {
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = axon_acp_lib::AcpAgentGatewaySender::new(gw_tx);
