...
//!
//! Hooks registered at `session/new` (`_meta["axon/hooks"]`) come in two flavors,
//! both matched by the agent ([`axon_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `axon/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `axon/hooks/event`,
...
use axon_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use axon_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "axon/hooks/event";
const HOOK_RUN_METHOD: &str = "axon/hooks/run";

/// Default per-callback bound for a client's `axon/hooks/run` reply; on timeout the gate
...
/// Outcome of the `axon/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "axon/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed blocked.invalid/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "axon/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("axon/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        axon_telemetry::session_ctx::log_event(axon_telemetry::events::HookBlocked {
    ...
    /// `axon/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = axon_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    axon_telemetry::session_ctx::log_event(
                        axon_telemetry::events::ClientHookGate {
    ...
    /// Issue one `axon/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `axon/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    axon_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
