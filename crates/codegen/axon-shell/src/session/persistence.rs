...
/// A feedback entry persisted to `~/.axon/sessions/.../feedback.jsonl`.
///
...
pub use axon_shared::session::session_dir;
...
/// `axon -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `axon -r <remote_id>` before this fix was deployed.
...
/// Searches across ALL cwd directories under `~/.axon/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.axon/sessions/`.
///
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            axon_workspace::session::git::resolve_persisted_session_git_metadata_sync(
        ...
        StorageMode::Writeback => {
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'axon login' first.",
                )
            })?;
...
/// worktree containing this session's cwd so `axon worktree gc` expires by
...
/// Walk `~/.axon/sessions/` and delete files with mtime older than `ttl_days`.
...
mod find_local_child_tests {
    ...
    /// Regression: a second `axon -r <remote_id>` must return the existing child
