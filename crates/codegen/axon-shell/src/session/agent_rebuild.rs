...
//! [`axon_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`axon_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`axon_agent::Agent`] owns an [`axon_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`axon_agent::AgentBuilder`]`::with_*` KNOB
//!
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<axon_tools::SharedAttributionCallback>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        agent
            .tool_bridge()
            .update_resource(axon_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(axon_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            axon_tools::computer::local::LocalTerminalBackend::new_local(
                axon_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: axon_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
