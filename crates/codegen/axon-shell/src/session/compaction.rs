...
fn prefire_lead_percent() -> u64 {
    std::env::var("AXON_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    use axon_sampling_types::ConversationItem;
    #[test]
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("AXON_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        axon_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("AXON_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG AXON_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(axon_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`axon-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                axon_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    axon_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    axon_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if axon_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: axon_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            axon_telemetry::events::CompactionTrigger::Manual => "manual",
            axon_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = axon_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            axon_hooks::event::HookEventName::PreCompact,
            axon_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            axon_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            axon_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            axon_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<axon_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            axon_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<axon_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
        ...
        let estimated_input_tokens =
            axon_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, axon_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = axon_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match axon_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(axon_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(axon_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(axon_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            axon_telemetry::session_ctx::log_event(
                                axon_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = axon_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    axon_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                axon_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                axon_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn axon_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn axon_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : axon_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        axon_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            axon_hooks::event::HookEventName::PostCompact,
            axon_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if axon_token_estimation::exceeds_threshold(
            ...
            let percentage = axon_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &axon_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = axon_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        axon_telemetry::session_ctx::log_event(axon_telemetry::events::AutoCompactFired {
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<axon_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<axon_acp_lib::AcpClientMessage>,
        ...
        let hunk_tracker_handle = axon_hunk_tracker::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            axon_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = axon_chat_state::ChatStateActor::spawn(
            vec![],
            axon_sampling_types::SamplingConfig {
            ...
            Box::new(axon_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: axon_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: axon_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: axon_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: axon_sampler::SamplerHandle::noop(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<axon_acp_lib::AcpClientMessage>();
    ...
    mod preserve_prefix {
        ...
        use axon_sampling_types::conversation::ConversationItem;
        #[test]
    ...
    fn api_error_with_context_window(context_window: u64) -> axon_sampler::SamplingErrorInfo {
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = axon_sampler::SamplingErrorInfo {
                    kind: axon_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = axon_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = axon_chat_state::CompactionMode::Summary;
