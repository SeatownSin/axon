...
use axon_workspace::file_system::CodebaseIndexManager;
use axon_workspace::permission::{
...
impl axon_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        axon_tools::types::resources::ManagedGatewayToolCallResponse,
    ...
    > {
        let auth_key = self
            ...
            .ok_or_else(|| axon_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                axon_tool_runtime::ToolError::unauthorized(detail)
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            axon_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            axon_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, axon_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, axon_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, axon_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, axon_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `axon-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<axon_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: axon_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<axon_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<axon_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<axon_file_utils::queue::UploadQueue>>,
    ...
    /// Kill-switch (`AXON_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, axon_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: axon_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<axon_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<axon_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: axon_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `axon-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: axon_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`axon_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`axon_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: axon_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<axon_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `axon prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &axon_agent::PromptContext) {
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use axon_tools::types::output::{MCPOutput, ToolOutput};
    use axon_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl axon_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl axon_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> axon_tool_protocol::ToolId {
            axon_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::axon_tool_runtime::ListToolsContext,
        ) -> axon_tool_types::ToolDescription {
            axon_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
...
mod managed_gateway_tool_tests {
    ...
    use axon_tools::types::output::{MCPOutput, ToolOutput};
    use axon_tools::types::tool::{ToolKind, ToolNamespace};
    use axon_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<axon_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
