...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    axon_config::MANAGED_CONFIG_FILENAME,
    axon_config::REQUIREMENTS_FILENAME,
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            axon_config::MANAGED_CONFIG_CACHE_FILE,
...
/// files (TUI tick vs `axon login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `axon setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `AXON_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("AXON_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `axon-http` classifier.
...
/// Override with `AXON_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("AXON_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// Resolve deployment key from `AXON_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (axon setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("AXON_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.axon/`, trying the
...
fn apply_fetched(
    ...
    let verified = if axon_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(axon_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        axon_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(axon_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            axon_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(axon_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &axon_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<axon_config::signed_policy::SignatureEnvelope> {
    use axon_config::signed_policy::now_unix;
    ...
    let claim = match axon_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &axon_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `axon login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `axon setup` could install config for.
...
/// are the non-forgeable layers. Recovery: reconnect / `axon setup`; ceasing to serve
...
/// Outcome of the `axon setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.axon`.
    Installed,
...
/// Result of `axon setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `axon setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `axon setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `axon setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `axon setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && axon_config::signed_policy::verification_active()
...
/// Run the `axon setup` sync for the current principal. The caller must check
