...
pub enum ClientMode {
    /// Headless mode (axon agent, axon agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (axon agent stdio, axon -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `axon update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `axon update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `axon update` just installed; the leader uses
