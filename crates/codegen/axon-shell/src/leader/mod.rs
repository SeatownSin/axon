...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.axon/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use axon_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = axon_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`axon -p`).
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : axon_version::VERSION,
                            }
                        )),
                    );
                }
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`axon -p` where hanging forever is unacceptable.
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("AXON_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "axon_shell=info,axon_acp_lib=warn,axon_mcp=warn".into());
