...
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("AXON_CODE_WEB_URL").unwrap_or_else(|_| AXON_CODE_WEB_URL.to_string());
...
async fn add_bundle_fetch_headers(
    ...
    axon_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("AXON_CODE_BACKEND_URL")
                .unwrap_or_else(|_| AXON_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    async fn send_with_auth(
        ...
        let builder = axon_file_utils::trace_context::inject_trace_context_into_request(
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| axon_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(axon_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<axon_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<axon_sampling_types::CompactionsRemaining> {
...
mod tests {
    ...
    /// REGRESSION: `axon setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::remove_var("AXON_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
