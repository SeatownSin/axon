...
use std::sync::Arc;
/// Credentials for authenticating with axon backend services.
///
...
/// User token (Axon users) sends `Bearer` + `X-AXON-Token-Auth: axon-axon-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct AxonAuthCredentials {
...
impl std::fmt::Debug for AxonAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AxonAuthCredentials")
            .field(
                "user_token",
...
impl AxonAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your AXON_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `axon login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> AxonAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> AxonAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-AXON-Token-Auth"),
                    obfstr::obfstr!("axon-axon-cli"),
                )
        } else {
            builder
        };
...
impl axon_auth::HttpAuth for AxonAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        AxonAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = AxonAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = AxonAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = AxonAuthCredentials::new(Some("static-token".into()));
