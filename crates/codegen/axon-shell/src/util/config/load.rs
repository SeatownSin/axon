...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("AXON_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.axon/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.axon/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("axon-code-fast-1"));
        } else {
