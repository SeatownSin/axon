...
pub(crate) const ENV_COMPACTION_TOOL_CHOICE: &str = "AXON_COMPACTION_TOOL_CHOICE";
...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "AXON_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `AXON_AUTO_COMPACT_THRESHOLD_PERCENT`
...
///      (populated from `axon_build_models[i].auto_compact_threshold_percent`;
...
///      (populated from `axon_build_settings.auto_compact_threshold_percent`)
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "AXON_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `AXON_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes AXON_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
