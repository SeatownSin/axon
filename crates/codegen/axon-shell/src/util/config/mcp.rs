...
pub use axon_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use axon_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.axon/config.toml` at each level.
/// Project-scoped `.axon/config.toml` entries override global `~/.axon/config.toml`
...
/// `.axon/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.axon/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.axon/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.axon/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .axon/config.toml"
                );
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    axon_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.axon/config.toml`.
...
/// Upsert an MCP server entry in `~/.axon/config.toml`.
///
...
/// config file, e.g. a project-scoped `.axon/config.toml`.
...
/// Delete an MCP server entry from `~/.axon/config.toml`.
///
...
/// config file, e.g. a project-scoped `.axon/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = axon_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.axon/config.toml` files.
...
// `.mcp.json` discovery moved to `axon-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use axon_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.axon/config.toml` files from `cwd` up to the
...
/// Path to a project-level config file (`<dir>/.axon/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".axon").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.axon/skills", "~/.axon/skills/special/SKILL.md"]
ignore = ["~/.axon/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.axon/skills", "~/.axon/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.axon/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${AXON_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
