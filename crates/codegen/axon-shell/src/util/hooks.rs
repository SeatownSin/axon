...
use axon_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .axon/hooks/ directories are still scanned (they hold
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".axon").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    axon_hooks::discovery::HookRegistry,
    Vec<axon_hooks::error::HookError>,
) {
