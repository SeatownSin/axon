//! Built-in files extracted to `~/.axon/` on startup.
...
///
/// These directories under `~/.axon/skills/` will be deleted on startup
...
/// Extract bundled files to `~/.axon/` on startup.
///
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".axon").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".axon/skills/help/SKILL.md"),
        )
        .unwrap();
