...
/// two-function shape in `axon-tools` (F29).
...
/// `axon-tools` — existing items keep their prior content when
...
/// binary. Wraps a `axon_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: axon_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: axon_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
/// provider command (if `AXON_AUTH_PROVIDER_COMMAND` is set) is
...
async fn build_sampler_client(
    ...
    let config = axon_sampler::SamplerConfig {
        ...
        ..axon_sampler::SamplerConfig::default()
    };
    axon_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use axon_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    /// OIDC scope (the same scope `axon login` writes today and
    ...
    ///   the operator's real `~/.axon/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `AXON_AUTH_PROVIDER_COMMAND` — selects an external
