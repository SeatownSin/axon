...
//! `StorageClient` constructed inside `axon_file_utils::gcs::*` helpers.
//!
...
//! to an `axon_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `axon_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// axon_file_utils::gcs::upload_bytes(
...
/// `AXON_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("AXON_SESSION_TRACES_BUCKET_DEFAULT");
