...
//! using the axon-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `axon/code/goto-definition` | Definition location(s) for symbol at position |
//! | `axon/code/goto-references` | Reference location(s) for symbol at position |
//! | `axon/code/find-definitions` | All definitions of a symbol by name |
//! | `axon/code/find-references` | All references to a symbol by name |
//! | `axon/code/status` | Indexing status |
...
/// Reason string for the `axon/code/status` response.
///
...
pub async fn handle(
    ...
    use axon_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "axon/code/goto-definition" => {
...
fn to_code_nav_ext_response(resp: axon_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for axon-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise blocked.invalid/codeNavigation.enabled to use code navigation"
        }
