...
fn load_filtered_marketplace_sources() -> Vec<axon_plugin_marketplace::MarketplaceSource> {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        axon_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let axon_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = axon_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = axon_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
            ...
            axon_hooks_plugins_types::ActionOutcome {
                status: axon_hooks_plugins_types::OutcomeStatus::Success,
                message: msg,
...
async fn handle_update(
    ...
    let source_identity = |s: &axon_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            axon_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            axon_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let marketplace_root = match &source.kind {
        ...
        axon_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = axon_plugin_marketplace::git::default_cache_root();
            match axon_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                axon_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = axon_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = axon_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, axon_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(axon_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
...
fn scan_source(
    ...
    let scan = axon_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = axon_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match axon_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: axon_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.axon/config.toml`.
async fn handle_add_source(url: &str) -> axon_hooks_plugins_types::ActionOutcome {
...
/// Remove a marketplace source from `~/.axon/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> axon_hooks_plugins_types::ActionOutcome {
    ...
    match tokio::task::spawn_blocking(move || remove_source_locked(&src)).await {
        Ok(outcome) => outcome,
        Err(e) => axon_hooks_plugins_types::ActionOutcome {
            status: axon_hooks_plugins_types::OutcomeStatus::InternalError,
            message: format!("Config write task failed: {e}"),
            requires_reload: false,
            requires_restart: false,
        },
...
fn remove_source_locked(source_url_or_path: &str) -> axon_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use axon_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a axon_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<axon_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        axon_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            axon_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
...
mod default_skills_purge_tests {
    use super::*;
    use axon_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = axon_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(axon_hooks_plugins_types::PluginComponents {
                skills: vec![axon_hooks_plugins_types::ComponentItem::new(
                    "code-review",
