...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `axon sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.axon/config.toml`,
...
/// Hot-reload the model catalog from `~/.axon/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .axon/plugins under it, plus
