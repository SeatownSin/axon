...
// rmcp is quarantined in axon-mcp; see that crate's docs.
use axon_mcp::rmcp;
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<axon_tools::bridge::ToolBridge>,
    event_writer: Option<&axon_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(axon_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = axon_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from axon-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `axon-mcp` directly.
...
impl axon_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<axon_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(axon_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<axon_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(axon_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
            ...
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(axon_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
