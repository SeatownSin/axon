//! Handler for blocked.invalid/git/worktree/* extension methods.
...
const WORKTREE_EXT_LOG: &str = "axon_worktree";
...
impl WorktreeNotificationSender for GatewayWorktreeNotifier {
    async fn send_worktree_status(&self, progress: WorktreeStatus) {
        ...
        let notification = acp::ExtNotification::new("axon/git/worktree/status", params.into());
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "axon/git/worktree/create_from_worktree" => {
            let mut req =
            ...
            let result = ops
                .dispatch(
                    &axon_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "axon/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &axon_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &axon_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
                    .await
                    .unwrap_or(axon_workspace::session::git::VcsKind::Git);
                if vcs_kind.is_jj() {
            ...
            let result = ops
                .dispatch(
                    &axon_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "axon/git/worktree/resume_session" => {
            ...
            let agent_id = axon_telemetry::id::agent_id();
        ...
        "axon/git/worktree/list" => {
            let req: axon_workspace::workspace_ops::WorktreeListReq =
        ...
        "axon/git/worktree/show" => {
            let req = serde_json::from_str::<ShowWorktreeRequest>(args.params.get())?;
            let op = axon_workspace::workspace_ops::WorktreeShowReq {
        ...
        "axon/git/worktree/gc" => {
            ...
            let op = axon_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "axon/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &axon_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "axon", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("axon"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.axon/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.axon/worktrees.db\""));
    }
