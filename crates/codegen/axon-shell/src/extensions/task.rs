...
/// Wire DTO for the `axon/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (axon-pager) build
...
/// Wire DTO for the `axon/task/kill` ext response payload (nested under
...
/// Wire DTO for the `axon/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (axon-pager) build
...
/// Wire DTO for the `axon/subagent/cancel` response payload (under `result` in
...
/// Handle `axon/scheduler/*` extension methods.
...
/// Handle `axon/subagent/*` extension methods.
...
mod tests {
    ...
    // ── blocked.invalid/subagent/cancel outcome wire DTO ──────────────────────────
