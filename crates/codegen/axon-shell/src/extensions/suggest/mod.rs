...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "axon/suggest" => handle_suggest(agent, args).await,
        "axon/suggestPrompt" => handle_suggest_prompt(agent, args).await,
        _ => Err(acp::Error::method_not_found()),
    }
}

/// Request/response for `axon/suggestPrompt` — predict the user's likely next
...
struct SuggestPromptRequest {
    ...
    /// override, or `axon-build-0.1` when its catalog offers it). One tier
    /// of the shell-side resolution in
    /// `prompt_suggest::effective_suggest_model`: env > config.toml > remote
    /// > this hint > `axon-build-0.1` default, catalog-guarded (a
...
/// forever. Reasoning models (e.g. `axon-build`) can take ~30s on a cold
