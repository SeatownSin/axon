//! `axon/memory/flush`, `axon/memory/rewrite`, and `axon/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("axon/compact_conversation") => handle_compact(agent, args).await,
        "axon/memory/flush" => handle_flush(agent, args).await,
        "axon/memory/rewrite" => handle_rewrite(agent, args).await,
