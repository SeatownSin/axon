...
const NOTIFICATION_METHOD: &str = "axon/terminal/pty/notification";
...
/// `AXON_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use axon_config::shell::{WindowsShell, detect_windows_shell};
