...
/// Resolved absolute path to bash. On Unix uses the `axon_config` shell
/// resolution cascade (`$AXON_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        axon_config::shell::unix_shell_path(axon_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `axon-tools`.
pub use axon_tools::util::pager_env;
...
/// tool output. Used when the client sets `axon/bashOutputNoColor: true` (e.g.
/// axon-pager which renders its own UI and doesn't need raw ANSI codes).
///
