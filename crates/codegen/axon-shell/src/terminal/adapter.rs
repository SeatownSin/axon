//! AcpTerminalAdapter: implements `axon-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates axon-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (axon-desktop) spawns with `shell: true`
...
/// Wraps axon-shell's ACP gateway to satisfy axon-tools' TerminalBackend.
