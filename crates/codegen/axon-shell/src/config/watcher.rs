...
/// Watches `~/.axon/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.axon/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.axon/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.axon/config.toml` so the loop above
        // watches `<cwd>/.axon/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.axon/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.axon/config.toml` trigger a
    ...
    /// `<cwd>/.axon/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.axon/` does not yet
/// exist at session-open time, the `.axon/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.axon/` followed by a
/// write to `<cwd>/.axon/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
        ...
        .watch(&axon_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .axon directory (non-recursive)",
        );
    }
}
...
/// expected for a freshly-opened session whose `<cwd>/.axon/` hasn't
...
/// Watches skill directories (`~/.axon/skills/`, `<repo>/.axon/skills/`, etc.)
...
/// `~/.axon` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](axon_agent::prompt::skills::collect_skill_config_dirs)
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_axon_home_and_vendor_dirs() {
        ...
        assert!(!g(&home.join("repo").join(".axon")));
    }

    /// Regression for the ~/.axon inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.axon/worktrees/`) must not
    ...
    /// A write to `<cwd>/.axon/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.axon/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.axon/config.toml` writes
