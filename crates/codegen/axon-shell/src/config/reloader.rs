...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.axon/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.axon/config.toml`,
    ...
    /// (`<cwd>/.axon/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.axon/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.axon/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.axon/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.axon/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.axon/config.toml` | `<cwd>`           |
...
/// for `.axon/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.axon/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `axon inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> axon_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> axon_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    /// The hash must reflect ancestor `.axon/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".axon").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .axon/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.axon/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.axon/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.axon/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.axon/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.axon/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.axon/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
