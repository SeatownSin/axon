...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "AXON_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$AXON_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${AXON_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$AXON_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$AXON_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the AXON_MEMORY env var.
...
fn memory_config_env_var_zero_does_not_enable() {
    with_axon_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "AXON_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_axon_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "AXON_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_axon_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "AXON_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_axon_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "AXON_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_axon_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override AXON_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_axon_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, AXON_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_axon_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override AXON_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$AXON_TEST_USER $AXON_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
    ...
    with_axon_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(! sa.enabled, "AXON_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_axon_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(
                ! sa.enabled, "AXON_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_axon_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config);
            assert!(sa.enabled, "--subagents CLI flag should override AXON_SUBAGENTS=0");
        },
    );
}
#[test]
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("AXON_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "AXON_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "AXON_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "AXON_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("AXON_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
    ...
    with_axon_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "AXON_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".axon/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".axon/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".axon/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".axon").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".axon/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".axon").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".axon").join("bundled");
    std::fs::create_dir_all(workspace.join(".axon").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".axon").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".axon").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".axon").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".axon/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".axon/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".axon/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".axon/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".axon/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".axon/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".axon/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".axon/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.axon/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "AXON_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
fn validate_hooks_path_rejects_outside_axon_home() {
    ...
    assert!(
        msg.contains("must be under ~/.axon/"),
        "should mention ~/.axon/ restriction, got: {msg}"
    );
}
#[test]
...
fn simulate_release_build() -> axon_test_support::EnvGuard {
    axon_test_support::EnvGuard::set(axon_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".axon"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
/// in `axon-agent`. An ABSOLUTE plugin path is used so the merged
...
/// AXON_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use axon_agent::plugins::{TrustStore, discover_plugins};
    use axon_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("AXON_HOME", home.path());
    let _flag = EnvGuard::unset("AXON_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. AXON_HOME-isolated (empty
/// store); AXON_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
