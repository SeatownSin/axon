...
//!
//! Run: `cargo bench -p axon-shell --bench session_list`
...
impl Fixture {
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = axon_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = axon_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(axon_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
