...
//! `AXON_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the Axon-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p axon-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_AXON_SKILL: &str = "ZZ_AXON_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".axon", "axon-skill", MARKER_AXON_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = AxonStdioClient::spawn_with_home_and_env(&server, workdir.path(), home, env).await;
...
/// Defaults (all vendors on): axon + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_AXON_SKILL),
            "axon-skill must always be present"
        );
        assert!(
...
/// `AXON_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; axon stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_AXON_SKILL),
            "axon-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `AXON_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; axon stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_AXON_SKILL),
            "axon-skill always present"
        );
...
/// `AXON_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `AXON_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `AXON_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `AXON_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("AXON_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("AXON_CURSOR_SKILLS_ENABLED", "false"),
            ("AXON_CURSOR_RULES_ENABLED", "false"),
            ("AXON_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_AXON_SKILL), "axon always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("AXON_CLAUDE_SKILLS_ENABLED", "false"),
            ("AXON_CLAUDE_RULES_ENABLED", "false"),
            ("AXON_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only axon-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
