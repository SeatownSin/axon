...
/// All tests share this as AXON_HOME (the `OnceLock` in axon-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = axon_shell::config::load_effective_config().unwrap();
    let effective_cfg = axon_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    axon_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = axon_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("axon-3"));
}
