...
//!
//! The tests exercise:
//! - **Smoke** (`axon --version`): binary loads without crashing
//! - **ACP stdio** (`axon agent stdio`): full protocol lifecycle via ClientSideConnection
//!
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "axon -p in git repo", Some(&server));
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "axon -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    ...
    assert_headless_success(&result, "axon -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "axon -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "axon -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "axon -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use axon_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        axon_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "axon -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        axon_test_support::scripted::ScriptedResponse::sse(
            axon_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "axon -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    ...
    assert_headless_success(&result, "axon -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "axon -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "axon -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "axon -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "axon -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (axon agent stdio)
//
// These test the agent as a server: spawn `axon agent stdio`, speak the full
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".axon")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("AXON_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("AXON_TELEMETRY_ENABLED".into(), "false".into()),
                ("AXON_FEEDBACK_ENABLED".into(), "false".into()),
                ("AXON_TRACE_UPLOAD".into(), "false".into()),
                ("AXON_INSTRUMENTATION".into(), "disabled".into()),
                ("AXON_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }
...
"#,
    ...
    .unwrap();
    h.env("AXON_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "axon -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "axon -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "axon -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_waits_for_short_background_task_and_exits_clean() {
    ...
    assert_headless_success(&result, "axon -p with short background task", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        marker.exists(),
