...
//!   `~/.axon/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `AXON_LOG_SAMPLING`/`AXON_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `AXON_DEBUG_LOG=1`) writes a per-session
//!   `~/.axon/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.axon/debug/` files).
//! - `AXON_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p axon-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$AXON_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".axon").join("debug")
}

/// List firehose `*.txt` files under `~/.axon/debug` (excluding the `latest.txt`
...
/// Build a headless `axon -p` command with a pinned `$AXON_HOME` so the firehose
/// lands under `<home>/.axon/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("AXON_HOME", home.join(".axon"));
    cmd.env_remove("AXON_DEBUG_LOG");
    cmd.env_remove("AXON_LOG_FILE");
    cmd.env_remove("AXON_LOG_SAMPLING");
    cmd.env_remove("AXON_HOOKS_LOG");
    cmd
}
...
/// headless `axon -p` client is near-silent, so its lazily-opened firehose may
...
/// A live `agent` session writes `~/.axon/debug/<sessionId>.txt` with real
...
/// The `--debug` FLAG (not `AXON_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `AXON_LOG_SAMPLING`/`AXON_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears AXON_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.axon/logs/sampling.jsonl` is never written — the `--debug`
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.axon/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
async fn axon_log_file_explicit_path_is_written() {
    ...
    assert!(
        custom.exists(),
        "explicit AXON_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "AXON_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
