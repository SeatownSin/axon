...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: AXON_BINARY_LEADER/AXON_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".axon").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set AXON_BINARY_LEADER/AXON_BINARY_CLIENT and run with --ignored"]
...
/// `axon update`'s relaunch signal against a REAL old leader: connect,
/// require `relaunch_v1`, send `RelaunchForUpdate`, and the leader exits so
/// the surviving client re-elects. Mirrors the private
/// `signal_leaders_to_relaunch` in `axon-pager-bin/src/main.rs` (which is
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".axon").join("auth.json");
