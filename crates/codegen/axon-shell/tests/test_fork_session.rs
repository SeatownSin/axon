...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    let model_id = acp::ModelId::new("axon-code-fast-1");
    ...
    storage
        .append_update(
            &info,
            &axon_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = axon_shell::session::storage::CopySessionOptions {
    ...
    assert_eq!(loaded.summary.current_model_id, acp::ModelId::new("axon-3"));
    assert_eq!(
    ...
    match &loaded.updates[0] {
        axon_shell::session::storage::SessionUpdate::Acp(notification) => {
