...
//! `AXON_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p axon-shell --test session_load_perf -- --nocapture
//!   cargo test -p axon-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use axon_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use axon_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("AXON_PERF_SCALE", 1).max(1);
        Self {
            turns: g("AXON_PERF_TURNS", 80) * scale,
            acu_per_turn: g("AXON_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("AXON_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("AXON_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("AXON_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("AXON_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("AXON_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("AXON_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("AXON_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `AXON_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("AXON_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `AXON_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("AXON_PERF_SESSION_SRC").is_ok() {
        return;
    }
        ...
        .run_until(async move {
            ...
            tokio::task::spawn_local(
                GatewayReceiver::new(gw_rx, agent_conn)
                    .with_on_meta(axon_file_utils::trace_context::span_from_meta_traceparent)
                    .run(),
            );
            ...
            let _ = axon_shell::instrumentation::finalize();
