...
//!
//! Knobs: AXON_PERF_GIT_FILES (default 300), AXON_PERF_GIT_PICKS (default 6).
...
use axon_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    axon_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use axon_hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use axon_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // AXON_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("AXON_E2E_LOG").ok();
    ScanCounter(
        axon_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    axon_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = axon_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("AXON_PERF_GIT_FILES", 300);
    let picks = env_usize("AXON_PERF_GIT_PICKS", 6);
