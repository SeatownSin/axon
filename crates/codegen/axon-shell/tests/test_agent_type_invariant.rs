...
//! - Session resume
//!
//! Each test spawns a real `axon agent stdio` process, speaks the full ACP
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".axon").join("models_cache.json");
    if cache.exists() {
...
async fn test_axon_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            axon_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            ...
            let incoming = axon_acp_lib::LineBufferedRead::spawn_local(incoming);
