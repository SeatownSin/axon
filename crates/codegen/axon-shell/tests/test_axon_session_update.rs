//! Integration test for `_axon/session/update` notifications.
//!
//! This test verifies that:
//! 1. Axon session notifications (e.g., diff_review) can be sent via ext_notification
...
use axon_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as AxonSessionUpdate,
};
use axon_shell::session::info::Info as SessionInfo;
use axon_shell::session::persistence::default_model_id;
use axon_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that Axon session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_axon_session_notification_storage_roundtrip() {
    ...
    let axon_notification = SessionNotification {
        session_id: session_id.clone(),
        update: AxonSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Axon(Box::new(axon_notification.clone())),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.updates.len(),
        2,
        "Should have 2 updates (1 Axon + 1 ACP)"
    );

    // Verify Axon notification
    match &loaded.updates[0] {
        SessionUpdate::Axon(notification) => {
            assert_eq!(notification.session_id, session_id);
            match &notification.update {
                AxonSessionUpdate::DiffReview { content } => {
...
/// must survive `updates.jsonl` and reload as a replayable `_axon/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Axon(Box::new(axon_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and Axon notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add Axon notification with totalTokens
    let axon_notification = SessionNotification {
        session_id: session_id.clone(),
        update: AxonSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_axon_session_notification_serialization() {
