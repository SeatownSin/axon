...
use axon_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
use axon_paths::AbsPathBuf;
use axon_workspace::permission::types::{
...
/// Shared `AXON_HOME` for the entire test binary. The `OnceLock` in
/// `axon-config` only allows one value per process, so all tests share
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("AXON_HOME", &path) };
        path
    })
}
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::AxonPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / AxonWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use axon_workspace::permission::types::{PermissionConfig, PromptPolicy};
