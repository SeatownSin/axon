...
//! `off`. Custom profiles via `~/.axon/sandbox.toml` or `.axon/sandbox.toml`.
...
/// Load sandbox config from `~/.axon/sandbox.toml` and `.axon/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Project config: <workspace>/.axon/sandbox.toml (additive only)
    let project_path = workspace.join(".axon").join("sandbox.toml");
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.axon/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.axon/sandbox.toml or .axon/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
