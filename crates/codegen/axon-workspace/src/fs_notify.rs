...
//! FsNotify adapter functions bridging [`axon_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](axon_codebase_graph::IndexManagerHandle).
//! The [`refresh_codebase_graph_after_head_change`] helper handles
//! git HEAD changes by diffing `ORIG_HEAD..HEAD`.

use std::path::{Path, PathBuf};

use axon_fsnotify::{FsEvent, FsEventKind};
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> axon_codebase_graph::FileEvent {
    use axon_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`axon_fsnotify::FsEventKind`] to the wire-type
/// [`axon_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> axon_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => axon_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => axon_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => axon_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => axon_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => axon_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](axon_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<axon_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<axon_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    axon_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<axon_codebase_graph::FileEvent> {
    ...
    match status.chars().next()? {
        ...
        _ => Some(axon_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &axon_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<axon_workspace_types::WorkspaceEvent>,
) {
    ...
    axon_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(axon_tools::util::pager_env());
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: axon_workspace_types::FsEventKind,
) -> axon_codebase_graph::FileEvent {
    use axon_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        axon_workspace_types::FsEventKind::Created => FileEventKind::Created,
        axon_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        axon_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        axon_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.axon/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use axon_workspace_types::FsEventKind as WsKind;
        assert_eq!(
