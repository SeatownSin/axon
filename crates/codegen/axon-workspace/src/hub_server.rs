...
use axon_tools::types::resources::Terminal;
...
) -> Vec<axon_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use axon_tools::computer::types::TaskKind;
    use axon_tools::types::resources::Terminal;
    use axon_tools::types::tool::ToolKind;
    use axon_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use axon_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use axon_workspace_types::rpc::search::FuzzyStatusReq;
        use axon_workspace_types::rpc::skills::DiscoverPluginsReq;
        use axon_workspace_types::rpc::workspace::{
        ...
        use axon_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<axon_tool_runtime::SessionContext>();
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use axon_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<axon_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use axon_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use axon_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(axon_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            axon_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use axon_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
