...
/// `AXON_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `AXON_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `AXON_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: axon_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
...
/// The durability idle-hold cap from `AXON_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("AXON_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
