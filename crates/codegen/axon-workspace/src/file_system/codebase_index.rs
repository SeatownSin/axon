...
use axon_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use axon_tools::util::axon_home::axon_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.axon/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `axon/code/status` reporting `reason: notStarted`.
    ...
    /// In `axon/code/status`, `resolve_index_handle()` calls
