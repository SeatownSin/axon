...
//!
//! Persists per-folder trust decisions to `~/.axon/trusted_folders.toml`.
...
//! a `./.axon/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.axon/`.
...
/// sharing `~/.axon/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.axon/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = axon_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match axon_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.axon");
        assert_eq!(
    ...
    fn register_axon_worktree(
        ...
        use axon_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
