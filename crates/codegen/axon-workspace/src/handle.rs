...
/// preStop-hook drain marker; override via `AXON_WORKSPACE_DRAINING_FILE`.
...
use axon_file_utils::events::types::CancellationCategory;
use axon_file_utils::events::{Event, SessionRelationship, TurnOutcomeLabel};
use axon_file_utils::queue::EnqueueOutcome;
use axon_tool_protocol::turn_hook::{AfterTurnAckPayload, AfterTurnAckStatus};
...
impl WorkspaceHandle {
    ...
    pub async fn trace_donation_reporter(
        ...
        axon_computer_hub_sdk::HubDonatingReporter,
        axon_computer_hub_sdk::TraceDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    /// teardown.
    ///
    /// [`LogDonationSender`]: axon_computer_hub_sdk::LogDonationSender
    pub async fn log_donation_layer(
        &self,
        service_name: &str,
    ) -> Option<(
        axon_computer_hub_sdk::LogDonationSender,
        axon_computer_hub_sdk::LogDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    pub async fn metric_donation_reporter(
        &self,
        service_name: &str,
    ) -> Option<axon_computer_hub_sdk::MetricDonationPump> {
        self.shared
            .hub_handle
            .lock()
            .await
    ...
    /// Construct a handle with an explicit `$AXON_WORKSPACE_HOME` and a
    /// pre-spawned [`UploadQueue`](axon_file_utils::queue::UploadQueue).
    ///
    ...
    pub(crate) fn new_with_data_collection(
        ...
        upload_queue: Arc<axon_file_utils::queue::UploadQueue>,
    ...
    fn build(
        config: WorkspaceConfig,
        workspace_home: std::path::PathBuf,
        upload_queue: Option<Arc<axon_file_utils::queue::UploadQueue>>,
        ...
        let local_registry = axon_computer_hub_sdk::LocalRegistry::new();
        ...
        let (hook_registry, hook_load_errors) = {
            use axon_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn axon_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                axon_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            if servers.is_empty() {
                ...
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    ...
                    true,
                    axon_tools::notification::ToolNotificationHandle::noop(),
                )));
        ...
        let session_event_writers: Arc<
            dashmap::DashMap<String, axon_file_utils::events::EventWriter>,
    ...
    /// The [`ToolServer`](axon_computer_hub_sdk::ToolServer) for this
    ...
    pub fn hub_server(&self) -> Option<axon_computer_hub_sdk::ToolServer> {
    ...
    pub async fn hub_server_blocking(&self) -> Option<axon_computer_hub_sdk::ToolServer> {
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<axon_tools::registry::types::ToolServerConfig>,
        capability: CapabilityMode,
        viewer_ctx: Option<axon_tool_runtime::WorkspaceViewerContext>,
    ...
    fn create_session_with_tracker_inner(
        ...
        let system_notify_channel = system_notifications
            .then(axon_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: axon_tools::registry::types::ToolServerConfig,
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<axon_tools::registry::types::ToolServerConfig>,
    ...
    pub async fn on_before_turn(
        &self,
        session_id: &str,
        payload: &axon_tool_protocol::turn_hook::BeforeTurnPayload,
    ) {
    ...
    pub async fn compute_turn_injections(
        &self,
        session_id: &str,
        request: &axon_tool_protocol::turn_hook::TurnHookRequest,
    ) -> axon_tool_protocol::turn_hook::HookReply {
        use axon_tool_protocol::turn_hook::{HookReply, TurnHookRequest};
        match request {
    ...
    /// `AXON_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `AXON_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn cancel_tool_call(&self, session_id: &str, call_id: &str) {
        self.shared.activity_tracker.tool_call_completed(
            call_id,
            Some(session_id),
            axon_file_utils::events::ToolOutcome::Cancelled,
        );
    ...
    pub fn hook_registry(&self) -> axon_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[axon_hooks::error::HookError] {
    ...
    pub fn get_or_create_codebase_index(
        &self,
        cwd: std::path::PathBuf,
    ) -> (Arc<axon_codebase_graph::IndexManagerHandle>, bool) {
        self.shared.codebase_indexes.lock().get_or_create(cwd)
    }
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<Arc<axon_codebase_graph::IndexManagerHandle>> {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(axon_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(axon_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(axon_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    async fn emit_environment_artifact(
    ...
    ) -> Option<axon_file_utils::queue::EnqueueOutcome> {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(axon_tracing::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        let outcome = upload_queue
            ...
            .await;
        match &outcome {
            axon_file_utils::queue::EnqueueOutcome::Failed { reason: _ } => {
                dc_log!(
    ...
    pub async fn start_session_mcp_servers(
        ...
        use axon_computer_hub_mcp_adapter::McpBridge;
        use axon_computer_hub_sdk::ToolServerHandler as _;
        use axon_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        use axon_tool_protocol::SessionId;
        let tool_server = {
        ...
        for result in mcp_results {
            match result {
                Ok(client) => {
                    ...
                    let transport: Arc<dyn axon_computer_hub_mcp_adapter::McpTransport> =
                        ...
                        Ok(handle) => {
                            for handler in handle.bridge.handlers() {
                                    ...
                                    .await
                                {
                                    ...
                                    axon_tool_protocol::ToolId::new(&qualified_name)
                                {
    ...
    pub async fn teardown_session_mcp(&self, session_id: &str) {
        ...
        let sid = match axon_tool_protocol::SessionId::new(session_id) {
            Ok(s) => s,
            Err(_) => return,
        };
        let mut tool_ids = session.mcp_tool_ids.lock().await;
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<axon_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<axon_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
    ...
    pub(crate) fn session_bind_resolver(
        &self,
        catalog: Arc<Vec<Arc<dyn axon_computer_hub_sdk::ToolServerHandler>>>,
        rpc_tool_id: axon_tool_protocol::ToolId,
    ) -> axon_computer_hub_sdk::SessionHandlerResolver {
        let weak_shared = Arc::downgrade(&self.shared);
        Arc::new(
            move |sid: axon_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(axon_tracing::local_or_random_span_ctx);
                ...
                async move {
                    let Some(shared) = weak_shared.upgrade() else {
                        WORKSPACE_BIND_FAILED_TOTAL
                            ...
                            .inc();
                        return Err(
                            axon_tool_runtime::ToolError::service_unavailable(
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || axon_tools::registry::types::ToolServerConfig {
                        tools: vec![],
                    ...
                    Ok(axon_computer_hub_sdk::ResolvedSessionHandlers {
                        handlers,
                        unserved_tool_ids,
                        resolve_error,
                    })
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (template_handlers, rpc_tool_id) = {
            ...
            let tool_names: Vec<String> = handlers
                ...
                .map(|h| h.tool_id().as_str().to_owned())
                .collect();
            let rpc_handler: Arc<dyn axon_computer_hub_sdk::ToolServerHandler> =
        ...
        let catalog: Arc<Vec<Arc<dyn axon_computer_hub_sdk::ToolServerHandler>>> =
        ...
        let (activity_notify_handle, activity_notify_rx) =
            axon_tools::notification::types::ToolNotificationHandle::channel();
        ...
        let listener_task = tokio::spawn(async move {
            while let Some(notification) = notification_rx.recv().await {
                match notification {
                    axon_computer_hub_sdk::HubNotification::ToolsChanged {
                        added,
                        removed,
                        updated,
                        ..
            ...
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        ...
                        let frame = axon_tool_protocol::ToolNotificationFrame::custom(
                            axon_tool_protocol::ToolId::new(
        ...
        let status_publisher_task = tokio::spawn(async move {
            ...
            async fn send_status(
                conn: &axon_computer_hub_sdk::HubConnection,
                ...
                let req = axon_tool_protocol::JsonRpcRequest {
                    jsonrpc: axon_tool_protocol::JsonRpcVersion,
                    id: axon_tool_protocol::JsonRpcId::from_request_id(&request_id),
                    session_id: None,
                    method: axon_tool_protocol::Method::ToolServerStatus
                        .as_wire_str()
                        .to_owned(),
                    params,
                };
...
fn build_session_routed_handlers(
    ...
    ws: &WorkspaceHandle,
) -> Vec<Arc<dyn axon_computer_hub_sdk::ToolServerHandler>> {
    ...
    for def in toolset.tool_definitions() {
        ...
        let mut desc = axon_tool_types::ToolDescription::new(
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &axon_tools::notification::types::ToolNotification,
) {
    use axon_tools::notification::types::ToolNotification;
    match notification {
...
/// The SIGTERM drain budget from `AXON_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("AXON_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`AXON_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("AXON_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
pub async fn connect_local_workspace(
    ...
    if let Ok(dir) = std::env::var("AXON_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("AXON_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("AXON_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
    ...
    let trace_source: Arc<dyn axon_file_utils::queue::TraceExportSource> = Arc::new(
        crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()),
    );
    let upload_queue = Arc::new(axon_file_utils::queue::UploadQueue::spawn(
        &workspace_home,
        trace_source,
        axon_file_utils::queue::UploadRetryPolicy::default(),
    ));
    ...
    upload_queue.cleanup_orphans(axon_file_utils::queue::DEFAULT_MAX_AGE);
...
/// Resolve `$AXON_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
...
/// (`AXON_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
/// keeps the legacy behaviour: [`WorkspaceShared::session_event_writer`] hands
/// back [`EventWriter::noop()`](axon_file_utils::events::EventWriter::noop)
/// and no `events.jsonl` is ever opened.
fn events_enabled() -> bool {
    std::env::var("AXON_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `AXON_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("AXON_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`AXON_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("AXON_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
async fn enqueue_workspace_tool_definitions(
    upload_queue: &axon_file_utils::queue::UploadQueue,
    session_id: &str,
    object_path: &str,
    bytes: &[u8],
) -> axon_file_utils::queue::EnqueueOutcome {
    use axon_file_utils::queue::EnqueueOutcome;
    let outcome = upload_queue
        .enqueue_bytes_blocking(
            bytes,
...
fn turn_outcome_label(outcome: axon_tool_protocol::turn_hook::TurnHookOutcome) -> TurnOutcomeLabel {
    use axon_tool_protocol::turn_hook::TurnHookOutcome;
    match outcome {
...
fn rewind_all_outcomes_from_env() -> bool {
    axon_config::env_bool("AXON_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
struct SessionToolHandle {
    tool_id: axon_tool_protocol::ToolId,
    desc: axon_tool_types::ToolDescription,
...
impl SessionToolHandle {
    fn new(
        ...
        workspace: WorkspaceHandle,
        session_id: String,
    ) -> Result<Self, axon_tool_protocol::IdError> {
        Ok(Self {
            tool_id: axon_tool_protocol::ToolId::new(tool_name)?,
            desc,
            workspace,
            session_id,
        })
...
impl axon_tool_runtime::ToolDyn for SessionToolHandle {
    fn id(&self) -> axon_tool_protocol::ToolId {
        self.tool_id.clone()
    }
    fn description(
        &self,
        _ctx: &::axon_tool_runtime::ListToolsContext,
    ) -> axon_tool_types::ToolDescription {
        self.desc.clone()
    }
    async fn execute(
        &self,
        ctx: axon_tool_runtime::ToolCallContext,
        args: serde_json::Value,
    ) -> axon_tool_runtime::ToolStream<axon_tool_runtime::TypedToolOutput> {
        use axon_tool_runtime::{ToolError, ToolErrorKind, ToolStreamItem, terminal_only};
...
impl WorkspaceHandle {
    ...
    pub fn create_local_harness(
        &self,
        session_id: &str,
    ) -> WorkspaceResult<axon_computer_hub_sdk::ToolHarness> {
        let session = self
            .session(session_id)
            .ok_or_else(|| WorkspaceError::SessionNotFound(session_id.to_string()))?;
        let toolset = session.toolset();
        let registry = axon_computer_hub_sdk::LocalRegistry::new();
        for def in toolset.tool_definitions() {
            let tool_name = def.function.name.clone();
            let desc = axon_tool_types::ToolDescription::new(
                tool_name.clone(),
                def.function.description.clone().unwrap_or_default(),
            );
            match SessionToolHandle::new(tool_name, desc, self.clone(), session_id.to_string()) {
                Ok(tool) => {
                    registry.register_dyn(Arc::new(tool) as Arc<dyn axon_tool_runtime::ToolDyn>);
                }
                Err(e) => {
                    tracing::warn!(
                        tool = % def.function.name, error = % e,
        ...
        let session_id = axon_tool_protocol::SessionId::new(session_id.to_string())
            .map_err(|e| WorkspaceError::HubError(format!("invalid session id: {e}")))?;
        Ok(axon_computer_hub_sdk::ToolHarness::local_only_with(
            registry,
            session_id,
            axon_tool_runtime::TypedExtensions::default(),
        ))
    }
}
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: axon_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    fn make_handle_inner(
        ...
        let config = WorkspaceConfig {
            ...
            plugin_discovery_config: Default::default(),
            hub_config: None,
            auth_provider: Some(Arc::new(axon_computer_hub_sdk::AuthCredential::bearer(
                "test-token",
            ))),
            server_metadata: None,
            status_config,
    ...
    impl axon_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> axon_tools::types::tool::ToolNamespace {
            axon_tools::types::tool::ToolNamespace::MCP
        }
        fn description_template(&self) -> &str {
            "bash cco stub"
        }
    }
    impl axon_tool_runtime::Tool for BashCcoStub {
        type Args = serde_json::Value;
        type Output = axon_tools::types::output::ToolOutput;
        fn id(&self) -> axon_tool_protocol::ToolId {
            axon_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::axon_tool_runtime::ListToolsContext,
        ) -> axon_tool_types::ToolDescription {
            axon_tool_types::ToolDescription::new(BASH_CCO_STUB_NAME, "bash cco stub")
        }
        async fn run(
            &self,
        ...
        ) -> Result<axon_tools::types::output::ToolOutput, axon_tool_runtime::ToolError>
        {
            ...
            Ok(axon_tools::types::output::ToolOutput::Bash(
                axon_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        axon_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    pub(crate) fn assert_bash_cco_terminal(typed: &axon_tool_runtime::TypedToolOutput) {
        use axon_tool_runtime::ToolOutput as _;
        let resp = typed
    ...
    async fn local_harness_preserves_bash_chat_completion_output() {
        use axon_tool_runtime::ToolCallContext;
        let handle = make_handle();
        register_bash_cco_stub(&handle);
        let harness = handle.create_local_harness("main").expect("local harness");
        let tool_id = axon_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id");
        let stream = harness
    ...
    async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
        use axon_file_utils::events::ToolOutcome;
    ...
    async fn toolset_terminal(
        toolset: &Arc<axon_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn axon_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<axon_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    ///
    /// [`TerminalRunRequest`]: axon_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> axon_tools::computer::types::TerminalRunRequest {
        axon_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: axon_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: axon_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> axon_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                axon_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> axon_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn assert_hunk_tracker_stops(tracker: &axon_hunk_tracker::HunkTrackerHandle) {
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let axon_tools::types::output::ToolOutput::TaskOutput(
            axon_tool_types::TaskOutputOutput::TaskNotFound(msg),
        ) = &result.output
        else {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        use axon_file_utils::events::ToolOutcome;
        use axon_tool_protocol::turn_hook::{AfterTurnPayload, BeforeTurnPayload, TurnHookOutcome};
        ...
        handle.shared().session_event_writer(sid).emit(
            axon_file_utils::events::Event::McpToolCallStarted {
    ...
    async fn before_turn_hooks_sync_session_yolo_mode() {
        use axon_tool_protocol::turn_hook::{BeforeTurnPayload, TurnHookRequest};
        let handle = make_handle();
        ...
        assert_eq!(
            reply,
            axon_tool_protocol::turn_hook::HookReply::default(),
    ...
    async fn before_turn_yolo_transition_emits_yolo_toggled_event() {
        use axon_tool_protocol::turn_hook::BeforeTurnPayload;
        let (handle, home) = make_handle_with_events();
        let sid = "sess-yolo";
        let _session = handle
    ...
    fn turn_outcome_label_maps_every_variant() {
        use axon_file_utils::events::TurnOutcomeLabel;
        use axon_tool_protocol::turn_hook::TurnHookOutcome;
        assert!(matches!(
    ...
    impl axon_file_utils::queue::TraceExportSource for UnreachableSource {
        fn resolve(&self) -> axon_file_utils::TraceExportConfig {
            axon_file_utils::TraceExportConfig {
                bucket_url: None,
                service_account_key: None,
                upload_method: axon_file_utils::UploadMethod::Proxy {
    ...
    fn spawn_test_queue(home: &std::path::Path) -> Arc<axon_file_utils::queue::UploadQueue> {
        let policy = axon_file_utils::queue::UploadRetryPolicy {
            initial_delay: std::time::Duration::from_secs(3600),
            ..Default::default()
        };
        Arc::new(axon_file_utils::queue::UploadQueue::spawn(
            home,
    ...
    /// `$AXON_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $AXON_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("AXON_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("AXON_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    fn make_queue_backed_handle_with(
        ...
        let auth: axon_computer_hub_sdk::SharedAuthProvider = Arc::new(
            axon_computer_hub_sdk::auth::AuthCredential::bearer("test-token"),
        );
        ...
        let source: Arc<dyn axon_file_utils::queue::TraceExportSource> =
            Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy));
        let policy = axon_file_utils::queue::UploadRetryPolicy {
            max_attempts: 1,
            ..Default::default()
        };
        let queue = Arc::new(axon_file_utils::queue::UploadQueue::spawn(
            home.path(),
            source,
            policy,
        ));
    ...
    async fn environment_artifact_enqueued_when_queue_present() {
        ...
        assert!(
            matches!(
                outcome,
                Some(axon_file_utils::queue::EnqueueOutcome::Enqueued)
            ),
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("axon-workspace-test-cwd-override");
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = axon_hooks::config::HookSpec {
                name: "injected".into(),
                event: axon_hooks::event::HookEventName::SessionStart,
    ...
    async fn fork_session_inherits_viewer_ctx_from_parent() {
        ...
        let parent = handle
            .create_session_with_tracker_and_viewer_ctx(
                ...
                axon_hunk_tracker::HunkTrackerHandle::noop(),
                None,
                CapabilityMode::All,
                Some(axon_tool_runtime::WorkspaceViewerContext {
    ...
    fn handler_names(resolved: &axon_computer_hub_sdk::ResolvedSessionHandlers) -> Vec<String> {
        resolved
            .handlers
            .iter()
            .map(|h| h.tool_id().as_str().to_owned())
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        assert!(
            reason.contains(axon_version::VERSION),
    ...
    async fn strict_rpc_only_bind_fails_closed_with_resolve_error_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            axon_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap(),
            Some(serde_json::json!(
    ...
    async fn lax_bind_without_metadata_uses_default_catalog_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            axon_tool_protocol::SessionId::new("bind-e2e-lax").unwrap(),
            None,
        )
        .await
        .expect("bind must succeed");
    ...
    async fn rejected_rebind_config_keeps_resolve_error_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-rejected").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "AxonBuild:read_file" }] },
    ...
    async fn explicit_empty_toolset_rebind_never_swaps_session_tools() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "AxonBuild:read_file" }] },
    ...
    async fn owner_toolset_survives_concurrent_consumer_shaped_rebinds() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-consumer-storm").unwrap();
    ...
    async fn restored_server_first_bind_ordering_decides_capability_and_toolset() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-restore-read-first").unwrap();
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-restore-write-first").unwrap();
        resolver(
            sid.clone(),
            Some(serde_json::json!(
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = axon_tool_protocol::SessionId::new("bind-e2e-restore-owner-first").unwrap();
    ...
    async fn compute_turn_injections_after_returns_skipped_ack_without_queue() {
        use axon_tool_protocol::turn_hook::{AfterTurnPayload, TurnHookOutcome, TurnHookRequest};
        let handle = make_handle();
    ...
    async fn compute_turn_injections_before_runs_turn_start_and_replies_noop() {
        use axon_tool_protocol::turn_hook::{BeforeTurnPayload, HookReply, TurnHookRequest};
        let handle = make_handle();
        let reply = handle
            .compute_turn_injections(
                "main",
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.axon`.
    #[tokio::test]
    ...
    fn make_handle_with_queue_routing(
        ...
        WorkspaceHandle,
        Arc<axon_file_utils::queue::UploadQueue>,
        tempfile::TempDir,
    ) {
    ...
    async fn wait_enqueued(queue: &axon_file_utils::queue::UploadQueue, want: u64) {
    ...
    async fn enqueue_workspace_tool_definitions_reports_enqueued_at_session_root() {
        ...
        assert_eq!(outcome, axon_file_utils::queue::EnqueueOutcome::Enqueued);
        assert_eq!(
            queue
                .stats()
                .enqueued
    ...
    async fn two_phase_drain_no_queue_marks_draining_and_returns_zero() {
        ...
        assert_eq!(
            snap.status,
            axon_tool_protocol::ToolServerLifecycleStatus::Draining
        );
        assert!(
