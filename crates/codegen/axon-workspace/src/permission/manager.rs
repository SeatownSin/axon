...
fn permission_mode_artifact_str(mode: axon_telemetry::enums::PermissionMode) -> &'static str {
    use axon_telemetry::enums::PermissionMode;
    match mode {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            axon_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
...
mod tests {
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = axon_acp_lib::acp_gateway::<acp::AgentSide, _>(client);
