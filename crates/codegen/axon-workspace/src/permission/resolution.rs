//! Permission resolution engine: merges native `.axon/config.toml`,
...
/// (fail-safe) with a warn + skip record for `axon inspect`.
...
/// user `~/.axon` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    axon_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.axon/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.axon/config.toml` when there is no git repo).
///
...
fn find_project_axon_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".axon").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".axon").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
fn managed_config_permissions(
    layers: &[axon_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `axon inspect`.
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `axon inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in find_claude_settings_paths(cwd) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `axon inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `axon inspect` via the outer resolver.
...
/// [`axon_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = axon_config::claude_managed_settings_path() else {
...
/// without `~/.axon`); `path` only names the layer in a non-bool warning.
...
mod tests {
    ...
    // Shared so `AXON_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // AXON_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("AXON_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_AXON_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate AXON_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate AXON_HOME so the claude-import marker reads clean (an
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.axon/requirements.toml` is not, despite its path.
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for axon inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        .unwrap();
        assert_eq!(
            axon_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
