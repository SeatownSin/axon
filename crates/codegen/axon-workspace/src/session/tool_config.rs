...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<axon_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<axon_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<axon_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<axon_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn axon_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            axon_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`AXON_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("AXON_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = axon_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `AXON_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: axon_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<axon_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$AXON_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$AXON_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let notification_handle = axon_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    axon_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            axon_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_web_fetch_config() -> axon_tools::implementations::axon_build::web_fetch::WebFetchConfig
{
    ...
    if let Ok(proxy) = std::env::var("AXON_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if axon_config::env_bool("AXON_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "AXON_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use axon_tools::types::resources::{State, WebCitationCounter};
