...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: axon_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, axon_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<axon_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<axon_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<axon_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<axon_hooks::error::HookError>,
    ...
    pub(crate) local_registry: axon_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn axon_tools::implementations::lsp::LspBackend>>,
    ...
    /// (`AXON_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](axon_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`AXON_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, axon_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<axon_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<axon_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](axon_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> axon_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<axon_file_utils::events::EventWriter> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](axon_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<axon_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, axon_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> axon_file_utils::events::EventWriter {
