...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<axon_tool_protocol::ServerId>()
        .err()
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = axon_computer_hub_sdk::DonatingLogLayer::new_inert();
    ...
    let url = Url::parse(&args.hub_url).map_err(|e| anyhow::anyhow!("invalid --hub-url: {e}"))?;
    {
        ...
        let profile = match std::env::var("AXON_SANDBOX_PROFILE").ok() {
            ...
            None if axon_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = axon_sandbox::is_active();
            ...
            tracing::info!(
                ...
                axon_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    ...
    axon_workspace::init_metrics();
    let ws_handle = axon_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    axon_computer_hub_sdk::flush_log_layer();
