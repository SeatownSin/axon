...
async fn call_tool(
    harness: &axon_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<axon_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "AxonBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
