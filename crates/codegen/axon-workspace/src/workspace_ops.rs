...
pub use axon_workspace_types::rpc::WorkspaceRpc;
pub use axon_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use axon_workspace_types::rpc::code_nav::{
...
pub use axon_workspace_types::rpc::fs::{
...
pub use axon_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use axon_workspace_types::rpc::hunks::{
...
pub use axon_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use axon_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use axon_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use axon_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use axon_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &axon_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: axon_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use axon_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &axon_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: axon_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use axon_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: axon_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: axon_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &axon_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: axon_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: axon_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> axon_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => axon_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => axon_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<axon_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = axon_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                axon_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &axon_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<axon_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<axon_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<axon_codebase_graph::QueryResult, axon_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<axon_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](axon_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: axon_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<axon_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](axon_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(axon_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = axon_tool_protocol::ToolId::new(name).map_err(|e| {
                    axon_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = axon_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    axon_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `axon-workspace-types` (and re-exported
    ...
    fn hunk_to_wire_serializes_identically() {
        use axon_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use axon_hunk_tracker::FileContentEntry;
        use axon_hunk_tracker::types::FileContentView;
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = axon_hooks::config::HookSpec {
            ...
            event: axon_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.axon/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = axon_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use axon_hooks::event::HookEventName as E;
    ...
    /// can't depend on `axon-hooks`, and `hook_registry_to_wire` only couples
