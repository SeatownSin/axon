...
pub enum HostedTool {
    ...
    /// This is Axon-specific — not part of the OpenAI Responses API, so it's
...
pub struct ConversationRequest {
    ...
    /// Custom headers for Axon tracking
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-axon-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Axon Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/axon-compaction` operate over axon-build's
...
impl axon_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> axon_compaction::CompactionRole {
        use axon_compaction::CompactionRole;
        // axon-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // axon-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing axon-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<axon_compaction::CompactionFileRef> {
        // axon-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `AxonTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<axon_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl axon_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/axon-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`axon-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/axon-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_axon_turn_idx: req.x_axon_turn_idx,
            x_axon_agent_id: req.x_axon_agent_id,
            x_axon_deployment_id: req.x_axon_deployment_id,
            x_axon_user_id: req.x_axon_user_id,
            trace: None,
        }
    }
}
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch is Axon-specific — not in async_openai's rs::Tool enum.
...
/// Return raw JSON tool definitions for Axon-specific hosted tools that
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The axon-shell agent does NOT use this path — a
...
/// ([crates/codegen/axon-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use axon_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // axon-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "axon-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("axon-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("axon-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("axon-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("axon-3".to_string()));
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in axon-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.axon/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.axon/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.axon/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("axon-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_axon_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_axon_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("axon-3");

        assert_eq!(item.model_id, Some("axon-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("axon-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("axon-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("axon-4.5"));
    ...
    //      (earlier axon-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_axon_build_shape() {
