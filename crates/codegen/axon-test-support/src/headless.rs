...
//!
//! Runs the axon binary as a subprocess with the mock server, captures output.
...
/// Run `axon` with the given args against the mock server, with a 60s timeout.
...
pub async fn run_headless_with_cmd(mut cmd: tokio::process::Command) -> HeadlessResult {
    ...
    let mut child = cmd
        .spawn()
        .unwrap_or_else(|e| panic!("failed to spawn axon binary at {}: {e}", binary.display()));
    ...
    .await
    {
        Ok(result) => (
            result.unwrap_or_else(|e| {
                panic!("failed to wait for axon binary {}: {e}", binary.display())
            }),
            false,
        ),
        Err(_) => {
            ...
            let status = child.wait().await.unwrap_or_else(|e| {
                panic!(
                    "failed to kill timed out axon binary {}: {e}",
                    binary.display()
                )
            });
            (status, true)
